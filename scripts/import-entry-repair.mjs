...
if (!inputPath) {
  throw new Error("Usage: node scripts/import-entry-repair.mjs <entry-repair.csv|jsonl> [--dry-run]");
}
...
if (rows.length === 0) throw new Error(`No entry repair rows found in ${inputPath}`);
...
for (const [index, row] of rows.entries()) {
  ...
  requireText(row, "new_native_word", line);
  requireText(row, "new_display_word", line);
  requireText(row, "source_note", line);
}

if (languageCodes.size > 3) {
  throw new Error(`Entry repair import may contain at most 3 languages, got ${languageCodes.size}`);
}
...
if (blockers.length > 0) {
  throw new Error(`Entry repair preflight failed:\n${blockers.join("\n")}`);
}

const timestamp = new Date().toISOString().replace(/[-:.]/g, "").replace(/Z$/, "Z");
const batchId = `entry_repair_${setId}_${[...languageCodes].join("_").toLowerCase()}_${timestamp}`;
...
insert into generation_batches (
  ...
  ${sqlString(batchId)},
  'translation',
  ${sqlString(`Entry/display repair for ${setId}: ${[...languageCodes].join(",")}`)},
  'structured_repair',
  'generated',
  now(),
  ${sqlString(inputPath)}
...
for (const [index, row] of rows.entries()) {
  ...
  const meaningId = requireText(row, "meaning_id", line);
  const languageCode = requireText(row, "language_code", line);
  const currentNativeWord = requireText(row, "current_native_word", line);
  const currentDisplayWord = requireText(row, "current_display_word", line);
  const newNativeWord = requireText(row, "new_native_word", line);
  const newDisplayWord = requireText(row, "new_display_word", line);
...
begin
  ...
  set
    native_word = ${sqlString(newNativeWord)},
    word_with_article_or_marker = ${sqlString(newDisplayWord)},
    ...
    usage_note = ${sqlNullableString(optionalText(row, "new_usage_note"))},
    quality_status = 'generated',
    source_note = ${sqlString(sourceNote)},
    updated_at = now()
  where meaning_id = ${sqlString(meaningId)}
    and language_code = ${sqlString(languageCode)}
    and native_word = ${sqlString(currentNativeWord)}
    and coalesce(word_with_article_or_marker, native_word) = ${sqlString(currentDisplayWord)};

  if not found then
    raise exception 'Entry repair update failed for %/%', ${sqlString(meaningId)}, ${sqlString(languageCode)};
  end if;
end $$;
`);
...
insert into generation_batch_items (
  ...
  'generated',
  ${sqlString(`entry repaired from ${currentDisplayWord} to ${newDisplayWord}`)}
)
...
console.log(`${dryRun ? "Validated" : "Imported"} entry repair: set_id=${setId} languages=${[...languageCodes].join(",")} rows=${rows.length} batch_id=${batchId}`);

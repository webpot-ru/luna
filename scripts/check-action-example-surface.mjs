...
if (setIds.length === 0) {
  throw new Error("Usage: node scripts/check-action-example-surface.mjs <set_id> [<set_id> ...]");
}
...
from (
  ...
  where msm.set_id in (${sqlLiteralList(setIds)})
    ...
    and et.language_code = 'MY'
  order by msm.set_id, msm.display_order
) rows;
`);

const findings = buildActionExampleSurfaceFindings(rows);
if (findings.blockers.length > 0) {
  console.error(`Action example surface check failed for ${setIds.join(", ")}: ${findings.blockers.length} blocker(s).`);
  for (const blocker of findings.blockers.slice(0, 160)) {
    console.error(formatActionExampleSurfaceFinding(blocker));
  }
...
console.log(`Action example surface OK for ${setIds.join(", ")}: ${findings.checked} MY action example row(s).`);

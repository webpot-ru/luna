...
const SCRIPT_PATH = "scripts/oxford/build-oxford-part003-support-translation-batch-te.mjs";
const DEFAULT_TRANSLATIONS_PATH =
  "config/oxford_3000_core_a1_part_003_300_v1_support_translation_batch_te_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`Telugu example for ${sourceHeadword} must end with sentence punctuation`);
    }
    if (!TELUGU_RE.test(display) || !TELUGU_RE.test(example)) {
      throw new Error(`Telugu row for ${sourceHeadword} must contain Telugu-script text`);
    }
    if (LATIN_RE.test(display) || LATIN_RE.test(example)) {
      throw new Error(`Telugu row for ${sourceHeadword} contains Latin-script fallback`);
    }
    if (UNEXPECTED_SCRIPT_RE.test(display) || UNEXPECTED_SCRIPT_RE.test(example)) {
      throw new Error(`Telugu row for ${sourceHeadword} contains an unexpected non-Telugu script`);
    }
...
function validateTranslationMap(exampleRows, translations) {
  ...
  if (missing.length || extra.length) {
    throw new Error(
      `TE translation source key mismatch: missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)}`
    );
  }
}
...
function buildSupportRow(row, translation, generatedAt) {
  return {
    ...
    article_display_policy: "not_applicable_no_article_layer_for_te",
...
function updateContract(contract, batchPath, summaryPath, rows) {
  ...
  contract.next_stage_options = [
    "Generate KN support-language translations/examples for Part 003 in the Oxford English-learning contour.",
    "Continue support-language translations/examples in documented order after KN.",
...
await writeFile(
  ...
  [
    ...
    "- Script-aware validation: TE Telugu native Telugu orthography, Telugu display/example presence, sentence punctuation, no Latin fallback and unexpected-script leak guard",

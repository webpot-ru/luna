...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-pl.mjs";
const DEFAULT_TRANSLATIONS = "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_pl_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tPL\texample_PL") {
    throw new Error(`Unexpected PL translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    if (!LATIN_RE.test(display) || !LATIN_RE.test(example)) {
      throw new Error(`Bad PL translation row ${index + 2}: display/example must contain Latin letters`);
    }
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_pl_v1",

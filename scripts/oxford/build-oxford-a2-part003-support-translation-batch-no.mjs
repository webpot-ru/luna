...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-no.mjs";
const DEFAULT_TRANSLATIONS =
  "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_no_article_display_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tNO\texample_NO") {
    throw new Error(`Unexpected NO translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    assertNorwegianText(display, `${sourceHeadword} NO`);
    assertNorwegianText(example, `${sourceHeadword} example_NO`);
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_no_article_display_v1",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: enabled where grammatically useful for Norwegian nouns",

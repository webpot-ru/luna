...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-te.mjs";
const DEFAULT_TRANSLATIONS = "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_te_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tTE\texample_TE") {
    throw new Error(`Unexpected TE translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    const parts = line.split("\t");
    if (parts.length !== 3) {
      throw new Error(`Bad TE translation row ${index + 2}: expected 3 tab-separated columns`);
    }
    const [sourceHeadword, display, example] = parts.map(normalizeText);
    if (!sourceHeadword || !display || !example) {
      throw new Error(`Bad TE translation row ${index + 2}: empty field`);
    }
    assertTeluguText(display, `${sourceHeadword} TE`);
    assertTeluguText(example, `${sourceHeadword} example_TE`);
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`Bad TE example punctuation for ${sourceHeadword}`);
    }
    if (map.has(sourceHeadword)) {
      throw new Error(`Duplicate TE translation key: ${sourceHeadword}`);
    }
...
function validateTranslationMap(rows, translations) {
  ...
  if (missing.length) {
    throw new Error(`Missing TE translations for source headwords: ${missing.join(", ")}`);
  }
  if (extra.length) {
    throw new Error(`TE translation map has unused rows: ${extra.join(", ")}`);
  }
}
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_te_v1",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: not included for Telugu",

...
const SCRIPT_VERSION = '2026-05-21.v2';
const LANGUAGE = 'MY';
const BATCH_ID = 'my_v1';
const SCRIPT_PATH = 'scripts/oxford/build-oxford-a2-part001-support-translation-batch-my.mjs';
...
const DEFAULT_TRANSLATIONS = path.join(
  PROJECT_ROOT,
  'config',
  `${RELEASE_ID}_support_translation_batch_my_v1.tsv`,
);
const DEFAULT_OUTPUT = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_my_v1.jsonl`,
);
const DEFAULT_SUMMARY = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_my_v1_summary.md`,
);

const SENTENCE_END_RE = /[.!?။]$/u;
const MYANMAR_RE = /\p{Script=Myanmar}/u;
const UNEXPECTED_SCRIPT_RE =
  /[A-Za-z\u0400-\u04FF\u0590-\u05FF\u0600-\u06FF\u0900-\u097F\u0980-\u09FF\u0B80-\u0BFF\u0C00-\u0C7F\u0C80-\u0CFF\u0D00-\u0D7F\u0E00-\u0E7F\u0E80-\u0EFF\u1780-\u17FF\u3040-\u30FF\u3400-\u9FFF\uAC00-\uD7AF]/u;
...
function readTranslations(filePath) {
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    assertBurmeseText(display, `${headword} MY`);
    assertBurmeseText(example, `${headword} example_MY`);
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`${filePath}:${lineIndex + 2}: Burmese example must end with sentence punctuation`);
    }
    if (map.has(headword)) {
...
function assertBurmeseText(value, context) {
  if (!MYANMAR_RE.test(value)) {
    throw new Error(`${context}: expected Myanmar-script Burmese text`);
  }
  if (UNEXPECTED_SCRIPT_RE.test(value)) {
    throw new Error(`${context}: unexpected non-Myanmar script`);
  }
}
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: 'codex_oxford_a2_part001_support_translation_batch_my_v1',
...
function updateContract(contract, opts, rows) {
  ...
  let replaced = false;
  contract.latest_support_translation_batches = existing.map((item) => {
    if (item.batch_id === BATCH_ID || item.languages?.includes(LANGUAGE)) {
      replaced = true;
      return batch;
    }
    return item;
  ...
  if (!replaced) {
    contract.latest_support_translation_batches.push(batch);
  }

  const coveredLanguages = new Set(
    contract.latest_support_translation_batches.flatMap((item) => item.languages ?? []),
  );
  if (coveredLanguages.size >= 52) {
    contract.status = 'support_language_batches_complete_not_delivery_ready';
    contract.next_stage_options = [
      'Run weak-language targeted review, deterministic sample reviews and source-backed support-language audits.',
      'Run support article display repair/check for article languages.',
      'After source-package QA gates pass, auto-publish final US/UK workbooks as native Google Sheets and run readback; do not import to ordinary deck-table Postgres or isolated Oxford DB.',
    ];
  } else {
    contract.status = 'support_language_batches_in_progress_not_delivery_ready';
    contract.next_stage_options = [
      'Generate the next support-language batch in documented order: KM.',
...
function main() {
  ...
  if (translations.size !== sourceRows.length) {
    throw new Error(`Expected ${sourceRows.length} MY translations, found ${translations.size}`);
  }
  ...
  const outputRows = sourceRows.map((row, index) => {
    ...
    if (!translation) {
      throw new Error(`Missing MY translation for ${sourceHeadword}`);
    }
  ...
  if (extra.length > 0) {
    throw new Error(`Unexpected MY translations: ${extra.join(', ')}`);
  }
  ...
  writeFileSync(
    opts.summary,
    [
      `# Oxford A2 Part 001 Support Translation Batch ${BATCH_ID}: ${RELEASE_ID}`,
      '',
      ...
      `- Languages: ${LANGUAGE}`,
      '- Article display: not applicable for Burmese',

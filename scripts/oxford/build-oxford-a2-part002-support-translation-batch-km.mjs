...
const SCRIPT_PATH = 'scripts/oxford/build-oxford-a2-part002-support-translation-batch-km.mjs';
...
const DEFAULT_TRANSLATIONS = path.join(
  PROJECT_ROOT,
  'config',
  `${RELEASE_ID}_support_translation_batch_km_v1.tsv`,
);
const DEFAULT_OUTPUT = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_km_v1.jsonl`,
);
const DEFAULT_SUMMARY = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_km_v1_summary.md`,
);
...
const UNEXPECTED_SCRIPT_RE =
  /[A-Za-z\u0400-\u04FF\u0590-\u05FF\u0600-\u06FF\u0900-\u097F\u0980-\u09FF\u0B80-\u0BFF\u0C00-\u0C7F\u0C80-\u0CFF\u0D00-\u0D7F\u0E00-\u0E7F\u0E80-\u0EFF\u1000-\u109F\u3040-\u30FF\u3400-\u9FFF\uAC00-\uD7AF]/u;
...
function assertKhmerText(value, context) {
  if (!KHMER_RE.test(value)) {
    throw new Error(`${context}: expected Khmer-script Khmer text`);
  }
  if (UNEXPECTED_SCRIPT_RE.test(value)) {
    throw new Error(`${context}: unexpected non-Khmer script`);
  }
}
...
function readTranslations(filePath) {
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    assertKhmerText(display, `${headword} KM`);
    assertKhmerText(example, `${headword} example_KM`);
    if (KHMER_TOKEN_SPACE_RE.test(example)) {
      throw new Error(`${filePath}:${lineIndex + 2}: Khmer example contains generated token spaces`);
    }
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`${filePath}:${lineIndex + 2}: Khmer example must end with sentence punctuation`);
    }
    if (map.has(headword)) {
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: 'codex_oxford_a2_part002_support_translation_batch_km_v1',
...
function updateContract(contract, opts, rows) {
  ...
  contract.next_stage_options = [
    'Generate the next support-language batch in documented order: LO.',
...
function main() {
  ...
  if (translations.size !== sourceRows.length) {
    throw new Error(`Expected ${sourceRows.length} KM translations, found ${translations.size}`);
  }
  ...
  const outputRows = sourceRows.map((row, index) => {
    ...
    if (!translation) {
      throw new Error(`Missing KM translation for ${sourceHeadword}`);
    }
  ...
  if (extra.length > 0) {
    throw new Error(`Unexpected KM translations: ${extra.join(', ')}`);
  }
  ...
  writeFileSync(
    ...
    [
      ...
      `- Languages: ${LANGUAGE}`,
      '- Article display: not applicable for Khmer',
      '- Translation status: `draft_native_style_needs_source_assisted_qa`',
      '- Example status: `draft_scene_preserving_needs_source_assisted_qa`',
      '- Script-aware validation: KM Khmer script presence, sentence punctuation, no generated token spaces inside examples and unexpected-script leak guard',

...
const SCRIPT_PATH = "scripts/oxford/build-oxford-part003-support-translation-batch-kn.mjs";
const DEFAULT_TRANSLATIONS_PATH =
  "config/oxford_3000_core_a1_part_003_300_v1_support_translation_batch_kn_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`Kannada example for ${sourceHeadword} must end with sentence punctuation`);
    }
    if (!KANNADA_RE.test(display) || !KANNADA_RE.test(example)) {
      throw new Error(`Kannada row for ${sourceHeadword} must contain Kannada-script text`);
    }
    if (LATIN_RE.test(display) || LATIN_RE.test(example)) {
      throw new Error(`Kannada row for ${sourceHeadword} contains Latin-script fallback`);
    }
    if (UNEXPECTED_SCRIPT_RE.test(display) || UNEXPECTED_SCRIPT_RE.test(example)) {
      throw new Error(`Kannada row for ${sourceHeadword} contains an unexpected non-Kannada script`);
    }
...
function validateTranslationMap(exampleRows, translations) {
  ...
  if (missing.length || extra.length) {
    throw new Error(
      `KN translation source key mismatch: missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)}`
    );
  }
}
...
function buildSupportRow(row, translation, generatedAt) {
  return {
    ...
    article_display_policy: "not_applicable_no_article_layer_for_kn",
    target_language_transcription_included: false,
...
function updateContract(contract, batchPath, summaryPath, rows) {
  ...
  contract.next_stage_options = [
    "Generate ML support-language translations/examples for Part 003 in the Oxford English-learning contour.",
    "Continue support-language translations/examples in documented order after ML.",
...
if (exampleRows.length !== RELEASE_ENTRY_COUNT) {
  throw new Error(`Expected ${RELEASE_ENTRY_COUNT} English example rows, got ${exampleRows.length}`);
}
if (exampleRows.some((row) => row.release_id !== RELEASE_ID)) {
  throw new Error(`English examples artifact is not exclusively for ${RELEASE_ID}`);
}
...
await writeFile(
  ...
  [
    ...
    "- Script-aware validation: KN Kannada native Kannada orthography, Kannada display/example presence, sentence punctuation, no Latin fallback and unexpected-script leak guard",
    "- Article display layer: not included",

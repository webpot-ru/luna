...
const SCRIPT_PATH = "scripts/oxford/build-oxford-b1-part001-support-translation-batch-km.mjs";
const DEFAULT_TRANSLATIONS = `config/${RELEASE_ID}_support_translation_batch_km_v1.tsv`;
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tKM\texample_KM") {
    throw new Error(`Unexpected KM translation TSV header in ${filePath}`);
  }
  const rows = [];
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    assertKhmerText(display, `${sourceHeadword} KM`);
    assertKhmerText(example, `${sourceHeadword} example_KM`);
...
function validateTranslationRows(exampleRows, translations) {
  ...
  if (extra.length) {
    throw new Error(`KM translation TSV has unused rows: ${extra.join(", ")}`);
  }
  for (const [index, sourceKey] of sourceKeys.entries()) {
    if (translationKeys[index] !== sourceKey) {
      throw new Error(
        `KM translation row order mismatch at row ${index + 2}: expected ${sourceKey}, found ${translationKeys[index]}`
      );
    }
  }
}
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_b1_part001_support_translation_batch_km_v1",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: not included for Khmer",

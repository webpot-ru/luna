...
const SCRIPT_PATH = 'scripts/oxford/build-oxford-a2-part001-support-translation-batch-ms.mjs';
...
const DEFAULT_TRANSLATIONS = path.join(
  PROJECT_ROOT,
  'config',
  `${RELEASE_ID}_support_translation_batch_ms_v1.tsv`,
);
const DEFAULT_OUTPUT = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_ms_v1.jsonl`,
);
const DEFAULT_SUMMARY = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_ms_v1_summary.md`,
);
...
function readTranslations(filePath) {
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    assertMalayText(display, `${headword} MS`);
    assertMalayText(example, `${headword} example_MS`);
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`${filePath}:${lineIndex + 2}: Malay example must end with sentence punctuation`);
    }
    if (map.has(headword)) {
...
function assertMalayText(value, context) {
  if (!LATIN_RE.test(value)) {
    throw new Error(`${context}: expected Latin-script Malay text`);
  }
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: 'codex_oxford_a2_part001_support_translation_batch_ms_v1',
...
function updateContract(contract, opts, rows) {
  ...
  contract.next_stage_options = [
    'Generate the next support-language batch in documented order: ID.',
...
function main() {
  ...
  if (translations.size !== sourceRows.length) {
    throw new Error(`Expected ${sourceRows.length} MS translations, found ${translations.size}`);
  }
  ...
  const outputRows = sourceRows.map((row, index) => {
    ...
    if (!translation) {
      throw new Error(`Missing MS translation for ${sourceHeadword}`);
    }
  ...
  if (extra.length > 0) {
    throw new Error(`Unexpected MS translations: ${extra.join(', ')}`);
  }

...
const SCRIPT_PATH = "scripts/oxford/build-oxford-part004-support-translation-batch-pt-br.mjs";
const DEFAULT_TRANSLATIONS_PATH =
  "config/oxford_3000_core_a1_part_004_147_v1_support_translation_batch_pt_br_article_display_v1.tsv";
...
const EUROPEAN_PORTUGUESE_RISK_RE =
  /\b(pequeno-almoço|comboio|rapariga|fixe|chávena|natas|rés do chão|ginásio|gelado|sumo|ganga|trabalhos de casa|condutor|condutora|autocarro|facto|catorze|dezasseis|dezassete|dezanove|vemo-nos|telemóvel|ementa|sandes|duche|camisola|secção|ténis|bilhete|casa de banho|sanita|tu|teu|tua|teus|tuas|ti)\b/iu;
...
async function parseTranslations(filePath) {
  ...
  if (lines.length !== RELEASE_ENTRY_COUNT) {
    throw new Error(`Expected ${RELEASE_ENTRY_COUNT} PT-BR translation rows, found ${lines.length}`);
  }
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`Brazilian Portuguese example for ${sourceHeadword} must end with sentence punctuation`);
    }
    if (!LATIN_SCRIPT_RE.test(display) || !LATIN_SCRIPT_RE.test(example)) {
      throw new Error(`Brazilian Portuguese row for ${sourceHeadword} must contain Latin-script text`);
    }
    if (UNEXPECTED_SCRIPT_RE.test(display) || UNEXPECTED_SCRIPT_RE.test(example)) {
      throw new Error(`Brazilian Portuguese row for ${sourceHeadword} contains an unexpected non-Latin script`);
    }
    const combined = `${display} ${example}`;
    if (EUROPEAN_PORTUGUESE_RISK_RE.test(combined) || EUROPEAN_PROGRESSIVE_RE.test(combined)) {
      throw new Error(`Brazilian Portuguese row for ${sourceHeadword} still contains European Portuguese risk text`);
    }
...
function validateTranslationMap(exampleRows, translations) {
  ...
  if (missing.length || extra.length || orderMismatch !== -1) {
    throw new Error(
      `PT-BR translation source key mismatch: missing=${JSON.stringify(missing)} extra=${JSON.stringify(
        extra
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    support_translation_source: "codex_authored_brazilian_portuguese_part004_support_tsv_not_oxford",
    support_example_source: "codex_authored_brazilian_portuguese_part004_support_tsv_not_oxford",
    batch_id: BATCH_ID,
...
function updateContract(contract, batchPath, summaryPath, rows, generatedAt) {
  ...
  contract.next_stage_options = [
    "Generate the next support-language batch in language order: ES-419.",

...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-lo.mjs";
const DEFAULT_TRANSLATIONS = "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_lo_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tLO\texample_LO") {
    throw new Error(`Unexpected LO translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    assertLaoText(display, `${sourceHeadword} LO`);
    assertLaoText(example, `${sourceHeadword} example_LO`);
    if (LAO_TOKEN_SPACE_RE.test(example)) {
      throw new Error(`Bad LO example spacing for ${sourceHeadword}: generated Lao token spaces`);
    }
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_lo_v1",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: not included for Lao",

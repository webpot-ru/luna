...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-sv.mjs";
const DEFAULT_TRANSLATIONS =
  "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_sv_article_display_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tSV\texample_SV") {
    throw new Error(`Unexpected SV translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    assertSwedishText(display, `${sourceHeadword} SV`);
    assertSwedishText(example, `${sourceHeadword} example_SV`);
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_sv_article_display_v1",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: enabled where grammatically useful for Swedish nouns",

...
const SCRIPT_PATH = "scripts/oxford/build-oxford-b1-part001-support-translation-batch-ru.mjs";
const DEFAULT_TRANSLATIONS = `config/${RELEASE_ID}_support_translation_batch_ru_v1.tsv`;
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tRU\texample_RU") {
    throw new Error(`Unexpected RU translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    assertRussianText(display, `${sourceHeadword} RU`);
    assertRussianText(example, `${sourceHeadword} example_RU`);
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_b1_part001_support_translation_batch_ru_v1",

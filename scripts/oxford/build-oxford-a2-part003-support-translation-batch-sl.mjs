...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-sl.mjs";
const DEFAULT_TRANSLATIONS = "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_sl_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tSL\texample_SL") {
    throw new Error(`Unexpected SL translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    assertSlovenianText(display, `${sourceHeadword} SL`);
    assertSlovenianText(example, `${sourceHeadword} example_SL`);
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_sl_v1",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: not included for Slovenian",

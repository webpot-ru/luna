...
const SCRIPT_PATH = "scripts/oxford/build-oxford-b1-part001-support-translation-batch-uz.mjs";
const DEFAULT_TRANSLATIONS = `config/${RELEASE_ID}_support_translation_batch_uz_v1.tsv`;
...
function validateTranslationRows(exampleRows, translations) {
  ...
  if (extra.length) {
    throw new Error(`UZ translation TSV has unused rows: ${extra.join(", ")}`);
  }
  for (const [index, sourceKey] of sourceKeys.entries()) {
    if (translationKeys[index] !== sourceKey) {
      throw new Error(
        `UZ translation row order mismatch at row ${index + 2}: expected ${sourceKey}, found ${translationKeys[index]}`
      );
    }
  }
}
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_b1_part001_support_translation_batch_uz_v1",

...
const SCRIPT_PATH = "scripts/oxford/build-oxford-part002-support-translation-batch-lo.mjs";
const DEFAULT_TRANSLATIONS_PATH =
  "config/oxford_3000_core_a1_part_002_300_v1_support_translation_batch_lo_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`Lao example for ${sourceHeadword} must end with sentence punctuation`);
    }
    if (!LAO_RE.test(display) || !LAO_RE.test(example)) {
      throw new Error(`Lao row for ${sourceHeadword} must contain Lao-script text`);
    }
    if (UNEXPECTED_SCRIPT_RE.test(display) || UNEXPECTED_SCRIPT_RE.test(example)) {
      throw new Error(`Lao row for ${sourceHeadword} contains an unexpected non-Lao script`);
    }
...
function validateTranslationMap(exampleRows, translations) {
  ...
  if (missing.length || extra.length) {
    throw new Error(
      `LO translation source key mismatch: missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)}`
    );
  }
}
...
await writeFile(
  ...
  [
    ...
    "- Script-aware validation: LO Lao native Lao orthography, Lao display/example presence, sentence punctuation and unexpected-script leak guard",

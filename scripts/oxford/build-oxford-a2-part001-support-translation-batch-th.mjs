...
const SCRIPT_PATH = 'scripts/oxford/build-oxford-a2-part001-support-translation-batch-th.mjs';
...
const DEFAULT_TRANSLATIONS = path.join(
  PROJECT_ROOT,
  'config',
  `${RELEASE_ID}_support_translations_th_v1.tsv`,
);
const DEFAULT_OUTPUT = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_th_v1.jsonl`,
);
const DEFAULT_SUMMARY = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_th_v1_summary.md`,
);
...
function readTranslations(filePath) {
  ...
  if (header !== 'source_headword\tdisplay_translation\texample_translation') {
...
function assertThaiText(value, context) {
  if (!THAI_RE.test(value)) {
    throw new Error(`${context}: expected Thai-script text`);
  }
}

function assertExampleShape(value, context) {
  assertThaiText(value, context);
  if (!/[.!?]$/u.test(value)) {
    throw new Error(`${context}: Thai example must end with ASCII sentence punctuation`);
  }
  if (THAI_TOKENIZATION_SPACE_RE.test(value)) {
    throw new Error(`${context}: Thai example should not contain spaces between Thai tokens`);
  }
}
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: 'codex_oxford_a2_part001_support_translation_batch_th_v1',
...
function updateContract(contract, opts, rows) {
  ...
  contract.next_stage_options = [
    'Generate the next support-language batch in documented order: MS.',
...
function main() {
  ...
  if (translations.size !== sourceRows.length) {
    throw new Error(`Expected ${sourceRows.length} TH translations, found ${translations.size}`);
  }
  ...
  const outputRows = sourceRows.map((row, index) => {
    ...
    if (!sourceHeadword) {
      throw new Error(`Source row ${index + 1}: missing headword`);
    }
    ...
    if (!translation) {
      throw new Error(`Missing TH translation for ${sourceHeadword}`);
    }

    assertThaiText(translation.display, `${sourceHeadword} display_translation`);
    assertExampleShape(translation.example, `${sourceHeadword} example_translation`);
  ...
  if (extra.length > 0) {
    throw new Error(`Unexpected TH translations: ${extra.join(', ')}`);
  }

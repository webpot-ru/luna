...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-vi.mjs";
const DEFAULT_TRANSLATIONS = "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_vi_v1.tsv";
const SENTENCE_END_RE = /[.!?。！？]$/u;
const VIETNAMESE_RE = /[ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚÝĂĐĨŨƠƯàáâãèéêìíòóôõùúýăđĩũơưẠ-ỹ]/u;
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tVI\texample_VI") {
    throw new Error(`Unexpected VI translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    if (!VIETNAMESE_RE.test(display) || !VIETNAMESE_RE.test(example)) {
      throw new Error(`Bad VI translation row ${index + 2}: display/example must contain Vietnamese diacritics`);
    }
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_vi_v1",
...
function updateContract(contract, batchPath, summaryPath, translationsPath, rows) {
  ...
  contract.next_stage_options = [
    "Generate the next support-language batch in documented order: TH.",

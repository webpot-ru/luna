...
const SCRIPT_PATH = "scripts/oxford/build-oxford-part002-support-translation-batch-hi.mjs";
const DEFAULT_TRANSLATIONS_PATH =
  "config/oxford_3000_core_a1_part_002_300_v1_support_translation_batch_hi_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`Hindi example for ${sourceHeadword} must end with sentence punctuation`);
    }
    if (!DEVANAGARI_RE.test(display) || !DEVANAGARI_RE.test(example)) {
      throw new Error(`Hindi row for ${sourceHeadword} must contain Devanagari-script text`);
    }
    if (UNEXPECTED_SCRIPT_RE.test(display) || UNEXPECTED_SCRIPT_RE.test(example)) {
      throw new Error(`Hindi row for ${sourceHeadword} contains an unexpected non-Devanagari script`);
    }
...
function validateTranslationMap(exampleRows, translations) {
  ...
  if (missing.length || extra.length) {
    throw new Error(
      `HI translation source key mismatch: missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)}`
    );
  }
}
...
await writeFile(
  ...
  [
    ...
    "- Script-aware validation: HI Devanagari native orthography, Hindi display/example presence, sentence punctuation and unexpected-script leak guard",

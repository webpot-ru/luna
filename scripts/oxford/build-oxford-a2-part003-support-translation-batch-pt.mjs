...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-pt.mjs";
const DEFAULT_TRANSLATIONS =
  "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_pt_article_display_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tPT\texample_PT") {
    throw new Error(`Unexpected PT translation TSV header in ${filePath}`);
  }
  const map = new Map();
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_pt_article_display_v1",
...
function updateContract(contract, batchPath, summaryPath, translationsPath, rows) {
  ...
  contract.next_stage_options = [
    "Generate the next support-language batch in documented order: ZH.",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: true where grammatically useful for European Portuguese nouns",

...
const SCRIPT_PATH = "scripts/oxford/build-oxford-part004-support-translation-batch-kk.mjs";
const DEFAULT_TRANSLATIONS_PATH =
  "config/oxford_3000_core_a1_part_004_147_v1_support_translation_batch_kk_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (lines.length !== RELEASE_ENTRY_COUNT) {
    throw new Error(`Expected ${RELEASE_ENTRY_COUNT} KK translation rows, found ${lines.length}`);
  }
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_part004_support_translation_batch_kk_v1",
...
function updateContract(contract, batchPath, summaryPath, rows, generatedAt) {
  ...
  contract.next_stage_options = [
    "Generate the next support-language batch in language order: AZ.",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: false; Kazakh uses normal Kazakh Cyrillic citation/base forms without artificial articles",
    "- Translation status: `draft_native_style_needs_source_assisted_qa`",
    "- Example status: `draft_scene_preserving_needs_source_assisted_qa`",
    "- Script-aware validation: KK Kazakh Cyrillic display/example cells, sentence punctuation, no Latin fallback and no unexpected-script leakage",

...
const SCRIPT_PATH = 'scripts/oxford/build-oxford-a2-part001-support-translation-batch-lo.mjs';
...
const DEFAULT_TRANSLATIONS = path.join(
  PROJECT_ROOT,
  'config',
  `${RELEASE_ID}_support_translation_batch_lo_v1.tsv`,
);
const DEFAULT_OUTPUT = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_lo_v1.jsonl`,
);
const DEFAULT_SUMMARY = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_lo_v1_summary.md`,
);
...
function readTranslations(filePath) {
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    assertLaoText(display, `${headword} LO`);
    assertLaoText(example, `${headword} example_LO`);
    if (LAO_TOKEN_SPACE_RE.test(example)) {
      throw new Error(`${filePath}:${lineIndex + 2}: Lao example contains generated token spaces`);
    }
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`${filePath}:${lineIndex + 2}: Lao example must end with sentence punctuation`);
    }
    if (map.has(headword)) {
...
function assertLaoText(value, context) {
  if (!LAO_RE.test(value)) {
    throw new Error(`${context}: expected Lao-script text`);
  }
  if (LATIN_RE.test(value)) {
    throw new Error(`${context}: Latin fallback text is not allowed`);
  }
  if (UNEXPECTED_SCRIPT_RE.test(value)) {
    throw new Error(`${context}: unexpected non-Lao script`);
  }
}
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: 'codex_oxford_a2_part001_support_translation_batch_lo_v1',
...
function updateContract(contract, opts, rows) {
  ...
  contract.next_stage_options = [
    'Generate the next support-language batch in documented order: NE.',
...
function main() {
  ...
  if (translations.size !== sourceRows.length) {
    throw new Error(`Expected ${sourceRows.length} LO translations, found ${translations.size}`);
  }
  ...
  const outputRows = sourceRows.map((row, index) => {
    ...
    if (!translation) {
      throw new Error(`Missing LO translation for ${sourceHeadword}`);
    }
  ...
  if (extra.length > 0) {
    throw new Error(`Unexpected LO translations: ${extra.join(', ')}`);
  }
  ...
  writeFileSync(
    ...
    [
      ...
      `- Languages: ${LANGUAGE}`,
      '- Article display: not applicable for Lao',
      '- Translation status: `draft_native_style_needs_source_assisted_qa`',
      '- Example status: `draft_scene_preserving_needs_source_assisted_qa`',
      '- Script-aware validation: Lao native-script text, no Latin fallback, no generated Lao token spaces, sentence punctuation and unexpected-script leak guard',

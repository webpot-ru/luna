...
const SCRIPT_PATH = "scripts/oxford/build-oxford-part002-support-translation-batch-bn.mjs";
const DEFAULT_TRANSLATIONS_PATH =
  "config/oxford_3000_core_a1_part_002_300_v1_support_translation_batch_bn_v1.tsv";
const SENTENCE_END_RE = /[.!?।]$/u;
const BENGALI_RE = /\p{Script=Bengali}/u;
const UNEXPECTED_SCRIPT_RE =
  /[\u0400-\u04FF\u0590-\u05FF\u0600-\u06FF\u0900-\u0963\u0966-\u097F\u0A80-\u0AFF\u0B80-\u0BFF\u0C00-\u0C7F\u0C80-\u0CFF\u0D00-\u0DFF\u0E00-\u0E7F\u0E80-\u0EFF\u1000-\u109F\u1780-\u17FF\u3040-\u30FF\u3400-\u9FFF\uAC00-\uD7AF]/u;
...
async function parseTranslations(filePath) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`Bengali example for ${sourceHeadword} must end with sentence punctuation`);
    }
    if (!BENGALI_RE.test(display) || !BENGALI_RE.test(example)) {
      throw new Error(`Bengali row for ${sourceHeadword} must contain Bengali-script text`);
    }
    if (UNEXPECTED_SCRIPT_RE.test(display) || UNEXPECTED_SCRIPT_RE.test(example)) {
      throw new Error(`Bengali row for ${sourceHeadword} contains an unexpected non-Bengali script`);
    }
...
function validateTranslationMap(exampleRows, translations) {
  ...
  if (missing.length || extra.length) {
    throw new Error(
      `BN translation source key mismatch: missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)}`
    );
  }
}

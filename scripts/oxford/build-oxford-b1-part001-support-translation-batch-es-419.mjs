...
const SCRIPT_PATH = "scripts/oxford/build-oxford-b1-part001-support-translation-batch-es-419.mjs";
const DEFAULT_TRANSLATIONS = `config/${RELEASE_ID}_support_translation_batch_es_419_article_display_v1.tsv`;
...
const SPAIN_SPANISH_RISK_RE =
  /\b(ordenador|zumo|vaqueros|coche|coches|billete|coste|nata|móvil|patata|bocadillo|aparcar|aparcamos|echar de menos|echo de menos|jersey|vosotros|vuestro|vuestra|vuestros|vuestras|bolígrafo|autobús|coger|coge|coged|cogemos|cogí|cogió|cogieron|coges|cogéis|piso de arriba|camarero|furgoneta|zapatilla|zapatillas|apetecer|apetece)\b/iu;
...
function validateTranslationRows(exampleRows, translations) {
  ...
  if (extra.length) {
    throw new Error(`ES-419 translation TSV has unused rows: ${extra.join(", ")}`);
  }
  for (const [index, sourceKey] of sourceKeys.entries()) {
    if (translationKeys[index] !== sourceKey) {
      throw new Error(
        `ES-419 translation row order mismatch at row ${index + 2}: expected ${sourceKey}, found ${translationKeys[index]}`
      );
    }
  }
}
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_b1_part001_support_translation_batch_es_419_article_display_v1",

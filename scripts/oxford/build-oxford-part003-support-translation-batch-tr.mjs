...
const SCRIPT_PATH = "scripts/oxford/build-oxford-part003-support-translation-batch-tr.mjs";
const DEFAULT_TRANSLATIONS_PATH =
  "config/oxford_3000_core_a1_part_003_300_v1_support_translation_batch_tr_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`Turkish example for ${sourceHeadword} must end with sentence punctuation`);
    }
    if (!LATIN_SCRIPT_RE.test(display) || !LATIN_SCRIPT_RE.test(example)) {
      throw new Error(`Turkish row for ${sourceHeadword} must contain Latin-script text`);
    }
    if (UNEXPECTED_SCRIPT_RE.test(display) || UNEXPECTED_SCRIPT_RE.test(example)) {
      throw new Error(`Turkish row for ${sourceHeadword} contains an unexpected non-Latin script`);
    }
...
function validateTranslationMap(exampleRows, translations) {
  ...
  if (missing.length || extra.length) {
    throw new Error(
      `TR translation source key mismatch: missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)}`
    );
  }
}
...
function buildSupportRow(row, translation, generatedAt) {
  return {
    ...
    article_display_policy: "not_applicable_no_article_layer_for_tr",
...
function updateContract(contract, batchPath, summaryPath, rows) {
  ...
  contract.next_stage_options = [
    "Generate SW support-language translations/examples for Part 003 in the Oxford English-learning contour.",
    "Continue support-language translations/examples in documented order after SW.",
...
await writeFile(
  ...
  [
    ...
    "- Script-aware validation: TR Turkish Latin orthography, Latin display/example presence, sentence punctuation and unexpected-script leak guard",

...
const SCRIPT_PATH = "scripts/oxford/build-oxford-part002-support-translation-batch-tl.mjs";
const DEFAULT_TRANSLATIONS_PATH =
  "config/oxford_3000_core_a1_part_002_300_v1_support_translation_batch_tl_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`Filipino example for ${sourceHeadword} must end with sentence punctuation`);
    }
    if (!LATIN_RE.test(display) || !LATIN_RE.test(example)) {
      throw new Error(`Filipino row for ${sourceHeadword} must contain Latin-script text`);
    }
    if (UNEXPECTED_SCRIPT_RE.test(display) || UNEXPECTED_SCRIPT_RE.test(example)) {
      throw new Error(`Filipino row for ${sourceHeadword} contains an unexpected non-Latin script`);
    }
...
function validateTranslationMap(exampleRows, translations) {
  ...
  if (missing.length || extra.length) {
    throw new Error(
      `TL translation source key mismatch: missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)}`
    );
  }
}

...
const SCRIPT_PATH = 'scripts/oxford/build-oxford-a2-part001-support-translation-batch-az.mjs';
...
const DEFAULT_TRANSLATIONS = path.join(
  PROJECT_ROOT,
  'config',
  `${RELEASE_ID}_support_translation_batch_az_v1.tsv`,
);
const DEFAULT_OUTPUT = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_az_v1.jsonl`,
);
const DEFAULT_SUMMARY = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_az_v1_summary.md`,
);
...
function readTranslations(filePath) {
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    assertAzerbaijaniText(display, `${headword} AZ`);
    assertAzerbaijaniText(example, `${headword} example_AZ`);
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`${filePath}:${lineIndex + 2}: Azerbaijani example must end with sentence punctuation`);
    }
    if (map.has(headword)) {
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: 'codex_oxford_a2_part001_support_translation_batch_az_v1',
...
function main() {
  ...
  if (translations.size !== sourceRows.length) {
    throw new Error(`Expected ${sourceRows.length} AZ translations, found ${translations.size}`);
  }
  ...
  const outputRows = sourceRows.map((row, index) => {
    ...
    if (!translation) {
      throw new Error(`Missing AZ translation for ${sourceHeadword}`);
    }
  ...
  if (extra.length > 0) {
    throw new Error(`Unexpected AZ translations: ${extra.join(', ')}`);
  }
  ...
  writeFileSync(
    ...
    [
      ...
      `- Languages: ${LANGUAGE}`,
      '- Article display: false; Azerbaijani uses normal Azerbaijani Latin citation/base forms without artificial articles',
      '- Translation status: `draft_native_style_needs_source_assisted_qa`',
      '- Example status: `draft_scene_preserving_needs_source_assisted_qa`',
      '- Script-aware validation: Azerbaijani Latin display/example cells, sentence punctuation and no unexpected-script leakage',

...
const SCRIPT_PATH = "scripts/oxford/build-oxford-b1-part001-support-translation-batch-th.mjs";
const DEFAULT_TRANSLATIONS = `config/${RELEASE_ID}_support_translation_batch_th_v1.tsv`;
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tTH\texample_TH") {
    throw new Error(`Unexpected TH translation TSV header in ${filePath}`);
  }
  const map = new Map();
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_b1_part001_support_translation_batch_th_v1",
...
function updateContract(contract, batchPath, summaryPath, translationsPath, rows) {
  ...
  const hadExisting = existing.some((item) => item.batch_id === BATCH_ID || item.languages?.includes(LANGUAGE));
  contract.latest_support_translation_batches = hadExisting
    ? existing.map((item) => (item.batch_id === BATCH_ID || item.languages?.includes(LANGUAGE) ? batch : item))
    : [...existing, batch];
  const coveredLanguages = new Set(contract.latest_support_translation_batches.flatMap((item) => item.languages ?? []));
  contract.status =
    coveredLanguages.size >= 52
  ...
  contract.next_stage_options =
    coveredLanguages.size >= 52
      ? [
          "Run weak-language targeted review, support translation sample review, all-fields sample review, source-backed support-language audit, support example quality audit and support article display repair/check.",
          "Export final US/UK workbooks and upload native Google Sheets under the standing Oxford auto-publish rule only after all source-package QA gates pass.",
          "Run Oxford Google Sheet readback, update contract/docs and register the final gate report after upload.",
        ]
      : [

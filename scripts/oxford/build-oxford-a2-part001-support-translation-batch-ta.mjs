...
const SCRIPT_PATH = 'scripts/oxford/build-oxford-a2-part001-support-translation-batch-ta.mjs';
...
const DEFAULT_TRANSLATIONS = path.join(
  PROJECT_ROOT,
  'config',
  `${RELEASE_ID}_support_translation_batch_ta_v1.tsv`,
);
const DEFAULT_OUTPUT = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_ta_v1.jsonl`,
);
const DEFAULT_SUMMARY = path.join(
  PROJECT_ROOT,
  'outputs/oxford-vocabulary/support-translations',
  `${RELEASE_ID}_support_translation_batch_ta_v1_summary.md`,
);
...
function readTranslations(filePath) {
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    assertTamilText(display, `${headword} TA`);
    assertTamilText(example, `${headword} example_TA`);
    if (!SENTENCE_END_RE.test(example)) {
      throw new Error(`${filePath}:${lineIndex + 2}: Tamil example must end with sentence punctuation`);
    }
    if (map.has(headword)) {
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: 'codex_oxford_a2_part001_support_translation_batch_ta_v1',
...
function main() {
  ...
  if (translations.size !== sourceRows.length) {
    throw new Error(`Expected ${sourceRows.length} TA translations, found ${translations.size}`);
  }
  ...
  const outputRows = sourceRows.map((row, index) => {
    ...
    if (!translation) {
      throw new Error(`Missing TA translation for ${sourceHeadword}`);
    }
  ...
  if (extra.length > 0) {
    throw new Error(`Unexpected TA translations: ${extra.join(', ')}`);
  }
  ...
  writeFileSync(
    ...
    [
      ...
      `- Languages: ${LANGUAGE}`,
      '- Article display: not applicable for Tamil',

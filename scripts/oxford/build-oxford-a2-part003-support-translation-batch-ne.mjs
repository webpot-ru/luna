...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-ne.mjs";
const DEFAULT_TRANSLATIONS = "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_ne_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tNE\texample_NE") {
    throw new Error(`Unexpected NE translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    assertNepaliText(display, `${sourceHeadword} NE`);
    assertNepaliText(example, `${sourceHeadword} example_NE`);
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_ne_v1",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: not included for Nepali",

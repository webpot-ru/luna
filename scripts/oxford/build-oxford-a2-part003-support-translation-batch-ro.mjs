...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-ro.mjs";
const DEFAULT_TRANSLATIONS =
  "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_ro_article_display_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tRO\texample_RO") {
    throw new Error(`Unexpected RO translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    assertRomanianText(display, `${sourceHeadword} RO`);
    assertRomanianText(example, `${sourceHeadword} example_RO`);
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_ro_article_display_v1",

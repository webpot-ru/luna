...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-is.mjs";
const DEFAULT_TRANSLATIONS = "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_is_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tIS\texample_IS") {
    throw new Error(`Unexpected IS translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    assertIcelandicText(display, `${sourceHeadword} IS`);
    assertIcelandicText(example, `${sourceHeadword} example_IS`);
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_is_v1",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: not included for Icelandic",

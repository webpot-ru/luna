...
const SCRIPT_PATH = "scripts/oxford/build-oxford-a2-part003-support-translation-batch-da.mjs";
const DEFAULT_TRANSLATIONS =
  "config/oxford_3000_core_a2_part_003_202_v1_support_translation_batch_da_article_display_v1.tsv";
...
async function parseTranslations(filePath) {
  ...
  if (header !== "source_headword\tDA\texample_DA") {
    throw new Error(`Unexpected DA translation TSV header in ${filePath}`);
  }
  const map = new Map();
  for (const [index, line] of lines.entries()) {
    ...
    assertDanishText(display, `${sourceHeadword} DA`);
    assertDanishText(example, `${sourceHeadword} example_DA`);
...
function buildSupportRow(exampleRow, translation, generatedAt) {
  return {
    ...
    reviewer: "codex_oxford_a2_part003_support_translation_batch_da_article_display_v1",
...
await writeFile(
  ...
  [
    ...
    `- Languages: ${LANGUAGE}`,
    "- Article display: enabled where grammatically useful for Danish nouns",

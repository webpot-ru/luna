...
export function mergePlaylistDiscoveryReports({ reports, routing, expectedRoutes = 4, generatedAt = new Date().toISOString() }) {
  const blockers = [];
  ...
  const expectedSupports = [...routing.supportToChannel.keys()].sort();
...
function main() {
  ...
  const snapshot = mergePlaylistDiscoveryReports({ reports, routing, expectedRoutes: options.expectedRoutes });

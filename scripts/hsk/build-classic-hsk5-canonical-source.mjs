...
const RELEASE_ID = "hsk2_classic_level_5_1300_v1";
const HSK_LEVEL = 5;
const EXPECTED_ROWS = 1300;
...
const mitExclusivePath = path.join(SOURCE_DIR, "hsk2_classic_level_5_upstream.source.json");
const hskAcademyPath = path.join(SOURCE_DIR, "hsk2_classic_level_5_hskacademy_reference.html");
const hewgillCumulativePath = path.join(SOURCE_DIR, "hsk2_classic_level_5_hewgill_cumulative_2500.html");
...
const overridesOutPath = path.resolve("config/hsk-classic-hsk5-card-overrides.json");
const examplesOutPath = path.resolve("config/hsk-classic-hsk5-examples.json");
...
async function main() {
  ...
  if (duplicateRows.length) {
    throw new Error(`Unexpected duplicate HSK5 simplified rows: ${JSON.stringify(duplicateRows)}`);
  }
  ...
  for (const [index, tile] of tiles.entries()) {
    ...
    const hewgill = hewgillCandidates.find((row) => row.hewgill_order > 1200) ?? hewgillCandidates[0] ?? null;
    ...
    baseEntry.hsk_canonical_source = {
      ...
      hsk_key: hskKey,
      derivation: mitEntry
        ? "hskacademy_1300_with_matching_complete_hsk_vocabulary_entry"
        : "hskacademy_1300_cross_source_curated_minimal_entry",
      ...
      support_sources: [
        "hsk-academy-1300",
        ...(hewgill ? ["hewgill-cumulative-2500"] : []),
        ...(mitEntry ? ["complete-hsk-vocabulary-exclusive-old-5"] : []),
      ],
    };
  ...
  const report = {
    ...
    method: "Use HSK Academy HSK5 1300-row reference order for HSK 2.0 classic level 5; enrich with MIT complete-hsk-vocabulary old/5 entries where exact simplified entries exist; use Hewgill cumulative 2500 as cross-source support. Do not treat MIT old/5 as final membership truth because it differs substantially from the 1300-row HSK Academy exclusive HSK5 list.",
    input_paths: {
      mit_exclusive_old_5: path.relative(ROOT, mitExclusivePath),
      hskacademy_reference_html: path.relative(ROOT, hskAcademyPath),
      hewgill_cumulative_2500_html: path.relative(ROOT, hewgillCumulativePath),
      previous_hsk1_to_hsk4_sources: previousSourcePaths.map((sourcePath) => path.relative(ROOT, sourcePath)),
    },
    source_counts: {
      mit_exclusive_old_5_rows: mitEntries.length,
      hskacademy_rows: tiles.length,
      hewgill_cumulative_rows: hewgillRows.length,
      canonical_rows_from_mit_entry: entries.filter((entry) => entry.hsk_canonical_source.support_sources.includes("complete-hsk-vocabulary-exclusive-old-5")).length,
      canonical_rows_cross_source_curated: entries.filter((entry) => !entry.hsk_canonical_source.support_sources.includes("complete-hsk-vocabulary-exclusive-old-5")).length,
      ...
      hewgill_supported_rows: entries.filter((entry) => entry.hsk_canonical_source.support_sources.includes("hewgill-cumulative-2500")).length,
    },
    duplicate_simplified_rows: duplicateRows.map(([simplified, count]) => ({ simplified, count })),
    overlap_with_hsk1_to_hsk4_count: overlapsWithPrevious.length,
    overlap_with_hsk1_to_hsk4: overlapsWithPrevious,
    ...
    notes: [
      "HSK Academy HSK5 has exactly 1300 rows, ids 1201-2500, and does not overlap current LunaCards HSK1-HSK4 source rows.",
      "MIT upstream old/5 has 1298 rows and differs heavily from the HSK Academy exclusive HSK5 reference, so it is candidate evidence only.",
      "Generated HSK5 examples are structural placeholders for the preparation workbook. They must be replaced with real pedagogical Chinese examples before target-language translation starts.",
      "AllSet HSK5 was not used in this local run because the source site returned 403 during fetch; HSK Academy plus Hewgill remain the membership/support sources for this snapshot.",
    ],
  };

#!/usr/bin/env python3
"""Build the HSK 3.0 Level 6 source snapshot from the local official PDF."""
...
def extract_level6_rows() -> list[dict]:
    reader = PdfReader(PDF_PATH)
    # PDF pages 94-128 (0-indexed 93-127) contain official vocabulary rows 3601-5000.
    text = "\n".join((reader.pages[index].extract_text() or "") for index in range(93, 128))
    text = clean_pdf_text(text)
    row_start = re.compile(r"(?<!\d)(3[6-9]\d{2}|4\d{3}|5000)\s+((?:6)(?:（[^）]+）)*)\s+")
        ...
        parsed_rows.append(
            {
                ...
                "source": {
                    ...
                    "extraction_note": (
                        ...
                        "rows 3601-5000 are the HSK 3.0 Level 6 vocabulary table."
                    ),
                },
            }
        )
    ...
    missing = [order for order in range(3601, 5001) if order not in orders]
    if len(parsed_rows) != 1400 or missing:
        raise ValueError(f"Expected 1400 continuous rows 3601-5000, got {len(parsed_rows)}; missing={missing[:20]}")
    return parsed_rows

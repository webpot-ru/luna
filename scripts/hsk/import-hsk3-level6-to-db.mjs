...
const releaseIdArg = process.argv.slice(2).find((arg) => !arg.startsWith("--"));
const RELEASE_CONFIGS = {
  hsk3_level_6_1400_v1: {
    expectedRows: 1400,
    expectedHskLevel: 6,
    expectedManualCells: 33864,
    expectedSampleRows: 530,
    sourcePath: "outputs/hsk/source/hsk3_level_6_1400_v1.source.json",
    workbookGatePath: "outputs/hsk/qa/hsk3_level_6_1400_v1_workbook_gate_20260608.json",
    manualGatePath: "outputs/hsk/qa/hsk3_level_6_1400_v1_manual_target_translation_gate_20260608.json",
    chineseExamplesPath: "outputs/hsk/qa/hsk3_level_6_1400_v1_chinese_examples_build_20260608.json",
    sampleAuditPath: "outputs/hsk/qa/hsk3_level_6_1400_v1_sample_10_per_language_quality_20260608.json",
    nativeStyleAuditPath: "outputs/hsk/qa/hsk3_level_6_1400_v1_native_style_sample_5_per_language_20260608.json",
    reportPath: "outputs/hsk/qa/hsk3_level_6_1400_v1_db_import_20260612.json",
    optionalGatePaths: {},
  },
  hsk3_level_6_1800_v2: {
    expectedRows: 1800,
    expectedHskLevel: 6,
    expectedManualCells: 9231,
    expectedSampleRows: 795,
    sourcePath: "outputs/hsk/source/hsk3_level_6_1800_v2.source.json",
    workbookGatePath: "outputs/hsk/qa/hsk3_level_6_1800_v2_workbook_gate_20260612.json",
    manualGatePath: "outputs/hsk/qa/hsk3_level_6_1800_v2_manual_target_translation_gate_20260612.json",
    chineseExamplesPath: "outputs/hsk/qa/hsk3_level_6_1800_v2_chinese_examples_build_20260612.json",
    sampleAuditPath: "outputs/hsk/qa/hsk3_level_6_1800_v2_sample_15_per_language_quality_20260612.json",
    nativeStyleAuditPath: "outputs/hsk/qa/hsk3_level_6_1800_v2_native_style_sample_5_per_language_20260612.json",
    reportPath: "outputs/hsk/qa/hsk3_level_6_1800_v2_db_import_20260612.json",
    optionalGatePaths: {
...
const RELEASE_ID = releaseIdArg || "hsk3_level_6_1400_v1";
const RELEASE_CONFIG = RELEASE_CONFIGS[RELEASE_ID];
if (!RELEASE_CONFIG) {
  throw new Error(`Unsupported HSK3 Level 6 release id: ${RELEASE_ID}`);
}
const EXPECTED_ROWS = RELEASE_CONFIG.expectedRows;
const EXPECTED_HSK_LEVEL = RELEASE_CONFIG.expectedHskLevel;
const EXPECTED_MANUAL_CELLS = RELEASE_CONFIG.expectedManualCells;
...
const sourcePath = path.join(ROOT, RELEASE_CONFIG.sourcePath);
const languagesPath = path.join(ROOT, "config/language-order.json");
const workbookGatePath = path.join(ROOT, RELEASE_CONFIG.workbookGatePath);
const manualGatePath = path.join(ROOT, RELEASE_CONFIG.manualGatePath);
const chineseExamplesPath = path.join(ROOT, RELEASE_CONFIG.chineseExamplesPath);
const sampleAuditPath = path.join(ROOT, RELEASE_CONFIG.sampleAuditPath);
const nativeStyleAuditPath = path.join(ROOT, RELEASE_CONFIG.nativeStyleAuditPath);
const reportPath = path.join(ROOT, RELEASE_CONFIG.reportPath);
...
function sourceKindFor({ language, row, wordValue, exampleValue }) {
  ...
  if (["classic_reuse_target_ready", "reused_target_ready"].includes(row.translation_status)) return "classic_reuse_target";
...
function validateOptionalGate({ name, gate }) {
  const blockers = [];
  if (gate.status && gate.status !== "ok") blockers.push({ type: "optional_gate_not_ok", gate: name, status: gate.status });
  if (gate.blockers?.length) blockers.push({ type: "optional_gate_has_blockers", gate: name, blockers: gate.blockers.length });
  if (name === "xlsx_readback" && gate.readback_status && gate.readback_status !== "ok") {
    blockers.push({ type: "optional_gate_readback_not_ok", gate: name, readback_status: gate.readback_status });
  }
  return blockers;
}
...
async function main() {
  ...
  const optionalGateEntries = Object.entries(RELEASE_CONFIG.optionalGatePaths ?? {});
  const [
    jsonlText,
    sourceEntries,
    allLanguages,
    ...
    fs.readFile(nativeStyleAuditPath, "utf8").then(JSON.parse),
    ...optionalGateEntries.map(([, gatePath]) => fs.readFile(path.join(ROOT, gatePath), "utf8").then(JSON.parse)),
  ]);
  const optionalGates = Object.fromEntries(optionalGateEntries.map(([name], index) => [name, optionalGateValues[index]]));
  ...
  blockers.push(
    ...
    ...validateRequiredGate({ name: "native_style_sample_audit", gate: nativeStyleAudit, expected: {} }),
    ...Object.entries(optionalGates).flatMap(([name, gate]) => validateOptionalGate({ name, gate }))
  );
  if (manualGate.counts?.manual_word_cells !== EXPECTED_MANUAL_CELLS || manualGate.counts?.manual_example_cells !== EXPECTED_MANUAL_CELLS) {
    blockers.push({
      type: "manual_gate_cell_count_mismatch",
      expected_word_cells: EXPECTED_MANUAL_CELLS,
      actual_word_cells: manualGate.counts?.manual_word_cells,
      expected_example_cells: EXPECTED_MANUAL_CELLS,
      actual_example_cells: manualGate.counts?.manual_example_cells,
    });
  }
  if (sampleAudit.sample_policy?.target_language_sample_rows !== RELEASE_CONFIG.expectedSampleRows) {
    blockers.push({
      type: "sample_audit_scope_mismatch",
      expected_sample_rows: RELEASE_CONFIG.expectedSampleRows,
      actual_sample_rows: sampleAudit.sample_policy?.target_language_sample_rows,
    });
  }
  if (nativeStyleAudit.sample_policy?.target_languages_checked !== 53) {
    blockers.push({
      type: "native_style_audit_scope_mismatch",
      expected_target_languages_checked: 53,
      actual_target_languages_checked: nativeStyleAudit.sample_policy?.target_languages_checked,
    });
  }
  ...
  const report = {
    rule_version: "hsk3-level6-db-import-v1",
    ...
    gates: {
      ...
      native_style_sample_audit: nativeStyleAudit.status,
      ...Object.fromEntries(Object.entries(optionalGates).map(([name, gate]) => [name, gate.status ?? gate.readback_status ?? "unknown"])),
    },
    db,
    blockers,
    warnings,

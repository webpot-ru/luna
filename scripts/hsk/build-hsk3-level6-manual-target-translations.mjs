...
const OUTPUT_PATH = path.join(CONFIG_DIR, "hsk3-level6-manual-target-translations.json");
const REPORT_PATH = path.join(ROOT, `outputs/hsk/qa/${RELEASE_ID}_manual_target_translation_build_20260608.json`);
const batchPattern = /^hsk3-level6-manual-translations-.+\.tsv$/iu;
  ...
  for (const { line_number: lineNumber, row } of parsed) {
    ...
    for (const [header, rawValue] of Object.entries(row)) {
      ...
      output[hskKey][code] = {
        ...
        source: "hsk3_level6_manual_target_v1",

...
const manualTsvPath = path.join(ROOT, "config/hsk3-level3-manual-examples.tsv");
const examplesOut = path.join(ROOT, "config/hsk3-level3-examples.json");
const glossesOut = path.join(ROOT, "config/hsk3-level3-en-glosses.json");
...
const report = {
  ...
  classic_reuse_examples: provenance.filter((row) => row.source.startsWith("classic_hsk")).length,
    ...
    "Rows without row-level Classic reuse must be authored in config/hsk3-level3-manual-examples.tsv.",
  ],
};

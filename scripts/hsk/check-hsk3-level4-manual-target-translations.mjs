...
const CLASSIC_TARGET_TRANSLATIONS_PATH = path.join(ROOT, "config/hsk3-level4-classic-reuse-target-translations.json");
const MANUAL_TARGET_TRANSLATIONS_PATH = path.join(ROOT, "config/hsk3-level4-manual-target-translations.json");
...
function markdownReport(report) {
  ...
  return `${[
    `# HSK3 Level 4 Manual Target Translation Gate`,
    ``,
...
if (sourceRows.length !== 1000) issue(blockers, "source_row_count_mismatch", `Expected 1000 source rows, got ${sourceRows.length}.`);
...
for (const hskKey of manualKeys) {
  ...
  if (!sourceRow) {
    issue(blockers, "manual_key_not_in_source", `Manual key is not in HSK3 Level 4 source: ${hskKey}.`, { hsk_key: hskKey });
    continue;
  }

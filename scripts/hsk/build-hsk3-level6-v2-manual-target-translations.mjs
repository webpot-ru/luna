...
const OUTPUT_PATH = path.join(CONFIG_DIR, "hsk3-level6-v2-manual-target-translations.json");
const REPORT_PATH = path.join(ROOT, `outputs/hsk/qa/${RELEASE_ID}_manual_target_translation_build_20260612.json`);
const batchPattern = /^hsk3-level6-v2-manual-translations-.+\.tsv$/iu;
...
const [sourceRows, reusedTargetTranslations, languages, configNames] = await Promise.all([
...
const pendingKeys = new Set([...sourceByKey.keys()].filter((key) => !reusedKeys.has(key)));
...
for (const batchName of batchNames) {
  ...
  for (const { line_number: lineNumber, row } of parsed) {
    ...
    if (normalized(row.simplified) && normalized(row.simplified) !== sourceRow.simplified) {
      blockers.push({
        code: "simplified_mismatch",
        batch: batchName,
        line: lineNumber,
        hsk_key: hskKey,
        expected: sourceRow.simplified,
        actual: normalized(row.simplified),
      });
      continue;
    }
    output[hskKey] ??= {};
    for (const [header, rawValue] of Object.entries(row)) {
      ...
      output[hskKey][code] = {
        ...
        source: "hsk3_level6_v2_manual_target_v1",
...
const report = {
  ...
  blockers,
  notes: [
    "This builder reads only config/hsk3-level6-v2-manual-translations-*.tsv.",
    "It intentionally ignores legacy hsk3-level6-manual-translations-*.tsv files.",

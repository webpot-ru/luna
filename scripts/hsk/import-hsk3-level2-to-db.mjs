...
const sourcePath = path.join(outputDir, "source/hsk3_level_2_200_v1.source.json");
...
async function main() {
  ...
  if (manualGate.counts?.manual_word_cells !== 4437 || manualGate.counts?.manual_example_cells !== 4437) {
    blockers.push({
      type: "manual_gate_cell_count_mismatch",
      expected_word_cells: 4437,
      actual_word_cells: manualGate.counts?.manual_word_cells,
      expected_example_cells: 4437,
  ...
  const report = {
    rule_version: "hsk3-level2-db-import-v1",

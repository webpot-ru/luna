...
const RELEASE_ID = "hsk3_level_5_1600_v1";
const DATE = "20260606";
const SOURCE_PATH = path.join(ROOT, "outputs/hsk/source/hsk3_level_5_1600_v1.source.json");
const REUSE_MAP_PATH = path.join(ROOT, "outputs/hsk/qa/hsk3_level_5_1600_v1_classic_reuse_map_20260606.json");
const EXAMPLES_PATH = path.join(ROOT, "config/hsk3-level5-examples.json");
const GLOSSES_PATH = path.join(ROOT, "config/hsk3-level5-en-glosses.json");
const MANUAL_TSV_PATH = path.join(ROOT, "config/hsk3-level5-manual-examples.tsv");
const BUILD_REPORT_PATH = path.join(ROOT, "outputs/hsk/qa/hsk3_level_5_1600_v1_chinese_examples_build_20260606.json");
...
const reuseByKey = new Map(reuseRows.map((row) => [row.hsk3_hsk_key ?? `${row.hsk3_order}:${row.hsk3_source_word}`, row]));
const manualRows = parseTsv(manualTsv);
const manualKeys = new Set();
const manualExpectedKeys = new Set(
  reuseRows
    .filter((row) => !row.classic_reuse_allowed)
    .map((row) => row.hsk3_hsk_key ?? `${row.hsk3_order}:${row.hsk3_source_word}`)
);

if (sourceRows.length !== 1600) issue(blockers, "source_row_count", `Expected 1600 source rows, found ${sourceRows.length}.`);
if (Object.keys(examples).length !== 1600) issue(blockers, "example_count", `Expected 1600 examples, found ${Object.keys(examples).length}.`);
if (Object.keys(glosses).length !== 1600) issue(blockers, "gloss_count", `Expected 1600 glosses, found ${Object.keys(glosses).length}.`);
if (reuseRows.length !== 1600) issue(blockers, "reuse_row_count", `Expected 1600 reuse rows, found ${reuseRows.length}.`);
if (manualRows.length !== 584) issue(blockers, "manual_tsv_row_count", `Expected 584 manual TSV rows, found ${manualRows.length}.`);
...
for (const { line_number: lineNumber, row } of manualRows) {
  ...
  if (!manualExpectedKeys.has(key)) {
    issue(blockers, "manual_key_not_expected", `Manual TSV key is not a blocked Classic-reuse row: ${key}.`, lineNumber);
  }
...
for (const sourceRow of sourceRows) {
  ...
  if (sourceRow.hsk_level !== 5) issue(blockers, "wrong_hsk_level", `Wrong hsk_level for ${key}: ${sourceRow.hsk_level}.`, sourceRow.hsk_order);
  ...
  if (reuse?.classic_reuse_allowed && provenance?.source === "hsk3_manual") {
    issue(blockers, "manual_overrides_reuse_allowed", `Manual example overrides reusable Classic row ${key}.`, sourceRow.hsk_order);
  }
  if (!reuse?.classic_reuse_allowed && provenance?.source !== "hsk3_manual") {
    issue(blockers, "blocked_reuse_not_manual", `Blocked Classic-reuse row is not manual: ${key}.`, sourceRow.hsk_order);
  }
}
...
if (buildReport.examples !== 1600 || buildReport.glosses !== 1600) {
...
if (buildReport.manual_examples !== 584 || buildReport.classic_reuse_examples !== 1016) {
  issue(
    blockers,
    "build_report_provenance_count_mismatch",
    `Build report provenance counts mismatch: manual=${buildReport.manual_examples}, classic=${buildReport.classic_reuse_examples}.`
  );
}
...
const report = {
  ...
  notes: [
    "This gate validates the HSK 3.0 Level 5 Chinese source example layer and English pivot gloss/example text.",

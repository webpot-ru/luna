...
const DATE = "20260612";
const requestedSamplePerLanguage = Number(process.argv[2] ?? process.env.HSK_SAMPLE_PER_LANGUAGE ?? 10);
if (!Number.isInteger(requestedSamplePerLanguage) || requestedSamplePerLanguage <= 0) {
  throw new Error(`Expected positive integer sample size, got: ${process.argv[2] ?? process.env.HSK_SAMPLE_PER_LANGUAGE}`);
}
const SAMPLE_PER_LANGUAGE = requestedSamplePerLanguage;
...
const REPORT_JSON = path.join(ROOT, `outputs/hsk/qa/${RELEASE_ID}_sample_${SAMPLE_PER_LANGUAGE}_per_language_quality_${DATE}.json`);
const REPORT_CSV = path.join(ROOT, `outputs/hsk/qa/${RELEASE_ID}_sample_${SAMPLE_PER_LANGUAGE}_per_language_quality_${DATE}.csv`);
const REPORT_MD = path.join(ROOT, `outputs/hsk/qa/${RELEASE_ID}_sample_${SAMPLE_PER_LANGUAGE}_per_language_quality_${DATE}.md`);

const toneMarkRegex = /[āáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜńňǹḿ]/iu;
const toneNumberRegex = /(?:[A-Za-züÜv:]+[1-5]\b|\b[1-5]\s*$)/u;
const hanRegex = /\p{Script=Han}/u;
const latinRegex = /[A-Za-züÜāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜńňǹḿ]/u;
const artifactRegex =
...
const acceptedExactEnglishLoanwordSamples = new Set([
  "HR:3865", // protein is the standard Croatian loanword for 蛋白质.
]);
...
function checkSampleCell(row, language, issues) {
  ...
  const context = { row: order, word, language_code: code };
  ...
  if (normalizedText(target) === normalizedText(row.simplified) && code !== "JA") {
    addIssue(issues, "blocker", "sample_exact_chinese_word_copy", "Target word is an exact copy of the Chinese source.", context);
  }
  ...
  if (!["EN", "EN-GB"].includes(code)) {
    ...
    if (normalizedText(target) === normalizedText(en) && !acceptedExactEnglishLoanwordSamples.has(`${code}:${order}`)) {
...
const report = {
  ...
  sample_policy: {
    ...
    sample_scope: `${SAMPLE_PER_LANGUAGE} filled workbook rows per target language after Level 6 reached complete_53_languages_filled.`,
  },
...
await fs.writeFile(
  REPORT_MD,
  [
    `# ${RELEASE_ID} Sample ${SAMPLE_PER_LANGUAGE} Per Language Quality Audit`,
    "",

...
if (rows.length !== 300) {
  pushIssue(blockers, "source_row_count", `Expected 300 HSK3.0 Level 1 rows, found ${rows.length}.`);
}
...
if (duplicateSimplified.length) {
  pushIssue(blockers, "duplicate_simplified", `Duplicate simplified rows: ${duplicateSimplified.map(([value, count]) => `${value} x${count}`).join(", ")}`);
}
...
for (const [index, row] of rows.entries()) {
  ...
  if (row.hsk_level !== 1) {
    pushIssue(blockers, "wrong_hsk_level", `Expected hsk_level 1, found ${row.hsk_level}.`, expectedOrder);
  }
...
await fs.writeFile(
  ...
  [
    ...
    "",
    "This gate validates only the HSK 3.0 Level 1 source layer and exact Classic overlap report. It does not build a workbook, import Docker/Postgres rows or upload Google Sheets.",
    "",

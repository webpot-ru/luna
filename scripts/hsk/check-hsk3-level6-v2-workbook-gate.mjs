...
const COURSE_METADATA_PATH = path.join(ROOT, "config/hsk3-level6-course-metadata.json");
...
const REUSED_TARGET_REPORT = path.join(ROOT, "outputs/hsk/qa/hsk3_level_6_1800_v2_reused_target_translation_build_20260612.json");
...
const EXPECTED_REUSED_READY_ROWS = 1619;
const EXPECTED_HSK3_ONLY_ROWS = 181;
const EXPECTED_FILLED_NON_ENGLISH_CELLS = 91_800;
...
const [csvText, sourceRows, examples, glosses, courseMetadata, manualTargetTranslations, languages, chineseBuildReport, classicTargetReport] =
  await Promise.all([
    ...
    fs.readFile(REUSED_TARGET_REPORT, "utf8").then(JSON.parse),
  ]);
...
if (rows.length !== 1800) issue(blockers, "row_count", `Expected 1800 workbook rows, found ${rows.length}`);
if (sourceRows.length !== 1800) issue(blockers, "source_row_count", `Expected 1800 source rows, found ${sourceRows.length}`);
...
if (classicTargetReport.blockers?.length) {
  issue(blockers, "reused_target_report_blocked", `Reused target build report has ${classicTargetReport.blockers.length} blockers`);
}
...
const completeReusedRows = statusCounts.reused_target_ready ?? 0;
...
if (completeReusedRows !== EXPECTED_REUSED_READY_ROWS) {
  issue(blockers, "reused_target_ready_count", `Expected ${EXPECTED_REUSED_READY_ROWS} reused-target-ready rows, found ${completeReusedRows}`);
}
...
for (const row of rows) {
  ...
  for (const code of nonEnglishTargetCodes) {
    ...
    if (row.translation_status === "reused_target_ready") {
      if (!word) issue(blockers, "reused_target_word_blank", `Reused-target-ready row ${row.hsk_key} missing ${code}`, Number(row.hsk_order));
      if (!example) issue(blockers, "reused_target_example_blank", `Reused-target-ready row ${row.hsk_key} missing example_${code}`, Number(row.hsk_order));
    }
...
for (const code of targetCodes) {
  ...
  if (metadata.title !== "HSK 6.") issue(blockers, "course_metadata_bad_title", `Bad title for ${code}: ${metadata.title}`);
  if (metadata.category !== "HSK 6") issue(blockers, "course_metadata_bad_category", `Bad category for ${code}: ${metadata.category}`);
  ...
  if (!metadata.description.includes("HSK 6") || !metadata.description.includes("HSK 3.0")) {
    issue(blockers, "course_metadata_bad_description", `Description must mention HSK 6 and HSK 3.0 for ${code}: ${metadata.description}`);
  }
}
...
const report = {
  ...
  checked_files: {
    ...
    reused_target_report: path.relative(ROOT, REUSED_TARGET_REPORT),
  },
};

...
const DATE = "20260604";
const OVERLAP_PATH = path.join(ROOT, "outputs/hsk/qa/hsk3_level_3_500_v1_classic_overlap_20260604.json");
...
const tonePolicyWords = new Map([
  ["爱人", "HSK 3.0 uses compact neutral-style `àiren`; Classic may store spaced neutral-style pinyin."],
  ["别人", "HSK 3.0 uses compact neutral-style `biéren`; Classic may store spaced neutral-style pinyin."],
  ["地方", "HSK 3.0 uses neutral-style `dìfang`; Classic may store spaced neutral-style pinyin."],
  ["经过", "HSK 3.0 uses compact neutral-style `jīngguò`; Classic may store spaced pinyin."],
  ["心里", "HSK 3.0 uses neutral-style `xīnlǐ`; Classic may store a spacing/style variant."],
]);

const blockedByOrder = new Map([
  [
    583,
    "HSK3 `得 dé` is the verb reading. Classic exact-word matches are the complement particle `de` and modal `děi`, so Classic reuse is unsafe.",
  ],
  [
    589,
    "HSK3 `地 dì` is the noun/place reading. Classic exact-word match is the adverbial particle `de`, so Classic reuse is unsafe.",
  ],
]);
...
function classify(row) {
  ...
  if (blockedByOrder.has(row.hsk3_order)) {
    return {
      classic_reuse_class: "reading_difference_review",
      classic_reuse_allowed: false,
      classic_reuse_notes: blockedByOrder.get(row.hsk3_order),
    };
  }

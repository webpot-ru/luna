...
const spec = RELEASES[RELEASE_ID];
if (!spec) {
  console.error(`Usage: node scripts/hsk/check-hsk3-source-gate.mjs <release_id>. Valid: ${Object.keys(RELEASES).join(", ")}`);
  process.exit(2);
}

const SOURCE_PATH = path.join(ROOT, `outputs/hsk/source/${RELEASE_ID}.source.json`);
...
if (rows.length !== spec.expectedRows) {
  pushIssue(blockers, "source_row_count", `Expected ${spec.expectedRows} HSK3.0 source rows, found ${rows.length}.`);
}
...
if (duplicateSimplified.length) {
  ...
  if (unsafeDuplicates.length) {
    pushIssue(
      ...
      `Duplicate simplified rows are not safely source_word/pinyin/pos-disambiguated: ${unsafeDuplicates
    ...
    pushIssue(
      ...
      `Official source contains disambiguated duplicate simplified rows: ${duplicateSimplified
        .map(([value, count]) => `${value} x${count}`)
        .join(", ")}`
    );
  }
...
for (const [index, row] of rows.entries()) {
  const expectedOrder = spec.rowMin + index;
  ...
  if (row.hsk_level !== spec.hskLevel) {
    pushIssue(blockers, "wrong_hsk_level", `Expected hsk_level ${spec.hskLevel}, found ${row.hsk_level}.`, expectedOrder);
  }
  ...
  if (row.hsk_order < spec.rowMin || row.hsk_order > spec.rowMax) {
    pushIssue(blockers, "hsk_order_out_of_range", `hsk_order ${row.hsk_order} outside ${spec.rowMin}-${spec.rowMax}.`, expectedOrder);
  }
  for (const field of ["source_word", "simplified", "pinyin"]) {
    if (!row[field]) pushIssue(blockers, "missing_core_field", `Missing ${field}.`, expectedOrder);
  }
  ...
  if (row.source?.local_pdf_path !== "outputs/hsk/source/hsk3_official_syllabus_vocab_chars_grammar_202511_202607.pdf") {
    pushIssue(blockers, "wrong_local_pdf_path", `Unexpected local PDF path: ${row.source?.local_pdf_path}.`, expectedOrder);
  }
}
...
const summary = {
  ...
  expected_rows: spec.expectedRows,
  order_range: `${spec.rowMin}-${spec.rowMax}`,
...
await fs.writeFile(
  ...
  [
    ...
    `Order range: ${summary.order_range}`,
    ...
    "",
    "This gate validates only the HSK 3.0 source layer and exact Classic overlap report. It does not build a workbook, import Docker/Postgres rows or upload Google Sheets.",
    "",

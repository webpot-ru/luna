...
const REUSE_MAP_PATH = path.join(ROOT, "outputs/hsk/qa/hsk3_level_4_1000_v1_classic_reuse_map_20260605.json");
const EXAMPLES_PATH = path.join(ROOT, "config/hsk3-level4-examples.json");
const GLOSSES_PATH = path.join(ROOT, "config/hsk3-level4-en-glosses.json");
const MANUAL_TSV_PATH = path.join(ROOT, "config/hsk3-level4-manual-examples.tsv");
const BUILD_REPORT_PATH = path.join(ROOT, "outputs/hsk/qa/hsk3_level_4_1000_v1_chinese_examples_build_20260605.json");
...
if (sourceRows.length !== 1000) issue(blockers, "source_row_count", `Expected 1000 source rows, found ${sourceRows.length}.`);
if (Object.keys(examples).length !== 1000) issue(blockers, "example_count", `Expected 1000 examples, found ${Object.keys(examples).length}.`);
if (Object.keys(glosses).length !== 1000) issue(blockers, "gloss_count", `Expected 1000 glosses, found ${Object.keys(glosses).length}.`);
if (reuseRows.length !== 1000) issue(blockers, "reuse_row_count", `Expected 1000 reuse rows, found ${reuseRows.length}.`);
if (manualRows.length !== 389) issue(blockers, "manual_tsv_row_count", `Expected 389 manual TSV rows, found ${manualRows.length}.`);
...
for (const sourceRow of sourceRows) {
  ...
  if (sourceRow.hsk_level !== 4) issue(blockers, "wrong_hsk_level", `Wrong hsk_level for ${key}: ${sourceRow.hsk_level}.`, sourceRow.hsk_order);
...
if (buildReport.examples !== 1000 || buildReport.glosses !== 1000) {
...
if (buildReport.manual_examples !== 389 || buildReport.classic_reuse_examples !== 611) {
  issue(
    blockers,
...
const report = {
  ...
  notes: [
    "This gate validates the HSK 3.0 Level 4 Chinese source example layer and English pivot gloss/example text.",

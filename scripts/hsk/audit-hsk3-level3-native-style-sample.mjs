...
const forcedReviewKeys = new Set(["716:斤"]);
...
function checkNativeStyleCell({ row, language, issues }) {
  ...
  if (row.hsk_key === "716:斤" && code === "RU" && /два\s+цзин/iu.test(exampleTarget)) {
    addIssue(
      issues,
      "blocker",
      "ru_jin_measure_calque",
      "Russian learner-facing example sounds calqued; use a natural approximate metric wording for 两斤.",
      { ...context, exampleTarget, suggested: "Я купил около килограмма яблок." }
    );
  }
}
...
const report = {
  ...
  notes: [
    ...
    "The first confirmed HSK3 Level 3 native-style pattern is Russian 两斤 rendered as 'два цзиня' in an ordinary learner-facing sentence.",
  ],
};

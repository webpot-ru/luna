...
const COURSE_METADATA_PATH = path.join(ROOT, "config/hsk3-level2-course-metadata.json");
...
const CHINESE_BUILD_REPORT = path.join(ROOT, "outputs/hsk/qa/hsk3_level_2_200_v1_chinese_examples_build_20260604.json");
const CLASSIC_TARGET_REPORT = path.join(ROOT, "outputs/hsk/qa/hsk3_level_2_200_v1_classic_target_translation_build_20260604.json");
...
if (rows.length !== 200) issue(blockers, "row_count", `Expected 200 workbook rows, found ${rows.length}`);
if (sourceRows.length !== 200) issue(blockers, "source_row_count", `Expected 200 source rows, found ${sourceRows.length}`);
...
if (completeClassicRows !== 113) issue(blockers, "classic_reuse_ready_count", `Expected 113 Classic-ready rows, found ${completeClassicRows}`);
if (completeManualRows + pendingRows !== 87) {
  issue(blockers, "manual_or_pending_row_count", `Expected 87 HSK3-only manual/pending rows, found manual=${completeManualRows}, pending=${pendingRows}`);
}
...
for (const code of targetCodes) {
  ...
  if (metadata.title !== "HSK 2.") issue(blockers, "course_metadata_bad_title", `Bad title for ${code}: ${metadata.title}`);
  if (metadata.category !== "HSK 2") issue(blockers, "course_metadata_bad_category", `Bad category for ${code}: ${metadata.category}`);
  ...
  if (!metadata.description.includes("HSK 2") || !metadata.description.includes("HSK 3.0")) {
    issue(blockers, "course_metadata_bad_description", `Description must mention HSK 2 and HSK 3.0 for ${code}: ${metadata.description}`);
  }
}

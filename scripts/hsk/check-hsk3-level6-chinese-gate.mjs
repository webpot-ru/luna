...
const DATE = "20260608";
const SOURCE_PATH = path.join(ROOT, "outputs/hsk/source/hsk3_level_6_1400_v1.source.json");
const REUSE_MAP_PATH = path.join(ROOT, "outputs/hsk/qa/hsk3_level_6_1400_v1_classic_reuse_map_20260608.json");
const EXAMPLES_PATH = path.join(ROOT, "config/hsk3-level6-examples.json");
const GLOSSES_PATH = path.join(ROOT, "config/hsk3-level6-en-glosses.json");
const MANUAL_TSV_PATH = path.join(ROOT, "config/hsk3-level6-manual-examples.tsv");
const BUILD_REPORT_PATH = path.join(ROOT, "outputs/hsk/qa/hsk3_level_6_1400_v1_chinese_examples_build_20260608.json");
...
if (sourceRows.length !== 1400) issue(blockers, "source_row_count", `Expected 1400 source rows, found ${sourceRows.length}.`);
if (Object.keys(examples).length !== 1400) issue(blockers, "example_count", `Expected 1400 examples, found ${Object.keys(examples).length}.`);
if (Object.keys(glosses).length !== 1400) issue(blockers, "gloss_count", `Expected 1400 glosses, found ${Object.keys(glosses).length}.`);
if (reuseRows.length !== 1400) issue(blockers, "reuse_row_count", `Expected 1400 reuse rows, found ${reuseRows.length}.`);
if (manualRows.length !== 664) issue(blockers, "manual_tsv_row_count", `Expected 664 manual TSV rows, found ${manualRows.length}.`);
...
if (buildReport.examples !== 1400 || buildReport.glosses !== 1400) {
...
if (buildReport.manual_examples !== 664 || buildReport.classic_reuse_examples !== 736) {
  issue(
    blockers,
...
const report = {
  ...
  notes: [
    "This gate validates the HSK 3.0 Level 6 Chinese source example layer and English pivot gloss/example text.",

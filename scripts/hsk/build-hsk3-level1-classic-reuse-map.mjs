...
const DATE = "20260604";
const OVERLAP_PATH = path.join(ROOT, "outputs/hsk/qa/hsk3_level_1_300_v1_classic_overlap_20260603.json");
...
const tonePolicyWords = new Map([
  ["多少", "HSK 3.0 uses neutral-tone-style `duōshao`; Classic stores citation-tone `duō shǎo`."],
  ["喂", "HSK 3.0 uses `wèi`; Classic HSK1 stores `wéi` for phone-greeting style. Treat as pronunciation policy/variant until example sense is fixed."],
  ["学生", "HSK 3.0 uses `xuéshēng`; Classic stores neutral-style `xué sheng`."],
]);
...
function classify(row) {
  ...
  if (String(hsk3Pinyin).includes("/")) {
    const variants = String(hsk3Pinyin).split("/");
    const variantMatch = matches.find((match) =>
      variants.some((variant) => stripSpaces(variant) === stripSpaces(match.classic_pinyin))
    );
    if (variantMatch) {
      return {
        classic_reuse_class: "variant_ok",
        classic_reuse_allowed: true,
        classic_reuse_notes: "HSK 3.0 lists multiple pinyin variants and one matches the Classic pinyin.",
        selected_classic_match: variantMatch,
      };
    }
  }
...
await fs.writeFile(
  ...
  [
    ...
    ...rows
      ...
      .map((row) => `- ${row.hsk3_order} ${row.hsk3_simplified}: ${row.classic_reuse_class}; ${row.classic_reuse_notes}`),
    "",
  ].join("\n")
);

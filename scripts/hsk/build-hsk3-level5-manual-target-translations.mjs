...
const OUTPUT_PATH = path.join(CONFIG_DIR, "hsk3-level5-manual-target-translations.json");
const REPORT_PATH = path.join(ROOT, `outputs/hsk/qa/${RELEASE_ID}_manual_target_translation_build_20260606.json`);
const batchPattern = /^hsk3-level5-manual-translations-.+\.tsv$/iu;
  ...
  for (const { line_number: lineNumber, row } of parsed) {
    ...
    for (const [header, rawValue] of Object.entries(row)) {
      ...
      output[hskKey][code] = {
        ...
        source: "hsk3_level5_manual_target_v1",

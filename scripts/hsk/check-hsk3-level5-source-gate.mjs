...
if (rows.length !== 1600) {
  pushIssue(blockers, "source_row_count", `Expected 1600 HSK3.0 Level 5 rows, found ${rows.length}.`);
}
...
if (duplicateSimplified.length) {
  ...
  if (unsafeDuplicates.length) {
    ...
    pushIssue(
      ...
      `Official Level 5 contains disambiguated duplicate simplified rows: ${duplicateSimplified
        .map(([value, count]) => `${value} x${count}`)
        .join(", ")}`
    );
  }
...
for (const [index, row] of rows.entries()) {
  ...
  if (row.hsk_level !== 5) {
    pushIssue(blockers, "wrong_hsk_level", `Expected hsk_level 5, found ${row.hsk_level}.`, expectedOrder);
  }
...
await fs.writeFile(
  ...
  [
    ...
    "",
    "This gate validates only the HSK 3.0 Level 5 source layer and exact Classic overlap report. It does not build a workbook, import Docker/Postgres rows or upload Google Sheets.",
    "",

...
const migrationPath = path.join(ROOT, "db/migrations/030_hsk3_release_items.sql");
const jsonlPath = path.join(outputDir, `${RELEASE_ID}.jsonl`);
const sourcePath = path.join(outputDir, "source/hsk3_level_1_300_v1.source.json");
...
const chineseGatePath = path.join(qaDir, `${RELEASE_ID}_chinese_gate_20260604.json`);
...
function validateRows({ rows, sourceEntries, languages }) {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    if (Number(row.hsk_order) !== order) blockers.push({ type: "hsk_order_mismatch", expected: order, actual: row.hsk_order });
    if (row.hsk_version !== "HSK 3.0") blockers.push({ type: "hsk_version_mismatch", order, actual: row.hsk_version });
    if (Number(row.hsk_level) !== 1) blockers.push({ type: "hsk_level_mismatch", order, actual: row.hsk_level });
    ...
    if (sourceEntry?.simplified !== row.simplified) {
      blockers.push({
...
async function main() {
  ...
  const [jsonlText, sourceEntries, allLanguages, workbookGate, chineseGate, sampleAudit] = await Promise.all([
    ...
    fs.readFile(chineseGatePath, "utf8").then(JSON.parse),
  ...
  for (const [name, gate] of [
    ["workbook_gate", workbookGate],
    ["chinese_gate", chineseGate],
    ["sample_quality_audit", sampleAudit],
  ]) {
  ...
  const report = {
    rule_version: "hsk3-db-import-v1",

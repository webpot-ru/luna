...
const CHINESE_BUILD_REPORT = path.join(ROOT, "outputs/hsk/qa/hsk3_level_1_300_v1_chinese_examples_build_20260604.json");
const TRANSLATIONS_OUT = path.join(ROOT, "config/hsk3-level1-classic-reuse-target-translations.json");
...
async function readClassicLevel(level) {
  ...
  for (const file of files) {
    ...
    for (const row of rows) {
      ...
      Object.assign(byWord.get(word), row);
    }
  }
  return { files, byWord };
}
...
const sourceOrderByWord = new Map(sourceRows.map((row) => [row.simplified, row.hsk_order]));
const translations = {};
const blockers = [];
const provenance = [];
...
for (const item of chineseReport.provenance ?? []) {
  ...
  const match = /^classic_hsk(\d+):(.+)$/u.exec(item.source);
  if (!match) {
    blockers.push({ order: item.order, word: item.word, issue: "bad_classic_source_pointer", source: item.source });
    continue;
  }
  const level = Number(match[1]);
  const classicWord = match[2].replace(/^\d+:/u, "");
  const classic = classicByLevel.get(level)?.byWord.get(classicWord);
  if (!classic) {
    blockers.push({ order: item.order, word: item.word, issue: "missing_classic_translation_row", source: item.source });
    continue;
  }
  ...
  for (const code of targetCodes) {
    ...
    if (!translation) blockers.push({ order: item.order, word: item.word, issue: "missing_translation", code, source: item.source });
    if (!example) blockers.push({ order: item.order, word: item.word, issue: "missing_example_translation", code, source: item.source });
  ...
  translations[item.word] = rowTranslations;
  provenance.push({
    order: item.order,
    word: item.word,
    source: item.source,
...
for (const word of Object.keys(translations)) {
  if (!sourceOrderByWord.has(word)) blockers.push({ word, issue: "translation_word_not_in_hsk3_source" });
}
...
const report = {
  ...
  notes: [
    ...
    "EN/EN-GB pivot cells are handled by config/hsk3-level1-en-glosses.json and config/hsk3-level1-examples.json.",
...
await fs.writeFile(
  ...
  [
    ...
    "",
    ...blockers.map((item) => `- BLOCKER ${item.issue}${item.order ? ` row ${item.order}` : ""}${item.code ? ` ${item.code}` : ""}: ${item.word ?? ""}`),
    "",
  ].join("\n")
);

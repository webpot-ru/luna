...
const examplesPath = path.join(ROOT, "config/hsk3-level2-examples.json");
const glossesPath = path.join(ROOT, "config/hsk3-level2-en-glosses.json");
const classicTargetTranslationsPath = path.join(ROOT, "config/hsk3-level2-classic-reuse-target-translations.json");
const manualTargetTranslationsPath = path.join(ROOT, "config/hsk3-level2-manual-target-translations.json");
const courseMetadataPath = path.join(ROOT, "config/hsk3-level2-course-metadata.json");
...
function validateSource(rows) {
  if (!Array.isArray(rows)) throw new Error("HSK3 Level 2 source snapshot must be a JSON array");
...
function validateChineseLayer(sourceRows, examples, glosses) {
  ...
  for (const key of Object.keys(examples)) {
    if (!sourceKeys.has(key)) throw new Error(`Example key is not in HSK3 Level 2 source: ${key}`);
  }
  for (const key of Object.keys(glosses)) {
    if (!sourceKeys.has(key)) throw new Error(`Gloss key is not in HSK3 Level 2 source: ${key}`);
  }
}
...
function validateCourseMetadata(languages, courseMetadata) {
  ...
  if (missing.length) throw new Error(`Missing HSK3 Level 2 Course Metadata for: ${missing.join(", ")}`);
  const oldSourceRefs = codes.filter((code) => /HSK 2\.0|2010-2021/u.test(courseMetadata[code].description));
  if (oldSourceRefs.length) throw new Error(`HSK3 Level 2 metadata has old Classic source refs for: ${oldSourceRefs.join(", ")}`);
  const repeatedEnglish = codes.filter(
    (code) => code !== "EN" && code !== "EN-GB" && courseMetadata[code]?.description === courseMetadata.EN?.description
  );
  if (repeatedEnglish.length) throw new Error(`HSK3 Level 2 Course Metadata descriptions repeat EN text for: ${repeatedEnglish.join(", ")}`);
}
...
function addReadme(workbook, contract, languages, reuseMap) {
  ...
  setValues(sheet, 1, 0, [
    ...
    ["Official source order", "301-500"],
    ["Internal language columns", languages.length],
    ["Buyer-facing language columns", "54 including ZH"],
    ["Main sheet", MAIN_SHEET],
    ["Status", "source/chinese/classic-reuse preparation; manual target languages pending for HSK3-only rows"],
...
function addSources(workbook, contract) {
  ...
  setValues(sheet, 1, 0, [
    ...
    [
      ...
      "Chinese base list for HSK 3.0 Level 2",
    ...
    [
      "hsk3-level2-source-snapshot",
      "Normalized source rows",
      path.relative(ROOT, sourcePath),
      "internal LunaCards normalization of official source",
      "Preserves official HSK 3.0 pinyin and row order 301-500.",
    ],
    [
      "hsk3-level2-classic-reuse-map",
    ...
    [
      "hsk3-level2-classic-reuse-target-translations",

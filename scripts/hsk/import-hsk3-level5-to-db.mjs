...
const EXPECTED_HSK_LEVEL = 5;
const EXPECTED_MANUAL_CELLS = 29784;
...
const sourcePath = path.join(outputDir, "source/hsk3_level_5_1600_v1.source.json");
const languagesPath = path.join(ROOT, "config/language-order.json");
const workbookGatePath = path.join(qaDir, `${RELEASE_ID}_workbook_gate_20260608.json`);
const manualGatePath = path.join(qaDir, `${RELEASE_ID}_manual_target_translation_gate_20260606.json`);
const chineseExamplesPath = path.join(qaDir, `${RELEASE_ID}_chinese_examples_build_20260606.json`);
const sampleAuditPath = path.join(qaDir, `${RELEASE_ID}_sample_10_per_language_quality_20260608.json`);
const nativeStyleAuditPath = path.join(qaDir, `${RELEASE_ID}_native_style_sample_5_per_language_20260608.json`);
const reportPath = path.join(qaDir, `${RELEASE_ID}_db_import_20260608.json`);
...
async function main() {
  ...
  const [jsonlText, sourceEntries, allLanguages, workbookGate, manualGate, chineseExamples, sampleAudit, nativeStyleAudit] = await Promise.all([
  ...
  const report = {
    rule_version: "hsk3-level5-db-import-v1",

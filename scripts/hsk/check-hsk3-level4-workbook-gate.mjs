...
const DATE = "20260606";
const CSV_PATH = path.join(ROOT, "outputs/hsk/hsk3_level_4_1000_v1.csv");
...
const COURSE_METADATA_PATH = path.join(ROOT, "config/hsk3-level4-course-metadata.json");
...
const CLASSIC_TARGET_REPORT = path.join(ROOT, "outputs/hsk/qa/hsk3_level_4_1000_v1_classic_target_translation_build_20260605.json");
...
const EXPECTED_CLASSIC_READY_ROWS = 611;
const EXPECTED_HSK3_ONLY_ROWS = 389;
const EXPECTED_FILLED_NON_ENGLISH_CELLS = 51_000;
...
if (rows.length !== 1000) issue(blockers, "row_count", `Expected 1000 workbook rows, found ${rows.length}`);
...
for (const code of targetCodes) {
  ...
  if (metadata.title !== "HSK 4.") issue(blockers, "course_metadata_bad_title", `Bad title for ${code}: ${metadata.title}`);
  if (metadata.category !== "HSK 4") issue(blockers, "course_metadata_bad_category", `Bad category for ${code}: ${metadata.category}`);
  ...
  if (!metadata.description.includes("HSK 4") || !metadata.description.includes("HSK 3.0")) {
    issue(blockers, "course_metadata_bad_description", `Description must mention HSK 4 and HSK 3.0 for ${code}: ${metadata.description}`);
  }
}

...
const sourcePath = path.join(outputDir, "source/hsk3_level_4_1000_v1.source.json");
const languagesPath = path.join(ROOT, "config/language-order.json");
const workbookGatePath = path.join(qaDir, `${RELEASE_ID}_workbook_gate_20260606.json`);
const manualGatePath = path.join(qaDir, `${RELEASE_ID}_manual_target_translation_gate_20260605.json`);
const chineseExamplesPath = path.join(qaDir, `${RELEASE_ID}_chinese_examples_build_20260605.json`);
const sampleAuditPath = path.join(qaDir, `${RELEASE_ID}_sample_10_per_language_quality_20260606.json`);
const reportPath = path.join(qaDir, `${RELEASE_ID}_db_import_20260606.json`);
...
async function main() {
  ...
  if (manualGate.counts?.manual_word_cells !== 19839 || manualGate.counts?.manual_example_cells !== 19839) {
    blockers.push({
      type: "manual_gate_cell_count_mismatch",
      expected_word_cells: 19839,
      actual_word_cells: manualGate.counts?.manual_word_cells,
      expected_example_cells: 19839,
  ...
  const report = {
    rule_version: "hsk3-level4-db-import-v1",

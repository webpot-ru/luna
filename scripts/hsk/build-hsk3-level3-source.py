#!/usr/bin/env python3
"""Build the HSK 3.0 Level 3 source snapshot from the local official PDF."""
...
def extract_level3_rows() -> list[dict]:
    reader = PdfReader(PDF_PATH)
    # PDF pages 16-28 (0-indexed 15-27) contain official vocabulary rows 501-1000.
    text = "\n".join((reader.pages[index].extract_text() or "") for index in range(15, 28))
    text = clean_pdf_text(text)
    row_start = re.compile(r"(?<!\d)([5-9]\d{2}|1000)\s+((?:3)(?:（[^）]+）)*)\s+")
        ...
        parsed_rows.append(
            {
                ...
                "source": {
                    ...
                    "extraction_note": (
                        ...
                        "rows 501-1000 are the HSK 3.0 Level 3 vocabulary table."
                    ),
                },
            }
        )
    ...
    missing = [order for order in range(501, 1001) if order not in orders]
    if len(parsed_rows) != 500 or missing:
        raise ValueError(f"Expected 500 continuous rows 501-1000, got {len(parsed_rows)}; missing={missing[:20]}")
    return parsed_rows

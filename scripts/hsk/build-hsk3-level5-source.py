#!/usr/bin/env python3
"""Build the HSK 3.0 Level 5 source snapshot from the local official PDF."""
...
def extract_level5_rows() -> list[dict]:
    reader = PdfReader(PDF_PATH)
    # PDF pages 54-93 (0-indexed 53-92) contain official vocabulary rows 2001-3600.
    text = "\n".join((reader.pages[index].extract_text() or "") for index in range(53, 93))
    text = clean_pdf_text(text)
    row_start = re.compile(r"(?<!\d)(2\d{3}|3[0-5]\d{2}|3600)\s+((?:5)(?:（[^）]+）)*)\s+")
        ...
        parsed_rows.append(
            {
                ...
                "source": {
                    ...
                    "extraction_note": (
                        ...
                        "rows 2001-3600 are the HSK 3.0 Level 5 vocabulary table."
                    ),
                },
            }
        )
    ...
    missing = [order for order in range(2001, 3601) if order not in orders]
    if len(parsed_rows) != 1600 or missing:
        raise ValueError(f"Expected 1600 continuous rows 2001-3600, got {len(parsed_rows)}; missing={missing[:20]}")
    return parsed_rows

...
const CLASSIC_TARGET_TRANSLATIONS_PATH = path.join(ROOT, "config/hsk3-level5-classic-reuse-target-translations.json");
const MANUAL_TARGET_TRANSLATIONS_PATH = path.join(ROOT, "config/hsk3-level5-manual-target-translations.json");
...
function markdownReport(report) {
  ...
  return `${[
    `# HSK3 Level 5 Manual Target Translation Gate`,
    ``,
...
const pendingKeys = [...sourceByKey.keys()].filter((key) => !classicKeys.has(key));
...
if (sourceRows.length !== 1600) issue(blockers, "source_row_count_mismatch", `Expected 1600 source rows, got ${sourceRows.length}.`);
...
for (const hskKey of manualKeys) {
  ...
  if (!sourceRow) {
    issue(blockers, "manual_key_not_in_source", `Manual key is not in HSK3 Level 5 source: ${hskKey}.`, { hsk_key: hskKey });
    continue;
  }
  if (classicKeys.has(hskKey)) {
    issue(blockers, "manual_key_is_classic_reuse", `Manual key already has Classic reuse target translations: ${hskKey}.`, { hsk_key: hskKey });
  }
  if (!pendingKeySet.has(hskKey)) {
    issue(blockers, "manual_key_not_pending_hsk3_only", `Manual key is not one of the pending HSK3-only rows: ${hskKey}.`, { hsk_key: hskKey });
  }
  ...
  for (const code of targetCodes) {
    ...
    if (placeholderRegex.test(translation) || placeholderRegex.test(exampleTranslation) || exactPlaceholderValues.has(translation.toLowerCase()) || exactPlaceholderValues.has(exampleTranslation.toLowerCase())) {
    ...
    if (normalizedText(translation) === normalizedText(sourceRow.simplified) || normalizedText(exampleTranslation) === normalizedText(sourceExample)) {
      issue(blockers, "manual_exact_chinese_copy", `${hskKey} ${code} is an exact Chinese source/example copy.`, { hsk_key: hskKey, language_code: code });
    }
...
if (missingManualKeys.length && requireComplete) {
  issue(blockers, "manual_layer_incomplete", `${missingManualKeys.length} pending HSK3-only rows are still missing manual target translations.`);
} else if (missingManualKeys.length) {
  issue(warnings, "manual_layer_incomplete", `${missingManualKeys.length} pending HSK3-only rows are still missing manual target translations.`);
}
...
const report = {
  ...
  counts: {
    ...
    classic_reuse_rows: classicKeys.size,
    pending_hsk3_only_rows: pendingKeys.length,

...
const MANUAL_TARGET_TRANSLATIONS_PATH = path.join(ROOT, "config/hsk3-level2-manual-target-translations.json");
const EXAMPLES_PATH = path.join(ROOT, "config/hsk3-level2-examples.json");
const GLOSSES_PATH = path.join(ROOT, "config/hsk3-level2-en-glosses.json");
...
function markdownReport(report) {
  ...
  return `${[
    `# HSK3 Level 2 Manual Target Translation Gate`,
    ``,
...
if (sourceRows.length !== 200) issue(blockers, "source_row_count_mismatch", `Expected 200 source rows, got ${sourceRows.length}.`);
...
for (const hskKey of manualKeys) {
  ...
  if (!sourceRow) {
    issue(blockers, "manual_key_not_in_source", `Manual key is not in HSK3 Level 2 source: ${hskKey}.`, { hsk_key: hskKey });
    continue;
  }
  ...
  for (const code of targetCodes) {
    ...
    if (code !== "JA" && (hanRegex.test(translation) || hanRegex.test(exampleTranslation))) {
      issue(blockers, "manual_han_in_non_japanese_target", `${hskKey} ${code} contains Han characters outside Japanese.`, { hsk_key: hskKey, language_code: code });
    }
    if (normalizedText(exampleTranslation) === normalizedText(englishExample)) {
      issue(blockers, "manual_exact_english_example_fallback", `${hskKey} ${code} example is an exact English fallback.`, { hsk_key: hskKey, language_code: code });
    }

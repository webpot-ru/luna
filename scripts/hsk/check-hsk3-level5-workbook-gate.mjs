...
const DATE = "20260608";
const CSV_PATH = path.join(ROOT, "outputs/hsk/hsk3_level_5_1600_v1.csv");
...
const COURSE_METADATA_PATH = path.join(ROOT, "config/hsk3-level5-course-metadata.json");
...
const CHINESE_BUILD_REPORT = path.join(ROOT, "outputs/hsk/qa/hsk3_level_5_1600_v1_chinese_examples_build_20260606.json");
const CLASSIC_TARGET_REPORT = path.join(ROOT, "outputs/hsk/qa/hsk3_level_5_1600_v1_classic_target_translation_build_20260606.json");
...
const EXPECTED_CLASSIC_READY_ROWS = 1016;
const EXPECTED_HSK3_ONLY_ROWS = 584;
const EXPECTED_FILLED_NON_ENGLISH_CELLS = 81_600;
...
if (rows.length !== 1600) issue(blockers, "row_count", `Expected 1600 workbook rows, found ${rows.length}`);
...
for (const code of targetCodes) {
  ...
  if (metadata.title !== "HSK 5.") issue(blockers, "course_metadata_bad_title", `Bad title for ${code}: ${metadata.title}`);
  if (metadata.category !== "HSK 5") issue(blockers, "course_metadata_bad_category", `Bad category for ${code}: ${metadata.category}`);
  ...
  if (!metadata.description.includes("HSK 5") || !metadata.description.includes("HSK 3.0")) {
    issue(blockers, "course_metadata_bad_description", `Description must mention HSK 5 and HSK 3.0 for ${code}: ${metadata.description}`);
  }
}

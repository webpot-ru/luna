...
const REUSE_MAP_PATH = path.join(ROOT, "outputs/hsk/qa/hsk3_level_3_500_v1_classic_reuse_map_20260604.json");
...
const MANUAL_TSV_PATH = path.join(ROOT, "config/hsk3-level3-manual-examples.tsv");
const BUILD_REPORT_PATH = path.join(ROOT, "outputs/hsk/qa/hsk3_level_3_500_v1_chinese_examples_build_20260604.json");
...
if (Object.keys(examples).length !== 500) issue(blockers, "example_count", `Expected 500 examples, found ${Object.keys(examples).length}.`);
if (Object.keys(glosses).length !== 500) issue(blockers, "gloss_count", `Expected 500 glosses, found ${Object.keys(glosses).length}.`);
if (reuseRows.length !== 500) issue(blockers, "reuse_row_count", `Expected 500 reuse rows, found ${reuseRows.length}.`);
if (manualRows.length !== 202) issue(blockers, "manual_tsv_row_count", `Expected 202 manual TSV rows, found ${manualRows.length}.`);
...
for (const sourceRow of sourceRows) {
  ...
  if (sourceRow.hsk_level !== 3) issue(blockers, "wrong_hsk_level", `Wrong hsk_level for ${key}: ${sourceRow.hsk_level}.`, sourceRow.hsk_order);
...
if (buildReport.examples !== 500 || buildReport.glosses !== 500) {
...
if (buildReport.manual_examples !== 202 || buildReport.classic_reuse_examples !== 298) {
  issue(
    blockers,
...
const report = {
  ...
  notes: [
    "This gate validates the HSK 3.0 Level 3 Chinese source example layer and English pivot gloss/example text.",

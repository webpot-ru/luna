...
const lockedSourcePinyin = new Map([
  ["还", "hái"],
  ["只", "zhī"],
  ["喂", "wèi"],
  ["学生", "xuéshēng"],
...
function compactPinyin(value) {
  return stripTones(value).replace(/[^a-z]/giu, "");
}

function compactPinyinVariants(value) {
  return String(value ?? "")
    .split("/")
    .map((variant) => compactPinyin(variant))
    .filter(Boolean);
}
...
function checkChineseRow(row, issues) {
  ...
  const sourcePinyin = row.pinyin;
  const examplePinyin = row.example_pinyin;
  ...
  if (!toneMarkRegex.test(sourcePinyin) && acceptedNeutralSourcePinyin.get(word) !== sourcePinyin) {
    addIssue(issues, "warning", "source_pinyin_no_tone_mark", "Source pinyin has no tone mark; valid only for neutral/modal syllables.", {
      row: order,
      word,
      pinyin: sourcePinyin,
    });
  }
  if (!toneMarkRegex.test(examplePinyin)) {
  ...
  const locked = lockedSourcePinyin.get(word);
  if (locked && sourcePinyin !== locked) {
    addIssue(issues, "blocker", "locked_source_pinyin_mismatch", `Expected ${locked}, got ${sourcePinyin}`, {
      row: order,
      word,
      pinyin: sourcePinyin,
    });
  ...
  const sourceBases = compactPinyinVariants(sourcePinyin);
  const exampleBase = compactPinyin(examplePinyin);
  if (sourceBases.length && exampleBase && !sourceBases.some((sourceBase) => exampleBase.includes(sourceBase))) {
    addIssue(issues, "warning", "source_pinyin_base_not_visible_in_example", "Source pinyin base is not visible in example pinyin; check tone sandhi or segmentation.", {
      row: order,
      word,
      pinyin: sourcePinyin,
      example_pinyin: examplePinyin,
...
function checkSampleCell(row, language, issues) {
  ...
  if (artifactRegex.test(exampleTarget)) {
    addIssue(issues, "blocker", "sample_example_artifact", "Target example contains placeholder/artifact text.", { ...context, exampleTarget });
  }
...
for (const language of languages) {
  ...
  const selected = chooseSample(eligible, code, SAMPLE_PER_LANGUAGE);
...
const languageSummary = languages.map((language) => {
  ...
  const blockerCount = blockers.filter((issue) => issue.code === code || issue.code === undefined).length;
  return {
    code,
    language: language.language,
    ...
    sampled_blockers: sampleRows
      .filter((row) => row.language_code === code)
      .flatMap((sample) => blockers.filter((issue) => issue.code === code && issue.row === sample.hsk_order)).length,
    blocker_count_note: blockerCount,
  };
});

const sourcePinyinWarnings = warnings.filter((issue) => issue.code === "source_pinyin_no_tone_mark");
...
const report = {
  ...
  sample_policy: {
    ...
    sample_scope:
      "10 filled workbook rows per target language. Non-English pending HSK3.0 manual rows are counted as pending and are not sampled as completed translations.",
  },
  chinese_pinyin_policy: {
    rows_checked: rows.length,
    source_pinyin_rows_checked: rows.length,
    example_pinyin_rows_checked: rows.length,
    locked_source_pinyin_rows_checked: lockedSourcePinyin.size,
    source_pinyin_no_tone_mark_warnings: sourcePinyinWarnings.length,
    source_pinyin_no_tone_mark_rows: sourcePinyinWarnings.map((issue) => ({
      row: issue.row,
      word: issue.word,
      pinyin: issue.pinyin,
    })),
  ...
  notes: [
    ...
    "It checks all Chinese source pinyin and example pinyin for Han leakage, tone-number notation and key locked readings.",
  ],
};
...
await fs.writeFile(
  ...
  [
    sampleHeaders.join(","),
    ...sampleRows.map((row) => sampleHeaders.map((header) => csvEscape(row[header])).join(",")),
  ].join("\n") + "\n"
);
...
await fs.writeFile(
  ...
  [
    ...
    `Locked Chinese readings checked: ${lockedSourcePinyin.size}`,
    ...
    "## Chinese Pinyin",
    "",
    `Unaccepted source pinyin no-tone warnings: ${sourcePinyinWarnings.length}`,
    sourcePinyinWarnings.length
      ? sourcePinyinWarnings.map((issue) => `- row ${issue.row} ${issue.word}: ${issue.pinyin}`).join("\n")
      : "- none",
    "",
    `Accepted neutral/modal source pinyin rows: ${acceptedNeutralSourcePinyinRows.length}`,
    ...acceptedNeutralSourcePinyinRows.map((row) => `- ${row.word}: ${row.pinyin}`),
    "",
    "## Findings",
    "",
    blockers.length ? blockers.map((issue) => `- BLOCKER ${issue.code}${issue.row ? ` row ${issue.row}` : ""}${issue.code ? ` ${issue.code}` : ""}: ${issue.message}`).join("\n") : "- No blockers.",
    "",

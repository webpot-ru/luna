...
const examplesPath = path.join(ROOT, "config/hsk3-level6-v2-examples.json");
const glossesPath = path.join(ROOT, "config/hsk3-level6-v2-en-glosses.json");
const classicTargetTranslationsPath = path.join(ROOT, "config/hsk3-level6-v2-reused-target-translations.json");
const manualTargetTranslationsPath = path.join(ROOT, "config/hsk3-level6-v2-manual-target-translations.json");
  ...
  return sourceRows.map((sourceRow) => {
    ...
    const row = {
      ...
      qa_notes: classicTranslations
        ? "HSK3.0 Chinese layer ready. Target-language cells copied from corrected Level 6 v2 reused target layer."
        : manualTranslations
...
function addReadme(workbook, contract, languages, reuseMap) {
  ...
  setValues(sheet, 1, 0, [
    ...
    ["Reused target translation layer", path.relative(ROOT, classicTargetTranslationsPath)],

...
if (rows.length !== 1000) {
  pushIssue(blockers, "source_row_count", `Expected 1000 HSK3.0 Level 4 rows, found ${rows.length}.`);
}
...
if (duplicateSimplified.length) {
  ...
  if (unsafeDuplicates.length) {
    ...
    pushIssue(
      ...
      `Official Level 4 contains disambiguated duplicate simplified rows: ${duplicateSimplified
        .map(([value, count]) => `${value} x${count}`)
        .join(", ")}`
    );
  }
...
for (const [index, row] of rows.entries()) {
  ...
  if (row.hsk_level !== 4) {
    pushIssue(blockers, "wrong_hsk_level", `Expected hsk_level 4, found ${row.hsk_level}.`, expectedOrder);
  }
...
await fs.writeFile(
  ...
  [
    ...
    "",
    "This gate validates only the HSK 3.0 Level 4 source layer and exact Classic overlap report. It does not build a workbook, import Docker/Postgres rows or upload Google Sheets.",
    "",

#!/usr/bin/env python3
"""Build HSK 3.0 source snapshots from the local official PDF."""
...
RELEASES = {
    "hsk3_level_6_1800_v2": {
        ...
        "page_start": 93,
        "page_stop": 138,
        "order_regex": r"(?:3[6-9]\d{2}|[45]\d{3}|5400)",
        "raw_level_regex": r"(?:6)(?:（[^）]+）)*",
        "extraction_note": (
            "Extracted from the official local PDF snapshot with bundled pypdf; "
            "rows 3601-5400 are the current official HSK 3.0 Level 6 vocabulary table. "
            "This supersedes the earlier local 1400-row boundary that stopped at 5000."
        ),
    },
    "hsk3_level_7_9_5600_v1": {
        "label": "HSK 3.0 Advanced Levels 7-9",
        "row_min": 5401,
        "row_max": 11000,
        "expected_rows": 5600,
        "hsk_level": "7-9",
        "page_start": 138,
        "page_stop": 278,
        "order_regex": r"(?:5[4-9]\d{2}|[6-9]\d{3}|10\d{3}|11000)",
        "raw_level_regex": r"(?:7-9)(?:（[^）]+）)*",
        "extraction_note": (
            "Extracted from the official local PDF snapshot with bundled pypdf; "
            "rows 5401-11000 are the combined HSK 3.0 advanced vocabulary table for Levels 7-9."
        ),
    },
}
...
def parse_level(raw_level: str) -> dict:
    ...
    if raw_level.startswith("7-9"):
        return {"level": "7-9", "raw_level": raw_level, "cross_level_notes": cross_level_notes}
    level_match = re.match(r"^\d+", raw_level)
    if not level_match:
        raise ValueError(f"Cannot parse HSK level from {raw_level!r}")
    return {"level": int(level_match.group(0)), "raw_level": raw_level, "cross_level_notes": cross_level_notes}
...
def extract_rows(release_id: str, spec: dict) -> list[dict]:
    reader = PdfReader(PDF_PATH)
    text = "\n".join(
        (reader.pages[index].extract_text() or "") for index in range(spec["page_start"], spec["page_stop"])
    )
    text = clean_pdf_text(text)
    row_start = re.compile(fr"(?<!\d)({spec['order_regex']})\s+({spec['raw_level_regex']})\s+")
    ...
    for index, match in enumerate(matches):
        ...
        if not spec["row_min"] <= order <= spec["row_max"]:
            continue
        start = match.end()
    ...
    missing = [order for order in range(spec["row_min"], spec["row_max"] + 1) if order not in orders]
    expected = spec["expected_rows"]
    if len(parsed_rows) != expected or missing:
        raise ValueError(
            f"Expected {expected} continuous rows {spec['row_min']}-{spec['row_max']}, "
            f"got {len(parsed_rows)}; missing={missing[:20]}"
        )
    return parsed_rows
...
def summarize(release_id: str, overlap_rows: list[dict]) -> dict:
...
def to_markdown(release_id: str, summary: dict, overlap_rows: list[dict]) -> str:
    sample_absent = ", ".join(
...
def main() -> None:
    if len(sys.argv) != 2 or sys.argv[1] not in RELEASES:
        valid = ", ".join(sorted(RELEASES))
        raise SystemExit(f"Usage: {Path(sys.argv[0]).name} <release_id>; valid release ids: {valid}")

    release_id = sys.argv[1]
    spec = RELEASES[release_id]
    ...
    new_rows = extract_rows(release_id, spec)
    classic_rows = load_classic_rows()
    overlap_rows = build_overlap(new_rows, classic_rows)
    summary = summarize(release_id, overlap_rows)
    ...
    overlap_md_out.write_text(to_markdown(release_id, summary, overlap_rows), encoding="utf-8")
    print(
        json.dumps(
            {
                "status": "ok",

...
const CLASSIC_TARGET_TRANSLATIONS_PATH = path.join(ROOT, "config/hsk3-level6-classic-reuse-target-translations.json");
const MANUAL_TARGET_TRANSLATIONS_PATH = path.join(ROOT, "config/hsk3-level6-manual-target-translations.json");
...
function markdownReport(report) {
  ...
  return `${[
    `# HSK3 Level 6 Manual Target Translation Gate`,
    ``,
...
if (sourceRows.length !== 1400) issue(blockers, "source_row_count_mismatch", `Expected 1400 source rows, got ${sourceRows.length}.`);
...
for (const hskKey of manualKeys) {
  ...
  if (!sourceRow) {
    issue(blockers, "manual_key_not_in_source", `Manual key is not in HSK3 Level 6 source: ${hskKey}.`, { hsk_key: hskKey });
    continue;
  }

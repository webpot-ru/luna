...
const MAIN_SHEET = "HSK3 Level 5";

const contractPath = path.join(ROOT, "config/hsk3-release-contract-v1.json");
const sourcePath = path.join(ROOT, "outputs/hsk/source/hsk3_level_5_1600_v1.source.json");
const reuseMapPath = path.join(ROOT, "outputs/hsk/qa/hsk3_level_5_1600_v1_classic_reuse_map_20260606.json");
const examplesPath = path.join(ROOT, "config/hsk3-level5-examples.json");
const glossesPath = path.join(ROOT, "config/hsk3-level5-en-glosses.json");
const classicTargetTranslationsPath = path.join(ROOT, "config/hsk3-level5-classic-reuse-target-translations.json");
const manualTargetTranslationsPath = path.join(ROOT, "config/hsk3-level5-manual-target-translations.json");
const courseMetadataPath = path.join(ROOT, "config/hsk3-level5-course-metadata.json");
...
function validateSource(rows) {
  if (!Array.isArray(rows)) throw new Error("HSK3 Level 5 source snapshot must be a JSON array");
  if (rows.length !== EXPECTED_ROWS) throw new Error(`Expected ${EXPECTED_ROWS} HSK3 rows, got ${rows.length}`);
  const seenKeys = new Set();
  for (const [index, row] of rows.entries()) {
    const expectedOrder = 2001 + index;
...
function validateChineseLayer(sourceRows, examples, glosses) {
  ...
  for (const key of Object.keys(examples)) {
    if (!sourceKeys.has(key)) throw new Error(`Example key is not in HSK3 Level 5 source: ${key}`);
  }
  for (const key of Object.keys(glosses)) {
    if (!sourceKeys.has(key)) throw new Error(`Gloss key is not in HSK3 Level 5 source: ${key}`);
  }
}
...
function validateCourseMetadata(languages, courseMetadata) {
  ...
  if (missing.length) throw new Error(`Missing HSK3 Level 5 Course Metadata for: ${missing.join(", ")}`);
  const oldSourceRefs = codes.filter((code) => /HSK 2\.0|2010-2021/u.test(courseMetadata[code].description));
  if (oldSourceRefs.length) throw new Error(`HSK3 Level 5 metadata has old Classic source refs for: ${oldSourceRefs.join(", ")}`);
  const repeatedEnglish = codes.filter(
    (code) => code !== "EN" && code !== "EN-GB" && courseMetadata[code]?.description === courseMetadata.EN?.description
  );
  if (repeatedEnglish.length) throw new Error(`HSK3 Level 5 Course Metadata descriptions repeat EN text for: ${repeatedEnglish.join(", ")}`);
}
...
function addReadme(workbook, contract, languages, reuseMap) {
  ...
  setValues(sheet, 1, 0, [
    ...
    ["Official source order", "2001-3600"],
...
function addSources(workbook, contract) {
  ...
  setValues(sheet, 1, 0, [
    ...
    [
      ...
      "Chinese base list for HSK 3.0 Level 5",
    ...
    [
      "hsk3-level5-source-snapshot",
      "Normalized source rows",
      path.relative(ROOT, sourcePath),
      "internal LunaCards normalization of official source",
      "Preserves official HSK 3.0 pinyin and row order 2001-3600.",
    ],
    [
      "hsk3-level5-classic-reuse-map",
    ...
    [
      "hsk3-level5-classic-reuse-target-translations",

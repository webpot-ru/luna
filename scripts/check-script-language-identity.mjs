...
if (setIds.length === 0) {
  throw new Error("Usage: node scripts/check-script-language-identity.mjs <set_id> [<set_id> ...]");
}
...
const findings = buildScriptLanguageIdentityFindings(rows);
if (findings.blockers.length > 0) {
  console.error(`Script/language identity check failed for ${setIds.join(", ")}: ${findings.blockers.length} blocker(s).`);
  for (const blocker of findings.blockers) console.error(formatScriptLanguageIdentityFinding(blocker));
  process.exit(1);
}

if (findings.warnings.length > 0) {
  console.error(`Script/language identity warnings for ${setIds.join(", ")}: ${findings.warnings.length} warning(s).`);
  for (const warning of findings.warnings.slice(0, 30)) console.error(formatScriptLanguageIdentityFinding(warning));
  const hidden = findings.warnings.length - 30;
  if (hidden > 0) console.error(`... +${hidden} more warning(s)`);
}

console.log(`Script/language identity OK for ${setIds.join(", ")}: ${rows.length} entry row(s), ${findings.warnings.length} warning(s).`);

...
function usage() {
  return [
    ...
    "",
    "Checks that Polyglot publication, playlist and progress registries agree.",
  ].join("\n");
}
...
function assertPolyglotIdentity({ row, label, blockers }) {
  ...
  if (Array.isArray(row.targetLangs) && row.targetLangs.map(normalizeCode).includes(normalizeCode(row.supportLang))) {
...
function main() {
  ...
  for (const row of activePublications) {
    ...
    if (!progressByKey.has(row.polyglotKey || "")) {
      blockers.push(`publication ${row.supportLang || ""} ${row.youtubeVideoId || ""}: missing progress item ${row.polyglotKey || "(missing key)"}`);
    }
  }

  for (const row of playlists.filter((item) => scoped(item, options))) {
    const key = row.playlist_key || row.key || "";
    if (!String(key).startsWith("POLYGLOT__")) blockers.push(`playlist ${key || "(missing key)"}: Polyglot registry must not contain ordinary playlist keys`);
    if (row.videoType !== "polyglot") blockers.push(`playlist ${key || "(missing key)"}: videoType must be polyglot`);
  }

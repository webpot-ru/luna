...
if (setIds.length === 0) {
  throw new Error("Usage: node scripts/check-sibling-language-copy.mjs <set_id> [<set_id> ...]");
}
...
const { warnings } = buildSiblingLanguageCopyFindings(rows);

if (warnings.length > 0) {
  console.error(`Sibling-language copy warnings for ${setIds.join(", ")}: ${warnings.length} warning(s).`);
  for (const warning of warnings.slice(0, 30)) console.error(formatSiblingLanguageCopyWarning(warning));
  const hidden = warnings.length - 30;
  if (hidden > 0) console.error(`... +${hidden} more warning(s)`);
}

console.log(`Sibling-language copy OK for ${setIds.join(", ")}: ${rows.length} row(s), ${warnings.length} warning(s).`);

...
| Food & Eating | `Food Basics`; `Fruit Basics`; `Meat, Fish & Dairy`; `Meals & Taste`; `Drink Basics`; `Coffee & Espresso Drinks`; `Tea & Hot Drinks`; `Juices, Smoothies & Cold Drinks`; `Cafe Drink Options`; `Fast Food Basics`; `Sauces & Extras`; `Takeaway & Dine-In Words`; `Alcoholic Drinks Basics`; `Bar & Alcohol Words`; `Basic Ingredients & Spices`; `Grocery Shopping Words`; `Market Shopping Words`; `Restaurant Words`; `Advanced Foods & Seafood`; vegetables, herbs/greens, bakery, sweets/snacks and diets remain backlog | Sort 25-43 generated in local DB; delivery evidence detailed through Sort 38 and needs backfill for Sort 39-43 | Strong practical path; broad food is split by shelf/category and level, not one giant deck. Sort 39-43 are current in local Postgres as `generated_checked` / `approved`, each 32 cards x 54 languages. Exact runner/Sheet evidence for Sort 39-43 was not present in the local ignored `outputs/` tree during the 2026-07-07 sync; backfill before making Sheet-specific delivery claims. |
| City & Transport | `Street & City Places`; `City Transport Basics`; `Metro & Public Transport Words`; `Direction Words`; `Taxi & Ride-share Words`; `Car & Driving Basics`; bus/train, road signs, parking and later car-interior/road words remain backlog | Sort 44-49 generated in local DB; delivery evidence needs backfill | Metro, stations, road/parking, taxi/ride-share and basic driving are now explicit current DB coverage. Sort 44-49 are `generated_checked` / `approved`, each 32 cards x 54 languages. |
| Travel & Accommodation | Travel basics, airport/flight objects, hotel check-in words, airport/baggage words, car rental words, travel problem words | Sort 50 candidate + planned backlog | `Travel Basics` exists locally as `generated` / `candidate_pool`, but it is not approved/generated_checked and is not current ready coverage. Airport nouns are separated from airport problem vocabulary. |
...
2. After the 2026-05-02 clean-start reset, only decks deliberately rebuilt from the clean DB through the documented production flow and confirmed by current state are current coverage. Local Postgres readback on 2026-07-07 confirms ordinary Deck Master Plan coverage through Sort 49: all Sort 1-49 rows are `generated_checked` / `approved` with 54-language DB coverage. Exact Google Sheet/readback evidence is documented through Sort 38 in older notes and must be backfilled from original runner/Drive artifacts for Sort 39-49 before Sheet-specific delivery claims.
...
8. The previous detailed delivery notes remain authoritative for Sort 30-38 Sheet ids and post-final audit counts. The 2026-07-07 sync advances current ordinary DB coverage to Sort 49. Sort 39-49 are all 32 cards x 54 languages = 1,728 language rows each and are `generated_checked` / `approved` in local Postgres. The next not-ready operational row is Sort 50 `Travel Basics`, currently `generated` / `candidate_pool` locally but not approved/generated_checked.

...
Если scheduled recovery блокируется существующим `unlisted` playlist, `.github/workflows/youtube-video-visibility.yml` запускается с `playlist_only=true`, пустым `video_id` и exact `playlist_id`. Этот режим проверяет OAuth channel ownership, меняет и читает обратно только playlist; использовать произвольное существующее video ID как технический носитель запрещено.

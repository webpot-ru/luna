...
Локальный provider repair после run `29325505411` сохраняет direct batch `5`, но ограничивает VectorEngine fallback двумя задачами на provider call. Production-shaped canary `2026-07-14T11:05:57Z..11:10:03Z` вернул обе тестовые metadata-задачи одним complete JSON, exact request IDs и `providerCallCount=1`; GitHub dispatch, render, TTS и YouTube writes не выполнялись. Это доказывает рабочий размер `2`, но также показывает высокую latency около четырех минут, поэтому VectorEngine остается последним fallback, а не primary backend.

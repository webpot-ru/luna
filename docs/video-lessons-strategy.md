...
- 2026-06-21 first-deck GitHub canary: run `27897570929` passed on pushed commit `9f9d319` for `home_kitchen_cookware_pilot_01`, `support=RU`, `langs=ES`, `mode=plan`, `privacy=unlisted`, `create_playlists=true`, `generate_thumbnails=true`. It generated the video, `youtube_metadata.json`, VectorEngine `gpt-image-2` thumbnail, thumbnail QA, metadata QA, SEO QA, TTS variant contract and publish plan without YouTube writes. Readback: thumbnail 1280 x 720 JPEG; MP4 H.264 1920 x 1080, 483.92s; target/support voices `edge_es-ES-AlvaroNeural` / `edge_ru-RU-DmitryNeural`; course URL `https://flashcardsluna.com/ru/courses/kitchenware-basic/study/standard?langs=es`. Metadata fell back to `source=template-ai-fallback` because VectorEngine Gemini returned non-JSON, so this canary is accepted only as a build/render/thumbnail/TTS/planner plumbing proof. It is not eligible for live `mode=apply` under the current metadata policy until metadata is regenerated through AI or human-curated metadata. The first real apply should create and record playlist `RU__ES__ordinary-vocabulary__a1-everyday`.

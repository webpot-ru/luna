...
- 2026-06-23 slow route-scoped retry update: the next safe layer deliberately used one target per GitHub run and only a few support routes at a time. Nine more missing targets were uploaded/scheduled and persisted (`ZH/DE`, `RO/EN`, `SR/EN`, `ZH/EN`, `RO/ES`, `SR/ES`, `ZH/ES`, `RO/FR`, `SR/FR`), while `TE/EN` failed before upload on a VectorEngine image-generation timeout and must be retried or configured for accepted automatic-thumbnail fallback before spending a YouTube upload. The current missing set is 52 targets; do not broaden this back into a 50-run wave unless route quota has been checked or reset and the stop rule remains active.

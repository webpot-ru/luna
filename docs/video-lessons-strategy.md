...
- `.github/workflows/youtube-video-publish.yml` can generate thumbnails after video/metadata creation when `generate_thumbnails=true`, but requires explicit `confirm_thumbnail_spend=GENERATE_THUMBNAILS` because custom thumbnails spend VectorEngine image credits. As of 2026-06-26, the thumbnail path is fail-closed: `generate_thumbnails=true` does not mean "generate images for every channel"; image generation is skipped unless the channel has `customThumbnailUploadAllowed=true`, and skipped channels use YouTube automatic thumbnails. As of 2026-07-13, after explicit user confirmation for Uzbek, Sinhala and Georgian, the current custom-thumbnail allowlist in `config/youtube-channels.json` is `en`, `ru`, `es`, `pt`, `ja`, `tr`, `th`, `ne`, `sw`, `vi`, `my`, `sr`, `uz`, `si` and `ka`; this supersedes the historical 2026-07-05 Georgian-pending note. Other channels stay `false` or unset. The workflow builds `outputs/video-generator/youtube-thumbnail-review-github.{json,csv,html,svg}` after thumbnail validation and before metadata/SEO/publish gates when thumbnail processing is enabled. In `mode=apply`, publish planning allows candidates without custom thumbnails only when they are explicitly marked `thumbnailUploadMode=first_frame_auto`.
- As of 2026-07-16, `longVideoUploadAllowed` is a separate per-channel capability in the same registry. Its current explicit true allowlist contains 16 channels: `en`, `ru`, `es`, `pt`, `ja`, `tr`, `vi`, `th`, `sr`, `my`, `ne`, `si`, `uz`, `ka`, `sw` and `zh`. This list is intentionally different from the 15-channel custom-thumbnail allowlist above: the user confirmed ZH for videos longer than 15 minutes, while `ZH.customThumbnailUploadAllowed` remains `false` because no successful `thumbnails.set` evidence was provided. Future changes must update these fields independently: custom-thumbnail permission controls only `thumbnails.set`; long-video permission permits the full Polyglot product. For a false/unknown long-video capability, campaign planning now converts a requested full product into measured `short_unverified` before claim, records the fallback reason, and never pays for a full render that will be rejected late. `shortUnverifiedPolyglotCardLimit=0` measures the whole candidate deck, retains the maximum contiguous prefix that fits `895` seconds (`14:55`) and writes `polyglot-duration-selection.json`; the cache is reused in final render, so this is not a second TTS pass. A blocked support `videoProductionReadiness` (currently HY / AI33) is instead deferred from the shared campaign with no claim or provider work. The short item never marks the required full Polyglot bundle as complete, and a full/short pair for the same support channel + bundle is a preflight blocker.
...
- 2026-08-03 playlist-image replacement lane: the default playlist-image apply remains missing-only and never overwrites an installed image. The explicit `replace` mode in `.github/workflows/youtube-playlist-image-batch-set.yml` and `--replace-existing` in `scripts/youtube-upload-playlist-images.mjs` require a fresh `playlistImages.list` readback proving the exact installed image ID, the matching playlist/channel registry identity, a Git-tracked replacement JPG and the `REPLACE_YOUTUBE_PLAYLIST_IMAGES` confirmation token. The uploader performs one `playlistImages.update` (`PUT`) per exact installed row, fails closed if the image ID has drifted or disappeared, waits for bounded readback and persists the replacement source/evidence. The 2026-08-03 review package is `data/youtube-playlist-covers/20260803-v2-replacement/manifest.json`: it contains `425` existing images across `10` channels; `2,518` playlists were absent and `23` had no durable playlist ID, so neither group is silently included in the replacement lane.
- 2026-08-05 missing-only continuation wave: the complete eight-route audit found `314` absent, ID-backed playlists on the `12` channels currently marked `playlistImageUploadAllowed=true`; the other absent rows remain blocked by channel capability or missing durable playlist ID. The first tracked continuation package is `data/youtube-playlist-covers/20260805-missing-only-wave1-5-per-channel/manifest.json` with exactly `5` absent rows per enabled channel (`60` inserts, estimated `3,192` general quota units). It contains only `auditState=absent` rows with copied readback evidence and is intentionally separate from the replacement package.
- 2026-08-05 missing-only continuation wave 2: after wave 1 completed with `60/60` confirmed playlist-image readbacks, `data/youtube-playlist-covers/20260805-missing-only-wave2-5-per-channel/manifest.json` selects the next `5` remaining absent rows per each of the same `12` enabled channels (`60` inserts, estimated `3,192` general quota units). Wave 1 keys are explicitly excluded; this package is missing-only and never replaces an installed image.
- 2026-08-05 missing-only continuation wave 3: after waves 1 and 2 completed with `120/120` confirmed readbacks, `data/youtube-playlist-covers/20260805-missing-only-wave3-5-per-channel/manifest.json` selects the next up-to-five absent rows per enabled channel (`43` inserts, estimated `2,291` general quota units), including the small remaining tails. Earlier wave keys are explicitly excluded.
- 2026-08-05 missing-only continuation wave 4: after wave 3 completed with `163/163` confirmed readbacks across waves 1-3, `data/youtube-playlist-covers/20260805-missing-only-wave4-5-per-channel/manifest.json` selects the next up-to-five absent rows for the remaining enabled tails (`28` inserts, estimated `1,490` general quota units). It covers `EN` (5), `SI` (5), `UZ` (5), `JA` (3), `TR` (5) and `KA` (5); all earlier wave keys are excluded and installed images are never replaced.
- 2026-08-05 missing-only continuation wave 5: after waves 1-4 reached `191/191` confirmed readbacks, `data/youtube-playlist-covers/20260805-missing-only-wave5-5-per-channel/manifest.json` selects the next up-to-five absent rows (`24` inserts, estimated `1,277` general quota units) for `EN` (5), `SI` (5), `UZ` (5), `TR` (4) and `KA` (5). All earlier wave keys are excluded and installed images are never replaced.
- 2026-08-05 missing-only continuation wave 6: after waves 1-5 reached `215/215` confirmed readbacks, `data/youtube-playlist-covers/20260805-missing-only-wave6-5-per-channel/manifest.json` selects the next up-to-five absent rows (`20` inserts, estimated `1,064` general quota units) for `EN` (5), `SI` (5), `UZ` (5) and `KA` (5). All earlier wave keys are excluded and installed images are never replaced.
- 2026-08-05 missing-only continuation wave 7: after waves 1-6 reached `235/235` confirmed readbacks, `data/youtube-playlist-covers/20260805-missing-only-wave7-5-per-channel/manifest.json` selects the remaining next-up-to-five absent rows (`19` inserts, estimated `1,011` general quota units) for `EN` (4), `SI` (5), `UZ` (5) and `KA` (5). All earlier wave keys are excluded and installed images are never replaced.
- 2026-08-05 missing-only continuation wave 8: after waves 1-7 reached `254/254` confirmed readbacks, `data/youtube-playlist-covers/20260805-missing-only-wave8-5-per-channel/manifest.json` selects the next `15` absent rows (estimated `798` general quota units) for `SI` (5), `UZ` (5) and `KA` (5). All earlier wave keys are excluded and installed images are never replaced.
- 2026-08-05 missing-only continuation wave 9: after waves 1-8 reached `269/269` confirmed readbacks, `data/youtube-playlist-covers/20260805-missing-only-wave9-5-per-channel/manifest.json` selects the next `15` absent rows (estimated `798` general quota units) for `SI` (5), `UZ` (5) and `KA` (5). All earlier wave keys are excluded and installed images are never replaced.
- 2026-08-05 missing-only continuation wave 10: after waves 1-9 reached `284/284` confirmed readbacks, `data/youtube-playlist-covers/20260805-missing-only-wave10-5-per-channel/manifest.json` selects the next `15` absent rows (estimated `798` general quota units) for `SI` (5), `UZ` (5) and `KA` (5). All earlier wave keys are excluded and installed images are never replaced.
- 2026-08-05 missing-only continuation wave 11: after waves 1-10 reached `299/299` confirmed readbacks, `data/youtube-playlist-covers/20260805-missing-only-wave11-5-per-channel/manifest.json` selects the next `12` absent rows (estimated `639` general quota units) for `SI` (5), `UZ` (5) and `KA` (2). All earlier wave keys are excluded and installed images are never replaced.
- 2026-08-05 missing-only continuation wave 12: after waves 1-11 reached `311/311` confirmed readbacks, `data/youtube-playlist-covers/20260805-missing-only-wave12-5-per-channel/manifest.json` closes the final `3` absent rows (estimated `161` general quota units) for `SI` (2) and `UZ` (1). All earlier wave keys are excluded and installed images are never replaced.
- 2026-08-03 playlist-image update transport correction: YouTube accepts the media replacement for `playlistImages.update` through a resumable upload session, not the multipart body used by the insert path. The replacement helper now initiates `PUT /resumable/upload/youtube/v3/playlistImages?uploadType=resumable&part=snippet` with the exact `snippet.playlistId`/`snippet.type` metadata, then sends the tracked JPG bytes to the returned `Location` with the required `Content-Range` header while explicitly preserving `uploadType=resumable&part=snippet` if the service omits those query fields from the session URL; the existing-image ID readback/drift guard remains required before initiation. The workflow also exposes `limit_per_channel` for a bounded control run (`0` means the full selected manifest). The earlier replacement attempts stopped on YouTube's `youtube.part/unexpectedPart` response before any successful image write, so no replacement was counted from those runs.
...
- 2026-08-04 playlist-image replacement transport follow-up: the previous resumable transport note is superseded. The helper now uses the generated Google API clients' `PUT /upload/youtube/v3/playlistImages?uploadType=resumable&part=snippet` media URL, includes the installed image `id` in update metadata, and sends bytes to the returned opaque `Location` without rewriting its query string. The existing-image readback/drift guard and `Content-Range` remain mandatory; the earlier `youtube.part/unexpectedPart` attempts produced no replacement writes.
- 2026-08-05 playlist-image replacement control finding: the API still returned `youtube.part/unexpectedPart` when `part=snippet` was present in the update session initiation, even with the official `/upload/youtube/v3/playlistImages` media URL and an untouched `Location`. The update path therefore omits `part` from the resumable initiation; the request body still carries the installed image `id` and `snippet` metadata, and the final session PUT keeps `Content-Range`.
- 2026-08-05 playlist-image session readback: after omitting `part` at initiation, YouTube echoed `part=snippet` into the resumable `Location` and rejected it on the final media PUT. The helper now removes only that echoed `part` query parameter while preserving the opaque session identifier and all other session fields; no replacement write has succeeded yet.
- 2026-08-05 playlist-image replacement control closure: GitHub controls `30977150575`, `30977368839` and `30977621121`, plus local NE controls, all reached YouTube with valid route OAuth and failed before a write with `400 youtube.part/unexpectedPart` on `playlistImages.update`. Tested resumable and multipart PUT, both `/upload` and `/resumable/upload`, official Java-client session ordering, both Google hosts, and resource variants with `id`/`kind`; none succeeded. An `insert` probe against an installed hero correctly returned `IMAGE_TYPE_ALREADY_EXISTS` and did not change state. No replacement was counted; do not launch the 425-row replacement manifest until YouTube update semantics are separately verified.

...
2026-06-20 export proof: `npm run export:youtube-channel-studio-tasks -- --manual-needed` produced 40 manual-needed tasks at `outputs/youtube-channel-assets/youtube-channel-studio-tasks-2026-06-20T11-51-23-110Z.json`, `.csv` and `.md`. This did not open Chrome, did not write to YouTube and did not print secrets.
...
2026-06-20 GitHub readback correction: the GitHub workflow exists, but API branding from GitHub is not yet ready to run. `gh workflow view` showed 0 successful `YouTube Channel Branding API` runs before verification; `gh secret list --repo webpot-ru/luna` showed only `VECTORENGINE_API_KEY`; `gh api repos/webpot-ru/luna/environments` showed no environments; and `gh secret list --env youtube-api-branding` returned 404 because the environment/secret does not exist. A safe `mode=plan` dispatch run `27872492518` failed because npm's command banner was redirected into the JSON report. The workflow is patched to call `node scripts/youtube-channel-branding.mjs --dry-run --json` directly. Do not run `mode=apply` until `YOUTUBE_OAUTH_BUNDLE_TGZ_B64` exists as a GitHub secret, `mode=plan` succeeds, `mode=readback` succeeds, and the user explicitly confirms YouTube API writes.
...
GitHub boundary for YouTube access: do not upload OAuth token JSON files, Google OAuth client secret JSON, `.local` contents or contact-email defaults to the repository. For channel API branding from GitHub, use only the encrypted repository secret `YOUTUBE_OAUTH_BUNDLE_TGZ_B64`, restore it only inside the workflow runner, and keep repo files limited to non-secret channel IDs, handles, support-language mappings, public branding assets and metadata templates. If GitHub Actions later needs direct video upload/publish access beyond channel branding, document the exact secret names/scope separately before enabling it.

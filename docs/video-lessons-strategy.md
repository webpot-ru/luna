...
- As of 2026-07-16, `longVideoUploadAllowed` is a separate per-channel capability in the same registry. Its current explicit true allowlist contains 16 channels: `en`, `ru`, `es`, `pt`, `ja`, `tr`, `vi`, `th`, `sr`, `my`, `ne`, `si`, `uz`, `ka`, `sw` and `zh`. This list is intentionally different from the 15-channel custom-thumbnail allowlist above: the user confirmed ZH for videos longer than 15 minutes, while `ZH.customThumbnailUploadAllowed` remains `false` because no successful `thumbnails.set` evidence was provided. Future changes must update these fields independently: custom-thumbnail permission controls only `thumbnails.set`; long-video permission controls whether a full Polyglot upload is safe. A false/unknown long-video capability selects the one permitted `short_unverified` product with a hard maximum of 895 seconds (`14:55`). The configured `shortUnverifiedPolyglotCardLimit=0` means dynamic selection, not an empty lesson: the renderer uses the whole deck as a candidate set, measures the actual cached TTS for each card plus intro/outro, retains the maximum contiguous prefix that fits and writes `polyglot-duration-selection.json`. The same cache is reused in final render, so this is not a second TTS pass. The short item never marks the required full Polyglot bundle as complete, and a full/short pair for the same support channel + bundle is a preflight blocker.
...
Full-scope Polyglot planning includes only canonical support/native channels with `longVideoUploadAllowed=true`. Custom-thumbnail permission never removes a channel from that candidate set. A false or unknown long-video capability is not a reason to skip the channel: campaign planning automatically converts its requested Polyglot item into the one dedicated `short_unverified` product before metadata, TTS and render. A short render is capped at `895s` (`14:55`), and any duration failure stops that item with no retry or replacement dispatch.
...
For every other support channel, `customThumbnailUploadAllowed=false` or an unknown value disables custom-cover creation/upload only. It does not itself decide Polyglot scope. The workflow uses YouTube automatic thumbnail fallback and must not call `thumbnails.set`; `longVideoUploadAllowed` decides the product before provider work: `true` permits full, false/unknown permits only the one short product.

Short rule for unverified channels, reaffirmed on 2026-07-15: every channel without `longVideoUploadAllowed=true` must receive the deliberately truncated Polyglot product through `content_scope=short_unverified`; it is not optional and it is not a reason to leave the channel without a Polyglot video. This product stays under YouTube's default 15-minute limit for accounts/channels that may not have advanced features. The completed first kitchen-deck short wave covered 42 channels that were unverified at that time. Later confirmations moved `VI`, `MY`, `SR`, `SI`, `UZ`, and `KA` into future full-scope eligibility; their historical `short_unverified` rows remain valid publication history but do not satisfy a full slot. Historical first-deck rows used `limit=30`; all future campaigns use `shortUnverifiedPolyglotCardLimit=0`, which measures the full candidate deck and selects the largest contiguous prefix that fits `max_duration_seconds=895` (`14:55`). The workflow runs `npm run check:polyglot-video-duration` after rendering; upload is blocked if ffprobe reports a duration above the cap. Short videos use separate progress/playlist keys, but an active full row is a hard preflight blocker for a short and an active short is a hard preflight blocker for a full. Short unverified uploads use YouTube automatic thumbnail fallback and must not call `thumbnails.set`.

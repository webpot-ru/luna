...
2026-06-20 GitHub readback correction: the GitHub workflow exists, but API branding from GitHub is not yet ready to run. `gh secret list --repo webpot-ru/luna` shows only `VECTORENGINE_API_KEY`; `gh api repos/webpot-ru/luna/environments` now shows environment `youtube-api-branding`; `gh secret list --env youtube-api-branding --repo webpot-ru/luna` returns an empty list, so `YOUTUBE_OAUTH_BUNDLE_TGZ_B64` is not present. A safe `mode=plan` dispatch run `27872492518` failed because npm's command banner was redirected into the JSON report. Commit `281be58` fixed the workflow to call `node scripts/youtube-channel-branding.mjs --dry-run --json` directly, and pushed plan run `27872580979` succeeded with `planned_channels=51` plus a non-secret report artifact. Do not run `mode=readback` or `mode=apply` until `YOUTUBE_OAUTH_BUNDLE_TGZ_B64` exists as an environment secret, `mode=readback` succeeds from GitHub, and the user explicitly confirms YouTube API writes.
...
GitHub boundary for YouTube access: do not upload OAuth token JSON files, Google OAuth client secret JSON, `.local` contents or contact-email defaults to the repository. For channel API branding from GitHub, use only the encrypted environment secret `YOUTUBE_OAUTH_BUNDLE_TGZ_B64` in environment `youtube-api-branding`, restore it only inside the workflow runner, and keep repo files limited to non-secret channel IDs, handles, support-language mappings, public branding assets and metadata templates. If GitHub Actions later needs direct video upload/publish access beyond channel branding, document the exact secret names/scope separately before enabling it.

...
- 2026-07-11 Deck #1 full Polyglot closeout: the approved manifest now has `51` exact support/bundle JPG slots. Durable-state and manifest reconciliation after runs `29156066511`, `29156070844`, `29156074065`, `29156077075`, `29156080227` and `29156083971` is `50/51` active, with no multiple-active Polyglot keys. The six new scheduled uploads are `MY/global` `B2pJBKf78rM`, `MY/east` `yQ3894R4b_4`, `VI/global` `-S-SHVrn74U`, `VI/east` `SGXn8aa_7Xs`, `SR/global` `fI4PgvlKTv0` and `SR/east` `RgeghsHzXcQ`; every row has an approved custom pre-rendered cover. The only missing approved slot is `KM/global_europe_core`: historical `OlbN0CO44T4` is `missing_video_public_readback_superseded`, and replacement remains blocked until a successful custom-thumbnail/advanced-feature readback changes `km.customThumbnailUploadAllowed` from `false`. Do not upload a full-scope KM replacement merely because its approved JPG exists.
- 2026-07-11 Deck #2 canonical Polyglot covers: publishing scope is exactly `12` canonical/custom-thumbnail-enabled physical channels times `4` core bundles, or `48` JPGs. Final manifest: `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cooking_actions_a1_a2-approved-polyglot-core-canonical-20260711/manifest.json`; visual review: sibling `contact-sheet.jpg`. The package excludes target-only native variants `EN-GB`, `ES`, `PT`, blocked `KM`, and optional `southeast_asia_core`. The artwork is local/pre-rendered with a prominent `POLYGLOT · N` badge; no VectorEngine/GPT Image call was made. This package supersedes the old `80`-cover planned manifest for GitHub publishing. Do not dispatch until the 48-slot no-spend metadata/cover preflight passes and the user separately approves the scheduled apply batch.
- 2026-07-07 thumbnail-only recovery for the final approved first-deck covers: the approved ordinary manifest has `686` `by-channel` JPG covers and the approved Polyglot manifest originally had `46` `by-channel` JPG covers; it was extended to `51` exact Polyglot slots on 2026-07-11. Initial GitHub thumbnail apply runs could not see most of the local images because the JPG files were ignored/untracked on the runner; affected rows reported `missing_cover` even though the files existed locally. Commit `de1ad78d` on branch `codex/youtube-scheduled-batch-20260707` tracks the missing approved cover files exactly (`669` ordinary + `44` Polyglot new JPGs; `19` JPGs were already tracked). After that commit, local no-spend ordinary planning reported `98` ready rows still missing custom thumbnails. Four thumbnail-only apply runs were dispatched on `de1ad78d` with no render/TTS/video upload/VectorEngine work: `28867320731` for `youtube-api-branding` (`EN,ES-419,JA,PT-BR,RU,TR`), `28867322077` for `youtube-api-youtube-2` (`TH,VI`), `28867323357` for `youtube-api-youtube-3` (`KM,MY,SR`) and `28867324575` for `youtube-api-youtube-4` (`NE`). Readback result: `28867320731` and `28867322077` completed `success` with persisted state; `28867323357` failed on the first `KM -> EN` thumbnail write for video `ANoeDeLN-yI` with YouTube API `youtube.thumbnail/forbidden`, so `config/youtube-channels.json` marks `km.customThumbnailUploadAllowed=false` until Studio/advanced-feature confirmation; `28867324575` successfully set `NE -> NO` on video `c0GBP1kNY9Y`, but its persist job was cancelled, so `config/youtube-published-videos.json` was repaired locally from the verified apply log. After that local repair, the no-spend ordinary plan reports `502` ready rows, `184` blocked rows (`53` `KM` rows blocked as `custom_thumbnail_not_confirmed`, `131` rows blocked as `no_active_publication`) and `38` ready rows still without `thumbnailSet=true` / `thumbnailUploadMode=custom`. No retry or additional GitHub dispatch has been launched after the `KM` failure readback.
...
- 2026-07-10 metadata backend correction: bulk ordinary dispatch plans `metadata_gemini_backend=api,vectorengine`, `metadata_concurrency=1`, `metadata_batch_size=4` and `metadata_rate_limit_ms=15000`. Each direct batch tries `GEMINI_API_KEY`, then `GEMINI_API_KEY_2`; only after both fail does the confirmed batch fall back to per-target VectorEngine metadata. Apply refuses the chain without `confirm_vectorengine_metadata=USE_VECTORENGINE_METADATA`. Metadata completes before render/TTS/upload.
...
For parallel GitHub workers, scheduled mode must pass the generation target preflight report as `--target-plan`. The planner uses the target's global ordinal within the channel's eligible target list and chooses that Nth-free slot. This prevents worker 0, worker 1 and worker 2 from all reserving the first local slot when they run at the same time from the same committed calendar. Any bulk-dispatch support list must be filtered to canonical support variants before this point: `EN`, `ES-419`, `PT-BR`, and the one-code channels.
...
GitHub ordinary bulk metadata workflow использует подтверждаемый chain `api,vectorengine`. Direct keys пробуются в порядке `GEMINI_API_KEY`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY`; если все прямые ключи дают ошибку, batch переходит к VectorEngine только при `confirm_vectorengine_metadata=USE_VECTORENGINE_METADATA`. Без подтверждения apply не запускается. Если полный подтверждённый chain не дал валидные metadata, workflow падает до video render/TTS/upload. Template fallback разрешён только для plan diagnostics; apply остаётся fail-closed.

2026-07-10 direct Gemini batching policy: ordinary metadata groups up to four targets per direct request, with `metadata_concurrency=1` and `metadata_rate_limit_ms=15000`. Direct Google calls use `POST /v1beta/models/{model}:generateContent`, `responseMimeType=application/json`, `thinkingLevel=minimal`, a balanced-JSON parser and a 120-second timeout. `responseSchema` is intentionally omitted: local isolation showed schema requests hanging while JSON MIME without schema reached the same valid key/model. The prompt still requires the exact object shape and downstream metadata/language/SEO gates remain fail-closed. If the direct batch fails after both keys, confirmed VectorEngine fallback uses the compact per-target prompt without repeating the failed direct calls for every target.
...
For `support=EN`, `english_bundle=global_europe_core` resolves visible targets to `ES,FR,DE,IT` after removing English. `EN-GB` is a target/studied variant only and must not be dispatched as a support/native row. For Russian canary with `bundle=global_europe_core`, expected targets are `EN,ES,FR,DE`.

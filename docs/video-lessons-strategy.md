...
- `.github/workflows/youtube-thumbnail-batch-set.yml` is the batch thumbnail-only workflow for already uploaded videos when local JPG covers are committed with a manifest. It has `mode=plan` for a GitHub-side dry run and `mode=apply` for bounded route/support groups; apply requires `confirm_youtube_write=APPLY_YOUTUBE_THUMBNAIL_BATCH`, an explicit `youtube_environment`, and calls only `scripts/youtube-set-video-thumbnail.mjs` / `youtube.thumbnails.set`. It must not render or upload videos and must not modify metadata, playlists, privacy, schedules or calendar reservations. The 2026-07-04 first-deck ordinary batch plan run `28703020717` verified `469 ready / 0 blocked`. The first route apply run `28703167453` set 85 thumbnails on `youtube-api-branding`, persisted state, then stopped on `youtube.quota/quotaExceeded`. After that quota stop, batch apply was made idempotent: it skips rows with `thumbnailSet=true` before calling `thumbnails.set`, so retries must not replay already-set thumbnails or spend quota on them. Route applies `28703588025` (`VI`, 38/38 on `youtube-api-youtube-2`), `28703751061` (`MY`, 36/36 on `youtube-api-youtube-3`) and `28703860472` (`NE`/`SW`, 6/6 on `youtube-api-youtube-4`) completed with successful apply and persist jobs. Current durable first-deck ordinary custom-thumbnail state is `447/469 thumbnailSet=true`; the remaining `22` missing rows are all on `youtube-api-branding`: `JA 6`, `PT 6`, `RU 4`, `TR 6`. Retry only those rows after route-1 quota reset. `KA` and `SR` stay out of the custom-thumbnail upload allowlist until separate proof/readback changes that policy.
...
2026-07-02 completed `romance_core` wave for `home_kitchen_cookware_pilot_01`: durable state has 51/51 scheduled uploaded Polyglot rows, with 9 full thumbnail-enabled supports (`EN`, `RU`, `ES`, `PT`, `JA`, `TR`, `TH`, `NE`, `SW`) and 42 `short_unverified` supports (`limit=30`, `max_duration_seconds=895`, automatic first-frame thumbnail). The configured same-family fallback is `RO`, so support collisions use `FR,IT,PT,RO` for `ES`, `ES,IT,PT,RO` for `FR`, `ES,FR,PT,RO` for `IT`, and `ES,FR,IT,RO` for `PT`. The wave had 26 successful uploads whose workflow/persist job was cancelled; those were recovered from child artifacts without reupload. Transient pre-upload TTS failures for `SV`, `BN`, `ET`, `TL` and `HY` succeeded on first retry. `config/youtube-publish-calendar.json` has matching `romance_core` reservations, so future waves must treat those physical-channel slots as occupied. Current gate: `npm run check:youtube-polyglot-state -- --set=home_kitchen_cookware_pilot_01 --bundle=romance_core --json`.

2026-07-04 completed `slavic_core` wave for `home_kitchen_cookware_pilot_01`: durable state has 51/51 scheduled uploaded Polyglot rows, with 9 full thumbnail-enabled supports (`EN`, `RU`, `ES`, `PT`, `JA`, `TR`, `TH`, `NE`, `SW`) and 42 `short_unverified` supports (`limit=30`, `max_duration_seconds=895`, automatic first-frame thumbnail). `slavic_core` targets are `RU,PL,CS,SK`; support collisions resolve with `BG`, for example `support=RU -> PL,CS,SK,BG` and `support=SK -> RU,PL,CS,BG`. Operational recovery notes: 13 successful uploads with cancelled workflow/persist were recovered from artifacts without reupload in commit `280e06f`; transient TTS retries closed `KN` (`28660612771`, video `am7EUIJszpw`) and `KA` (`28661072913`, video `b-EPMmY3ejI`). Final route-1 completion runs were `28698845409` (`RU`, video `16izzfan6XI`), `28698864894` (`FR`, `624J068RRmc`), `28698880215` (`JA`, `slG9za2NrSQ`), `28698895943` (`TR`, `Oi8FzQqfn-M`) and `28698962897` (`DE`, `K3ILZouSXC8`); all completed with successful upload and `persist-polyglot-publish-state`. `config/youtube-publish-calendar.json` has matching 51 `slavic_core` Polyglot reservations from durable state. Existing same-channel same-`publishAt` conflicts with ordinary scheduled videos remain a deliberate schedule/visibility repair topic, not a reason to reupload. Current gate: `npm run check:youtube-polyglot-state -- --set=home_kitchen_cookware_pilot_01 --bundle=slavic_core --json`.

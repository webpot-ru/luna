...
2026-06-21 direct-set generation proof: the remaining 50 channels were regenerated with `npm run generate:youtube-channel-banners -- --confirm-spend --codes=... --no-skip-existing --regenerate-raw --regenerate-source --skip-refit`, then all 51 direct upload JPGs/previews/contact sheets were produced with `python3 scripts/export-vectorengine-channel-banners-direct.py`. After a native-style copy screen, the 13 flagged/polish languages were corrected in `config/youtube-channel-banner-copy.json` and redrawn through the same VectorEngine path: `CS`, `ES`, `FR`, `HI`, `HU`, `ID`, `JA`, `KO`, `NE`, `PT`, `SW`, `TR`, `UZ`. Local QA checked 51/51 direct upload files as `2560x1440`, under 6 MB, with metadata `localizedCopy.brand=FlashcardsLuna`; `node scripts/sync-youtube-channel-branding-assets.mjs --to-assets --verify` synced 53 public branding assets, and `npm run plan:youtube-channel-branding -- --channel=en` plus `--channel=az` both resolved the new direct banner paths. No live YouTube upload was performed by this local generation pass.

2026-06-21 native-style banner-copy screen: the direct set is visually acceptable as channel art, but it is not native-speaker certified. A planned Gemini second-pass failed locally with `IneligibleTierError / unsupported client` from Gemini CLI, and no Gemini API key fallback was available. The first local linguistic screen flagged `FR`, `HU`, `ID`, `SW` for required changes and `CS`, `ES`, `HI`, `JA`, `KO`, `NE`, `PT`, `TR`, `UZ` for polish; those 13 copies were corrected and redrawn on 2026-06-21. Remaining pre-upload risk is rendered-text fidelity, especially complex scripts that still require native or OCR/vision readback: `KM`, `LO`, `MY`, `SI`, `ML`, `TA`, `TE`, `KN`, `KA`, `HY`, `BN`, `HI`.
...
- `npm run build:youtube-thumbnail-review -- <metadata-file-or-dir> --output-prefix=<prefix>` builds the pre-upload review bundle: `<prefix>.json`, `<prefix>.csv`, `<prefix>.html` and `<prefix>.svg`. It maps every thumbnail to `setId`, `supportLang`, `targetLang`, title, expected thumbnail text, `playlist_key`, `publishAt` and file paths, and blocks duplicate `setId + supportLang + targetLang` rows or missing thumbnails. For scheduled runs, pass `--require-publish-at` so a thumbnail cannot reach upload without a visible planned public time in the manifest.
- `.github/workflows/youtube-video-publish.yml` can generate thumbnails after video/metadata creation when `generate_thumbnails=true`, but requires explicit `confirm_thumbnail_spend=GENERATE_THUMBNAILS` because this spends VectorEngine image credits. The workflow now always builds `outputs/video-generator/youtube-thumbnail-review-github.{json,csv,html,svg}` after thumbnail validation and before metadata/SEO/publish gates. In `mode=apply`, publish planning refuses candidates without custom thumbnails.
...
- `config/youtube-publish-schedule-policy.json` is the per-support-language publication calendar. It currently covers all 51 configured public support-language channels, assigns each channel an IANA timezone, and uses six default local slots per channel (`08:30`, `11:30`, `14:30`, `17:30`, `20:30`, `23:30`). Accepted starting cadence is therefore **6 public releases per channel per local day**, not immediate public release after upload. Therefore 50 videos for one ordinary support-language channel take about 9 local days, and the shared `en`/`es`/`pt` channels take about 18 local days for one full 54-variant deck because they receive two support variants. Since all 51 channels are already configured, the correct first production shape is one deck across all 51 channels, not one channel at a time. The safety limit is deck-wave depth and per-channel cadence: do not start many decks at once or bypass schedule/readback; run one-deck/all-channels waves, then watch 24h/72h/7d metrics plus policy health before adding the next deck wave or raising cadence. If the first waves stay healthy, the next review point may consider 8-12 public releases/day/channel, but that is not the starting plan.
...
- `npm run plan:youtube-publish-schedule -- <metadata-file-or-dir> --start-date=YYYY-MM-DD [--limit=50] [--limit-per-channel=50] [--write-metadata]` plans scheduled upload times without YouTube writes. With `--write-metadata`, it writes `privacyStatus=private`, `publishAt`, `scheduledPublishAt` and `publishSchedule.analyticsCheckpointsAt` into each scheduled `youtube_metadata.json`.
...
9. write `videoId`, `playlistId`, `playlistItemId`, status, privacy, `publishAt` if scheduled, and readback timestamp to `outputs/youtube-publish-ledger.jsonl`;

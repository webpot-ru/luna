...
- 2026-07-14 metadata batching correction: bulk ordinary dispatch uses `metadata_gemini_backend=api,vectorengine`, sequential metadata batches, `metadata_batch_size=10` and `metadata_rate_limit_ms=15000`. One structured `generateContent` request returns metadata for up to 10 target videos. A recoverable direct failure advances once through `GEMINI_API_KEY`, `GEMINI_API_KEY_2`, then `GOOGLE_API_KEY`; only after the direct keys are exhausted may the explicitly confirmed VectorEngine provider receive the same whole batch once. `api,vectorengine` is parsed as an ordered chain and can never fall through to Gemini CLI. CLI is local/explicit only. Polyglot uses the same direct-key-first chain for its single metadata task and no longer performs three blind VectorEngine retries. Apply remains fail-closed before render/TTS/upload if the chain or per-item language/SEO gates fail.
- 2026-07-14 key-order clarification (supersedes the key-order sentence immediately above): direct order is primary `GEMINI_API_KEY` (or legacy `GOOGLE_API_KEY` alias), then `GEMINI_API_KEY_2` only when the first key receives a quota/permission failure, then one explicitly confirmed VectorEngine fallback. Invalid JSON or a provider-wide 5xx does not spend the second key. CLI is never implicit.
- The same order is enforced in ordinary publish, Polyglot publish, legacy compile/test workflows and playlist/metadata repair tools. Legacy compile/test workflows require `confirm_vectorengine_metadata=USE_VECTORENGINE_METADATA` before their final VectorEngine fallback; repair workflows retain their existing explicit AI-spend confirmation.
...
- Anti-duplicate identity is an active publication assignment, not a title, hash or run id. Ordinary identity is `setId + canonical supportLang + targetLang`. A Polyglot product slot is `setId + canonical supportLang + bundle + contentScope`; `targetsHash` is only the content fingerprint inside that slot and never permits a second active video. `full` and `short_unverified` are separate scopes, so a short row never satisfies one of the four required long/full slots. Legacy rows without a current `polyglotKey` are matched by an order-independent target set within the same scope. Historical support rows collapse as `EN-GB -> EN`, `ES -> ES-419` and `PT -> PT-BR`. An apply workflow must never use `allow_republish=true`: repair, supersede or delete the prior publication before creating a replacement.
...
- The same workflow builds `youtube-publication-snapshot.json` and a compact generated publication map. Optional `persist_snapshot=true` may commit only those two generated state files to the selected branch after a complete audit; default `false` keeps the run fully read-only. The snapshot keeps live-readback counts separate from durable-only rows and live duplicate groups separate from registry-only conflicts. Public/scheduled/private state is evaluated at each route's API-audit timestamp so a later report build cannot relabel an earlier readback. It never substitutes for the fresh strict per-support control report required by apply.
...
GitHub ordinary bulk metadata workflow использует подтверждаемый chain `api,vectorengine`. Direct keys пробуются в порядке `GEMINI_API_KEY`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY`; если все прямые ключи дают recoverable ошибку, тот же полный batch один раз переходит к VectorEngine только при `confirm_vectorengine_metadata=USE_VECTORENGINE_METADATA`. Без подтверждения apply не запускается. Если полный chain не дал валидные metadata для каждого `requestId`, workflow падает до video render/TTS/upload. Template fallback разрешён только для plan diagnostics; apply остаётся fail-closed. Polyglot metadata использует тот же порядок провайдеров под существующим `confirm_metadata_spend=GENERATE_POLYGLOT_METADATA`.

2026-07-14 direct key-order clarification (supersedes the key-order sentence immediately above): primary is `GEMINI_API_KEY` or its legacy `GOOGLE_API_KEY` alias; secondary is `GEMINI_API_KEY_2`, used only after a quota/permission failure on primary; VectorEngine is the final explicitly confirmed fallback.

2026-07-14 direct Gemini batching policy: ordinary metadata groups up to 10 target-video tasks per direct request (`metadata_batch_size=10`), with sequential batches and `metadata_rate_limit_ms=15000` only between batches. Direct Google calls use `POST /v1beta/models/{model}:generateContent`, `responseMimeType=application/json` and a structured object whose `items` array has exactly one entry per stable `requestId`. One five-target channel therefore uses one request instead of five. Structured-output syntax is not semantic proof: every returned item still passes request-id completeness, support-language, metadata and SEO gates. A non-recoverable request/schema error stops immediately instead of consuming the second key or VectorEngine. Google Batch API is not used because its free tier is unavailable and its asynchronous turnaround is unnecessary for this workflow; this is one normal synchronous `generateContent` request containing several metadata tasks.
...
- `.github/workflows/youtube-polyglot-bulk-publish-dispatcher.yml` - launches many single Polyglot child runs without watching them; each child owns and persists its own publication/calendar state.
...
The dispatcher report and GitHub artifacts are operational evidence, not the durable source of truth. Durable state is the Polyglot registry/progress/playlist files above. The bulk parent must not try to recover child artifacts immediately after fire-and-forget dispatch. If a later bounded readback proves that a child uploaded a video but its own persist job failed or was canceled, the correct separately approved action is artifact state recovery, not a second upload.
...
Current rule (superseding the historical proxy below): custom-thumbnail permission and full-video permission are independent. `customThumbnailUploadAllowed=false` forbids custom-cover generation/upload only; it does not remove the support from full Polyglot planning. The workflow must use `generate_thumbnails=false`, automatic YouTube thumbnail fallback and the normal render-duration/upload preflight. A duration or YouTube account-limit failure stops that support after render and is reported without retry.

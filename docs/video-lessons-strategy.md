...
2026-07-14 direct Gemini batching policy: standalone ordinary metadata groups up to 10 target-video tasks per direct request (`metadata_batch_size=10`). The mixed 51-channel campaign goes further: `.github/workflows/youtube-publication-campaign.yml` groups ordinary and Polyglot tasks by the four GitHub route environments, then sends up to 10 stable `requestId` tasks in each synchronous request before any render/TTS starts. A standard 306-video `5 ordinary + 1 Polyglot` wave therefore uses 32 route-batched requests instead of 102 worker requests. Direct Google calls use `POST /v1beta/models/{model}:generateContent`, JSON response schema and exact request-id completeness. Structured output is not semantic proof: every item still passes support-language, metadata and SEO gates. A non-recoverable request/schema error stops before render; Google Batch API is not used.
...
Full-scope Polyglot planning includes all 51 canonical physical support/native channels. Custom-thumbnail permission never removes a channel from that candidate set. After render, videos at or below `895s` may proceed on every channel; a longer video proceeds only when `longVideoUploadAllowed=true`. Unknown or false long-video capability blocks only that over-limit rendered video, with no retry or replacement dispatch.
...
For every other support channel, `customThumbnailUploadAllowed=false` or an unknown value disables custom-cover creation/upload only. It does not skip full-scope Polyglot by itself. The workflow uses YouTube automatic thumbnail fallback and must not call `thumbnails.set`; full-video eligibility is decided separately by the rendered duration and `longVideoUploadAllowed` capability gate.

...
- Thumbnail-only source-artifact diagnostics must consume the full `find | sort` stream before limiting displayed paths; a `head` consumer under `pipefail` can terminate `sort` with `Broken pipe` after a successful artifact download and falsely block the write stage. Exact multi-video repairs use `.github/workflows/youtube-thumbnail-batch-set.yml` with a committed manifest containing exact video IDs, tracked JPGs and `thumbnailSet=false`; an omitted `set_id` is allowed only when every row is already pinned by that exact manifest. Batch runs are serialized per branch through the workflow-level concurrency group. Do not move that serialization to identical pending persist jobs: GitHub keeps only one pending job per concurrency group and cancels older pending jobs even when `cancel-in-progress=false`, which can leave successful `thumbnails.set` calls without durable registry persistence.
...
- 2026-08-15 Deck #5 cover preparation: `home_bathroom_essentials_a1` / Sort 5 `Bathroom Essentials` uses the reviewed `assets/youtube-cover-templates/deck5-bathroom-approved-base.png` and the new `config/youtube-cover-templates.json` entry. `npm run build:youtube-cover-assets -- --set=home_bathroom_essentials_a1` produced `911` deterministic video JPGs (`847` ordinary + `64` full Polyglot) in `data/youtube-cover-assets/deck5-bathroom-approved-20260815/` for the `16` channels with `customThumbnailUploadAllowed=true`; every video asset is 1280x720 and the package has no skipped rows. Playlist covers are a separate shared language-pair asset, not one new playlist per deck: the existing universal square base and CoreText renderer produced `799` 1024x1024 JPGs in `data/youtube-playlist-covers/deck5-bathroom-shared-playlists-20260815/` (`790` durable IDs plus `9` local-only rows). Course Metadata and the full card matrix are exported from the local DB for all 54 language roots; every available support/target pair contains the same 35 deck cards. No provider or YouTube write occurred. Before any GitHub campaign, the exact manifests and deck JSON must be Git-tracked and pass the normal thumbnail/playlist-image gates.
- 2026-08-26 Deck #7 cover preparation: `home_living_room_basics_a1` / Sort 7 `Living Room Basics` uses the user-approved Variant 1 watercolor base `assets/youtube-cover-templates/deck7-living-room-illustrated-approved-base.png`. The deterministic renderer produced `909` horizontal video JPGs in `data/youtube-cover-assets/deck7-living-room-approved-20260826/`: `845` ordinary and `64` full Polyglot rows across the `16` channels whose current config has `customThumbnailUploadAllowed=true`, with `0` skipped rows. The local source payload contains the full generated/readback-verified 30-card x 54-language deck and was exported with `--local-only`; the build made `0` image-provider calls and `0` YouTube writes. Playlist images remain shared language-pair assets and require a separate exact plan/readback before any replacement; publication still requires a fresh complete all-route control and an immutable campaign claim.
- 2026-08-31 Deck #8 cover preparation: `home_dining_room_table_setup_a1_a2` / Sort 8 `Dining Room & Table Setup` uses the user-approved watercolor base `assets/youtube-cover-templates/deck8-dining-room-illustrated-approved-base.png`, visually aligned with Decks #6 and #7. The deterministic renderer produced `909` horizontal JPGs in `data/youtube-cover-assets/deck8-dining-room-approved-20260831/`: `845` ordinary and `64` full Polyglot rows across the `16` channels whose current config has `customThumbnailUploadAllowed=true`; all are 1280x720, below 2 MB, and the manifest has `0` skipped rows. The `--local-only` payload exports the current 40-card x 54-language database matrix without Drive access. Shared language-pair playlist covers are reused rather than duplicated per deck. The build made `0` provider calls and `0` YouTube writes; publication still requires a fresh complete all-route control, immutable campaign claim and separate apply approval.
...
- YouTube video metadata must treat Course Metadata as canonical for the deck/topic name. `scripts/lib/video-generator.mjs` resolves metadata in this order: current Docker/Postgres `content_set_localizations` when available, then the GitHub offline deck JSON `courseMetadata`, then legacy offline `titles`, then English Course Metadata and only finally internal `content_sets` fallback. Generated `youtube_metadata.json` includes `deckMetadataSource` so a run artifact shows whether the title came from DB/offline Course Metadata or from a fallback. GitHub also requires a non-empty offline card array for every ordinary support/target pair and every target inside a selected Polyglot assignment; campaign planning validates that complete assignment matrix before claim, and runner generation fails closed instead of attempting local Postgres. After editing Course Metadata or cards in Docker/Postgres or Google Sheets, refresh the offline deck JSON with `scripts/export-and-upload-deck.mjs <set-id> --local-only` before the next campaign, review it and commit the exact JSON; GitHub does not start Docker/Postgres and cannot see local DB edits.
...
- 2026-07-08 second-deck offline-data requirement: the first route-2/3/4 apply dispatch for `home_kitchen_cooking_actions_a1_a2` failed before YouTube upload because clean GitHub runners do not run the local Postgres database at `127.0.0.1:55433`. The compatibility command `scripts/export-deck-data.mjs <set-id>` delegates to the canonical `scripts/export-and-upload-deck.mjs <set-id> --local-only` exporter, so both paths produce one complete all-language JSON shape with `courseMetadata`, `titles`, `descriptions`, `levelSignals` and all support/target card combinations. A fixed support-language allowlist is forbidden. Commit the exact `data/decks/<set-id>.json` before GitHub rendering; a dry-run with an unavailable `DATABASE_URL` must still resolve every selected assignment from the offline JSON.
...
- Because render/TTS can outlive the 3-hour evidence window, apply performs one second read-only live audit immediately before the first YouTube write. The uploader refuses a missing, stale, non-strict or unhealthy control report; do not bypass this with direct local apply.
...
| **SL** | Словенский | `edge_sl-SI-RokNeural` | 2026-08-16 primary after empty-audio incident; single-voice contract |
...
| **SK** | Словацкий | `edge_sk-SK-LukasNeural` | 2026-08-12 primary after empty-audio incident; single-voice contract |
...
2026-08-16 Slovenian production repair: Deck #5 parent run `31899114043` received repeated zero-byte audio from `sl-SI-PetraNeural` and stopped all ten `SL` ordinary renders before YouTube upload. The tracked `SL` primary is therefore restored to the same-locale `edge_sl-SI-RokNeural` single-voice contract. A bounded live smoke produced a non-empty `14,976`-byte MP3 with measured duration `2.496s`. Do not add Petra as a hidden per-utterance fallback: that would spend four failed attempts per segment and could mix speakers inside one lesson.
...
Short rule: `content_scope=short_unverified` is used either by an explicit deliberately truncated request or by the recorded pre-claim fallback for a support without long-video permission. It stays under YouTube's default 15-minute limit for accounts/channels that may not have advanced features. Historical first-deck short rows remain valid publication history and do not satisfy the full scope for a future upgrade, but an active short is already the operational fallback for that Polyglot product slot. Publication control therefore removes that pair from actionable missing tails and records it separately as `fallbackCoveredPolyglotTails`; a full upgrade remains explicit and blocked while the short is active. All future short campaigns use `shortUnverifiedPolyglotCardLimit=0`, which measures the full candidate deck and selects the largest contiguous prefix that fits `max_duration_seconds=895` (`14:55`). The workflow runs `npm run check:polyglot-video-duration` after rendering as a defence-in-depth gate: legacy/manual full claims on unconfirmed channels are blocked above `895s`, confirmed channels may exceed it, and short videos are always blocked above the cap. Short videos use separate progress/playlist keys, but an active full row is a hard preflight blocker for a short and an active short is a hard preflight blocker for a proposed full. Short unverified uploads use YouTube automatic thumbnail fallback and must not call `thumbnails.set`.

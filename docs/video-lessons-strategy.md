...
GitHub boundary for YouTube access: do not upload OAuth token JSON files, Google OAuth client secret JSON, `.local` contents or contact-email defaults to the repository. If GitHub Actions later needs direct YouTube upload/publish access, convert the needed per-channel credentials into encrypted GitHub Actions secrets or environment secrets only after explicit approval, document the exact secret names and scope, and keep repo files limited to non-secret channel IDs, handles, support-language mappings, banner paths and metadata templates.

...
- 2026-06-23 slow route-scoped retry update: the next safe layers deliberately used one target per GitHub run and only a few support routes at a time. They uploaded/scheduled and persisted `ZH/DE`, `RO/EN`, `SR/EN`, `ZH/EN`, `RO/ES`, `SR/ES`, `ZH/ES`, `RO/FR`, `SR/FR`, `ZH/FR`, `RO/IT`, `SR/IT`, `UZ/RU`, `ZH/IT`, `RO/PT`, `SR/PT`, `AZ/PT`, `ZH/PT`, `SR/RU`, `AZ/RU`, `AZ/ZH`, `JA/FR`, `LT/IT` and `KN/EN`; `RO/RU` also uploaded successfully and was recovered from its non-secret artifact after a persist-push failure. `TE/EN` failed before upload on a VectorEngine image-generation timeout and must be retried or configured for accepted automatic-thumbnail fallback before spending a YouTube upload. The current missing set is 36 targets. Larger batches are possible only as multiple separate one-video `workflow_dispatch` runs, not as `worker_count=10` inside one support channel. Recommended next layer is 6-8 runs; a 10-run batch is acceptable after one clean layer if it is split across route buckets, keeps `concurrency=1`, `metadata_concurrency=1`, `thumbnail_concurrency=1`, and stops immediately on the first YouTube quota/rate, thumbnail, or persist-push systemic error. Do not broaden this back into a 50-run wave unless route quota has been checked or reset and the stop rule remains active.

...
- **2026-07-05 thumbnail lookup priority for ordinary videos** (implemented in `scripts/copy-pre-rendered-thumbnails.mjs`, `scripts/youtube-set-video-thumbnail.mjs`, `scripts/generate-youtube-thumbnails.mjs`): lookup checks two locations in order. **First**: `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cookware_pilot_01-ordinary-target-language-large-pair-folders-20260704/by-support/<SUPPORT>__<SupportName>/<SUPPORT>__<TARGET>__<SupportName>_to_<TargetName>/youtube_thumbnail.jpg`. This folder contains **individual ordinary covers** for each support×target pair where the target language is taught as a standalone ordinary video on that support channel. **Second** (fallback for missing pairs only): `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cookware_pilot_01-ordinary-target-language-large-pair-folders-20260704-scheduled-only-20260705/by-support/…` — same structure, scheduled-wave subset only. **Critical caveat**: if a target language (e.g. SK, SL, SR on SW channel, or TE, TH, TL on NE channel) was not published as an *ordinary standalone* video on that support channel — only as part of a polyglot bundle — then the `by-support/<SUPPORT>/` folder for that pair **does not exist**. The covers for those pairs exist on *another* channel's `by-support/` folder (e.g. `NE__Nepali/NE__SK__…/youtube_thumbnail.jpg`) but cannot be reused because the text is localized for a different viewer language. In this case new ordinary covers must be rendered for the correct support channel before publishing.

...
- 2026-07-14 metadata batching correction: bulk ordinary dispatch uses `metadata_gemini_backend=api,vectorengine`, sequential metadata batches, `metadata_batch_size=10` and `metadata_rate_limit_ms=15000`. One structured `generateContent` request returns metadata for up to 10 target videos. Direct order is primary `GEMINI_API_KEY` (or legacy `GOOGLE_API_KEY` alias), then `GEMINI_API_KEY_2` for quota/permission, response-integrity, timeout, network or HTTP 5xx failure, then one explicitly confirmed VectorEngine call for the same whole batch. Response-integrity includes non-`STOP` finish reason, malformed/truncated JSON and an incomplete exact request-ID set. Ordinary and mixed-campaign batch requests use `maxOutputTokens=60000` under the `gemini-3.5-flash` model limit `65536`; large structured responses receive `600000 ms`, while prompt-level copy limits prevent intentional verbosity. `api,vectorengine` can never fall through to Gemini CLI; no provider receives a blind same-key retry. Campaign batches write atomic checkpoints and upload route artifacts even after later failure. A separately approved recovery can reuse exact validated completed batches through `metadata_resume_run_id`; no retry or dispatch is automatic. Polyglot uses the same provider order for its single metadata task. Apply remains fail-closed before render/TTS/upload if the chain or per-item language/SEO gates fail.

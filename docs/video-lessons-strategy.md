...
Polyglot bulk dispatch is also separate from the ordinary bulk dispatcher. `.github/workflows/youtube-polyglot-bulk-publish-dispatcher.yml` and `npm run dispatch:youtube-polyglot-bulk-publish` plan or launch one Polyglot child workflow per support channel through `.github/workflows/youtube-polyglot-video-publish.yml`; they must not call the ordinary `.github/workflows/youtube-video-publish.yml`. The dispatcher is dry-run by default, uses `support_source=channel-keys` by default for one video per physical channel, defaults to `max_parallel=4` for GitHub API stability, and writes `outputs/youtube-polyglot-bulk-publish-dispatcher-report.json`. For physical-channel rollout, `supports=ALL` includes the configured channel keys across all routes, including `HY` on `youtube-4`; use `supports=route:youtube-4` or explicit `supports=HY` for a narrower Armenian/route test. For `support=EN` and `support=EN-GB`, the dispatcher uses `english_bundle=global_europe_core`; the planner removes English from the visible target set and fills with fallback `IT`, yielding `ES,FR,DE,IT`. Live dispatch requires explicit confirmations for dispatch, render, TTS, AI metadata, YouTube writes, public visibility and optional thumbnail spend.

If a child Polyglot apply fails with a playlist-classified transient error, the bulk dispatcher must not rerun the full video upload. With `confirm_playlist_repair=APPLY_YOUTUBE_PLAYLIST_INSERT`, it waits `playlist_retry_delay_seconds` and dispatches `.github/workflows/youtube-polyglot-playlist-insert-repair.yml`, which calls `playlistItems.insert` against `config/youtube-polyglot-published-videos.json` / `config/youtube-polyglot-playlists.json` by `polyglotKey`. Repair results are recorded in the dispatcher report and persisted by the repair workflow; video reupload remains blocked unless `allow_republish` is intentionally set in a separate decision.

2026-06-29 bulk recovery rule: a child run where `youtube-polyglot-video` succeeded but `persist-polyglot-publish-state` was cancelled/failed is an uploaded-orphan state gap, not a missing video. The dispatcher must mark it as `state_persist_failed` / `needsStateRecovery` and the parent workflow must run `npm run recover:youtube-polyglot-state` to download child artifacts and merge Polyglot publication/progress/playlist state before any rerun. Do not rerender or reupload those supports until `config/youtube-polyglot-published-videos.json` has been reconciled. The dispatcher also retries transient GitHub CLI/API failures, gives planner subprocesses a bounded timeout, and treats `edge-tts` `NoAudioReceived` as a transient TTS failure instead of a route-stopping publication failure.

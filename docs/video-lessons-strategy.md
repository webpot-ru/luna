...
- API-shape guard: `playlists.list` readback can request `fields=items(...)`, but `playlists.update` returns a single playlist resource and must request fields like `id,snippet(...),status(...)` without `items(...)`. Run `node --check scripts/youtube-repair-playlist-metadata.mjs`, a read-only `npm run repair:youtube-playlist-metadata -- --route=<route>`, and `npm run check:youtube-playlist-naming` before retrying an apply batch after script changes;

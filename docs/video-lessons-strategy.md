...
Until that HY create-speech path is green, YouTube publishing must exclude Armenian both as support and target language. The publish workflow input `exclude_langs` defaults to `HY`, and `scripts/plan-youtube-generation-targets.mjs` supports `--exclude-targets` / `--exclude-langs`; the preflight report must show excluded HY pairs as `skippedExcludedTargetCount`, not render/upload attempts. Do not override with `exclude_langs=NONE` for bulk runs until HY TTS write/readback is fixed.

...
- 2026-07-10 metadata backend correction: bulk ordinary dispatch plans `metadata_gemini_backend=api,vectorengine`, `metadata_concurrency=1`, `metadata_batch_size=5` and `metadata_rate_limit_ms=15000`. Each five-target direct batch tries `GEMINI_API_KEY`, then `GEMINI_API_KEY_2`; only after both fail does the same complete five-target batch make one confirmed VectorEngine request. VectorEngine no longer receives one request per target. Both providers must return exactly one unique item for every requested `targetLang`; missing, duplicate or extra items fail the whole channel batch in apply mode. Apply refuses the chain without `confirm_vectorengine_metadata=USE_VECTORENGINE_METADATA`. Metadata completes before render/TTS/upload.
...
2026-07-10 Gemini batching policy: ordinary metadata groups five targets from one support channel per request, with `metadata_concurrency=1` and `metadata_rate_limit_ms=15000`. Direct Google calls use `POST /v1beta/models/{model}:generateContent`, `responseMimeType=application/json`, `thinkingLevel=minimal`, a balanced-JSON parser and a 120-second timeout. `responseSchema` is intentionally omitted: local isolation showed schema requests hanging while JSON MIME without schema reached the same valid key/model. The prompt still requires the exact object shape and downstream metadata/language/SEO gates remain fail-closed. If the direct batch fails after both keys, confirmed VectorEngine fallback receives the same five targets in one compact batch request. A complete exact target set is mandatory; the pipeline does not split or retry an incomplete VectorEngine response as frequent per-target calls. AI titles shorter than the SEO minimum use the already-localized deterministic template title and record `aiMetadata.titleFallback`, preventing a short generated title such as the earlier `ZH -> BN` result from reaching the upload gate.

...
2026-07-03 partial `slavic_core` wave for `home_kitchen_cookware_pilot_01`: durable state has 46/51 scheduled uploaded Polyglot rows, with 6/9 full thumbnail-enabled supports (`EN`, `ES`, `PT`, `TH`, `NE`, `SW`) and 40/42 `short_unverified` supports (`limit=30`, `max_duration_seconds=895`, automatic first-frame thumbnail). `slavic_core` targets are `RU,PL,CS,SK`; support collisions resolve with `BG`, for example `support=RU -> PL,CS,SK,BG` and `support=SK -> RU,PL,CS,BG`. Operational recovery notes: 13 successful uploads with cancelled workflow/persist were recovered from artifacts without reupload in commit `280e06f`; final transient TTS retries closed `KN` (`28660612771`, video `am7EUIJszpw`) and `KA` (`28661072913`, video `b-EPMmY3ejI`). Remaining blockers are `RU`, `FR`, `JA`, `TR` with route-1 `quotaExceeded` before upload plus `DE`, which hit `edge-tts NoAudioReceived` before upload but also belongs to route 1. Do not retry those five until route-1 quota reset/readback; never reupload any of the 46 durable rows. `config/youtube-publish-calendar.json` has been repaired from durable `slavic_core` state with 46 Polyglot reservations. That repair exposes four already-real same-channel same-`publishAt` conflicts with ordinary scheduled videos: `PT` at `2026-07-04T13:30:00.000Z`, `EN` at `2026-07-04T18:30:00.000Z`, `ET` at `2026-07-04T08:30:00.000Z`, and `KA` at `2026-07-04T07:30:00.000Z`; resolve only through deliberate schedule/visibility repair, not reupload. Current gate: `npm run check:youtube-polyglot-state -- --set=home_kitchen_cookware_pilot_01 --bundle=slavic_core --json`.

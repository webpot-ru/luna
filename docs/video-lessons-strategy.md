...
- As of 2026-07-16, `longVideoUploadAllowed` is a separate per-channel capability in the same registry. Its current explicit true allowlist contains 16 channels: `en`, `ru`, `es`, `pt`, `ja`, `tr`, `vi`, `th`, `sr`, `my`, `ne`, `si`, `uz`, `ka`, `sw` and `zh`. This list is intentionally different from the 15-channel custom-thumbnail allowlist above: the user confirmed ZH for videos longer than 15 minutes, while `ZH.customThumbnailUploadAllowed` remains `false` because no successful `thumbnails.set` evidence was provided. Future changes must update these fields independently: custom-thumbnail permission controls only `thumbnails.set`; long-video permission allows a full Polyglot upload to exceed `895` seconds. With a false/unknown long-video capability, the requested full product remains full but is upload-eligible only after the measured render is at most `895` seconds (`14:55`). `shortUnverifiedPolyglotCardLimit=0` applies only to an explicitly requested `short_unverified` product: the renderer measures the whole candidate deck, retains the maximum contiguous prefix that fits and writes `polyglot-duration-selection.json`. The same cache is reused in final render, so this is not a second TTS pass. The short item never marks the required full Polyglot bundle as complete, and a full/short pair for the same support channel + bundle is a preflight blocker.
...
Full-scope Polyglot planning includes every canonical support/native channel unless an explicit `short_unverified` product was requested. Custom-thumbnail permission never removes a channel from that candidate set. A false or unknown `longVideoUploadAllowed` value is not a reason to change `contentScope`: the full render is allowed to proceed only when its measured duration is at most `895s` (`14:55`); a longer full render is stopped before upload, reported without retry, and does not become a short automatically.
...
For every other support channel, `customThumbnailUploadAllowed=false` or an unknown value disables custom-cover creation/upload only. It does not itself decide Polyglot scope. The workflow uses YouTube automatic thumbnail fallback and must not call `thumbnails.set`. `longVideoUploadAllowed=true` permits full videos longer than `895s`; false/unknown allows the requested full product only up to the measured `895s` duration cap.

Short rule: `content_scope=short_unverified` is used only for an explicitly requested, deliberately truncated Polyglot product. It stays under YouTube's default 15-minute limit for accounts/channels that may not have advanced features. Historical first-deck short rows remain valid publication history but do not satisfy a full slot. All future short campaigns use `shortUnverifiedPolyglotCardLimit=0`, which measures the full candidate deck and selects the largest contiguous prefix that fits `max_duration_seconds=895` (`14:55`). The workflow runs `npm run check:polyglot-video-duration` after rendering: full videos on unconfirmed channels are blocked above `895s`, while confirmed channels may exceed it; short videos are always blocked above the cap. A failed full duration gate is not retried or converted into a short product. Short videos use separate progress/playlist keys, but an active full row is a hard preflight blocker for a short and an active short is a hard preflight blocker for a full. Short unverified uploads use YouTube automatic thumbnail fallback and must not call `thumbnails.set`.

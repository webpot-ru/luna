...
Norwegian/Bokmål is one language/product contour in this project, not separate `NO` and `NB` products. `NO` is the usual public/support/channel code, `NB` is the internal DB/data code, and both point to the public site/study code `no`. Video descriptions, QR links and `courseUrl` must use `?langs=no`, not `?langs=nb`, while playlist/data identities can still use their documented internal codes.
...
- **2026-07-05 thumbnail source rule: all ordinary covers for `home_kitchen_cookware_pilot_01` are fully pre-rendered as deterministic local JPEGs and committed to the repository. VectorEngine / GPT Image generation is permanently disabled for this deck. Do not call `generate:youtube-thumbnails` with `--confirm-spend` for this deck. Do not set `generate_thumbnails=true` in GitHub workflows for this deck.** The correct pipeline is: (1) video-generator creates `outputs/video-generator/<pair>/youtube_metadata.json`; (2) run `node scripts/copy-pre-rendered-thumbnails.mjs` locally or in GitHub before publish; (3) the script copies the matching pre-rendered JPEG into `outputs/video-generator/<pair>/youtube_thumbnail.jpg` and sets `thumbnailUploadMode=custom` in metadata; (4) the publish workflow uploads it as a custom thumbnail. The script is fail-closed: if it cannot find a matching pre-rendered file it throws an error rather than silently falling back to VectorEngine or first-frame auto. All support×target pairs — including SW→SK, SW→SL, SW→SR, NE→TE, NE→TH, NE→TL — have committed covers in `by-support/` in branch `codex/norwegian-course-url-repair-48`; if a pair appears missing, check that the correct branch is checked out before concluding any cover is absent.
- 2026-07-07 deck #1 thumbnail canonical refresh supersedes the older 2026-07-03 / 2026-07-04 local proof folders for `home_kitchen_cookware_pilot_01`; those old proof folders were moved with `scripts/move-to-trash.sh`, not permanently deleted. The current local deterministic outputs are exactly two design-prototype roots: ordinary approved-channel pair covers at `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cookware_pilot_01-approved-channel-pairs-target-language-first-20260707/` and full published Polyglot covers for approved channels at `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cookware_pilot_01-approved-polyglot-target-languages-20260707/`. Ordinary covers render `686` JPGs for the 13 approved physical custom-thumbnail channels / 16 support variants, with learned target language plus A1 as the largest headline and localized Course Metadata title/description below. Polyglot covers render `46` full published rows only, excluding `short_unverified`, with visible target-language bundles as the largest text block and Course Metadata topic/description below. Both batches are 1280 x 720 JPGs under 2 MB, use only the accepted bitmap/imagegen composition references plus deterministic local `sharp` overlays, use no country flags as the primary language signal, and perform no YouTube upload, VectorEngine, GPT Image or paid image generation.
- 2026-07-07 GitHub thumbnail wiring: `scripts/generate-youtube-thumbnails.mjs` checks the two canonical `20260707` deck #1 manifests before legacy `20260704` prototype folders. If a matching committed JPG exists, it copies that file next to `youtube_metadata.json`, records `thumbnailSource=pre-rendered-design-prototype`, and does not call VectorEngine. Local smoke test `outputs/tmp/youtube-thumbnail-lookup-smoke-20260707/` verified the lookup for `NE -> TR` ordinary and `KM` full Polyglot `global_europe_core`. Because `outputs/` is ignored by default, a GitHub publish run can use these covers only after the needed manifest(s) and selected JPGs are force-added and pushed.
- 2026-07-07 design-prototype cleanup: `outputs/design-prototypes/` is intentionally reduced to the current deck #1 output roots above, `outputs/design-prototypes/youtube-thumbnail-imagegen-references-20260703/` for bitmap bases, `outputs/design-prototypes/youtube-thumbnail-test-covers-imagegen-style-v3-large-text-clean-polyglot-20260703/` as the accepted v3 style reference, and the deck #2 Polyglot background candidate pack below. Earlier `food-menu-*`, `template-*`, `imagegen-style-v1/v2`, `next-10-decks-*`, `kitchenware-polyglot-ru-v3-*` and 2026-07-04 deck #1 proof folders are historical only and should not be used by new chats unless restored deliberately from project Trash. If GitHub Actions must upload these thumbnails later, the final selected `20260707` image directories need to be explicitly added to git despite output ignore rules; do not add the whole `outputs/design-prototypes/` tree blindly.
- 2026-07-07 deck #2 `Cooking Actions` / `home_kitchen_cooking_actions_a1_a2` Polyglot background candidates are fixed locally at `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cooking_actions_a1_a2-polyglot-background-candidates-20260707/`. This package intentionally retains only variants `B`, `C` and `D`, with `D` (`variant-d-clean-universal-recommended.png`) as the current recommendation. Earlier generated ordinary, first-Polyglot and variant-A concepts from the temporary Codex generated-images folder were not copied into the project package and should be ignored. The first deterministic text overlay batch for deck #2 ordinary approved-channel covers exists at `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cooking_actions_a1_a2-approved-channel-pairs-target-language-first-20260707/`: `686` local JPGs, `1280 x 720`, under `2 MB`, `13` approved physical custom-thumbnail channels / `16` support variants, target-language-first headline, localized Course Metadata from `content_set_localizations`, no YouTube upload, no VectorEngine, no GPT Image and no paid image generation. Manifest: `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cooking_actions_a1_a2-approved-channel-pairs-target-language-first-20260707/manifest.json`; contact sheet: `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cooking_actions_a1_a2-approved-channel-pairs-target-language-first-20260707/contact-sheet.jpg`.
- 2026-07-07 deck #2 Polyglot planned Wave 1 cover batch exists at `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cooking_actions_a1_a2-approved-polyglot-wave1-planned-20260707/`: `80` local JPGs, `1280 x 720`, under `2 MB`, `16` approved support variants x `5` Wave 1 bundles (`global_europe_core`, `romance_core`, `east_asia_core`, `slavic_core`, `southeast_asia_core`). Target-language names are the largest text, localized Course Metadata comes from `content_set_localizations`, and support self-languages are replaced from each bundle's `fallbackLangs`. This is planned/local proof only because `config/youtube-polyglot-published-videos.json` has `0` rows for deck #2; after actual Polyglot uploads/schedules, upload-linked covers should be regenerated from registry rows. Manifest: `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cooking_actions_a1_a2-approved-polyglot-wave1-planned-20260707/manifest.json`; contact sheet: `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cooking_actions_a1_a2-approved-polyglot-wave1-planned-20260707/contact-sheet.jpg`. No YouTube upload, no VectorEngine, no GPT Image and no paid image generation were performed.
...
- Shared viewer channels use explicit support-variant priority from `config/youtube-publish-schedule-policy.json`. [2026-07-05 Decision]: To minimize video rendering load, TTS voice costs, and YouTube API quota limits, we will ONLY use a single primary variant of paired dialects as **Support Languages** (native viewer channels): US English (EN) on the `en` channel, Latin American Spanish (ES-419) on the `es` channel, and Brazilian Portuguese (PT-BR) on the `pt` channel. The secondary support variants (British English `EN-GB`, Spain Spanish `ES`, and European Portuguese `PT`) are completely excluded as support languages. However, as **Target Languages** (languages being studied), **both dialect variants are preserved** (e.g., general English `EN` and British English `EN-GB` are both generated and published as target languages for other channels), as learners may specifically want to study a particular accent or dialect.
...
## 4.2. YouTube Shorts Local Render Contour

2026-07-07 в LUNA2 добавлен локальный portrait-render контур для коротких вертикальных видео YouTube Shorts. Это не заменяет обычные горизонтальные видео и не является live upload pipeline.

Source of truth files:

```text
scripts/build-deck-shorts.mjs
...
```bash
npm run build:deck-shorts -- --set home_bathroom_essentials_a1 --target ES --support RU --card-limit 5 --quiz-limit 2 --transition flip
```

Local runtime requirements:

- `edge-tts` must be available either on `PATH` or at `.venv-source-tools/bin/edge-tts`; this matches the existing ordinary video `getTtsAudio()` helper. On 2026-07-07 the local proof machine used a gitignored `.venv-source-tools` runtime with `edge-tts 7.2.8`.
- `scripts/lib/screenshot-batch.mjs` normally imports the project `playwright` package. If local `node_modules/playwright-core` hangs or is unhealthy, set `PLAYWRIGHT_MODULE_PATH=/absolute/path/to/playwright` to use an alternate installed Playwright package for the screenshot step. This is a local runtime override, not a production dependency.

Контракт текущей версии:

- output directory: `outputs/shorts-generator/<set>_<target>_<support>/`;
- MP4 filename: `shorts_<target>_<support>.mp4`;
- resolution: `1080x1920`, `25 fps`, `yuv420p`;
- default learning slice: first `5` cards;
- default quiz slice: first `2` cards, deterministic, no random shuffle;
...
- no intro and no QR code in outro;
- outro CTA points the viewer to `flashcardsluna.com` / channel profile rather than relying on clickable description links;
- `scripts/lib/shorts-outro-translations.mjs` owns the current Shorts-only outro copy. It returns naturalized title/subtitle/notice/audio for all 54 language contours and also audits the `NB` technical key as an alias of `NO`. `NO` and `NB` must stay the same Norwegian Bokmål contour with identical Shorts copy. Validate it with `npm run check:shorts-outro-localization`. Ordinary horizontal videos still use `config/video-localization.json`; do not copy the Shorts "channel profile" CTA into the ordinary QR/description outro unless that separate workflow is intentionally changed.
...
2026-07-07 web/source scan and product decision: Shorts should optimize for a repeatable micro-challenge, fast comprehension and replay/comment behavior, not for full lesson completeness. The current source signals:

- YouTube official Shorts discovery guidance says Shorts are ranked by viewer personalization and performance, including chose-to-view, average view duration, average percentage viewed, likes and survey feedback. Topic interest, competition, seasonality, metadata/search match, sounds and hashtags can also affect discovery: `https://support.google.com/youtube/answer/11914225?co=YOUTUBE._YTVideoType%3Dshorts`.
- YouTube official editing guidance explicitly recommends timed on-screen text, voiceover and timeline control to keep viewers oriented through fast edits: `https://support.google.com/youtube/answer/13380879`.
- YouTube's Shorts creator guidance frames the format as 3 minutes or less, broadly shareable across homepage/Shorts/channel surfaces, and recommends universal appeal plus simple viewer engagement prompts: `https://support.google.com/youtube/answer/13363900`.
- TikTok Creative Center examples emphasize native-feeling feed pacing, curiosity-first hooks, scenario-driven framing and localized/translated creative as reusable short-form patterns: `https://ads.tiktok.com/creative/creativeCenter`.
- Public language-learning creator patterns worth borrowing are the structure, not the persona: Duolingo's TikTok success came from fun/trending-audio/community behavior, while later public reporting shows the risk of over-optimizing for chaotic brand behavior. Easy Languages shows authentic speech/subtitle patterns; Xiaomanyc/Laoshu-style videos show that "speak the viewer's language and trigger recognition" is a strong polyglot hook. LunaCards should keep a clean educational brand and use these as format signals, not copy their character or tone.

Ordinary single-language Shorts format candidates. These use exactly one studied target language and one support/viewer language, usually `15-45s`, `3-5` learning cards plus `1-2` quiz prompts:

- `word_quiz_3_2_1`: viewer-language prompt, short pause/countdown, target reveal, TTS, one compact example. This is the current `build-deck-shorts.mjs` default direction.
- `pronunciation_trap`: "Do not pronounce it like <support-language intuition>" plus target TTS repeat. Use only when transcription/voice evidence is present; do not invent phonology claims.
- `common_mistake_fix`: "Not X, say Y" for high-confidence wording traps. Use only from approved deck data or reviewed language notes, never from ad hoc AI grammar guesses.
- `survival_three_words`: three practical words in one scene, for example taxi, restaurant, grocery, directions, pharmacy or hotel check-in.
- `visual_object_flash`: object/card appears first, target word and audio follow, then one short example. Best for concrete nouns and visual everyday decks.
- `mini_story_scene`: one micro-situation using three words from the same deck, then a final one-question recall.
- `odd_choice_quiz`: three answer choices, one correct target word, with a comment hook after reveal.
- `script_reading_hook`: for non-Latin scripts, show native script, project transcription/romanization field and TTS. Do not generate fake transliteration when the data does not provide it.
- `speed_review_loop`: five fast cards arranged so the final frame loops naturally into the first frame. Use for retention/replay tests.
- `comment_next_word`: ask viewers which word/scene to cover next. Source decks, QA and wording still stay DB/spec driven.

Polyglot Shorts are a separate product mode, not ordinary Shorts with more target languages and not `short_unverified` long-video fallback. Use `videoType=polyglot_shorts` once registry/upload support exists. They should use `config/polyglot-video-bundles.json`, keep `3-5` target languages maximum, use `1-3` words only, and target `15-35s`.

Polyglot Shorts format candidates:

- `guess_the_language`: one spoken word/example, viewers guess the language before reveal.
- `one_word_5_languages`: same meaning in a compact approved bundle, with rapid TTS and script contrast.
- `odd_one_out`: four related languages plus one outlier, viewers guess the outlier.
- `false_friend_trap`: visually/sound-similar words with different meanings. Requires reviewed evidence; no invented false friends.
- `same_root_chain`: cognate/root chain across a language family, only where deck data supports it.
- `script_shock`: same support-language meaning across scripts, for example Latin/Cyrillic/Greek/Arabic/Devanagari/Thai/Khmer depending on bundle.
- `travel_survival_pack`: one survival phrase/word across a practical travel bundle.
- `no_english_challenge`: for non-English support channels, avoid English as the anchor and use the support viewer language as the only explanation layer.

Pilot plan before building a bulk Shorts registry:
...
- `langs` использует public study-code mapping из `config/video-public-course-links.json`, а не обязательно сырой internal/data code. Для Norwegian/Bokmål `NO` и `NB` являются одним языковым контуром и оба должны давать `langs=no`; `langs=nb` считается ошибкой.
...
2026-07-07 route-3/4 recovery decision: old full `global_europe_core` Polyglot rows for `BN`, `ET`, `HR`, `IS`, `BG`, `KM`, `LT`, `LO`, `LV`, `MY`, `AZ`, `HY`, `IT`, `KA`, `KK`, `KN` and `ML` were marked `missing_video_public_readback_superseded` after public oEmbed returned `404` and route live-upload reconciliation did not prove the old video ids. Their `short_unverified` rows remain active/current. Do not reupload all 17 as full Polyglot: among this recovery set, local full-scope planning is ready only for `KM` and `MY`; non-custom channels remain blocked for full Polyglot and are covered by `short_unverified` unless/until their custom-thumbnail/advanced features are verified.
...
Before launching `romance_core` for the first-deck eligible-channel wave, resolve the fallback policy explicitly. The current config has `romance_core.fallbackLangs = ["EN", "DE"]`, so planner output is `support=ES -> FR,IT,PT,EN` and `support=PT -> ES,FR,IT,EN`. If same-family fallback is required for this bundle, update `config/polyglot-video-bundles.json` with an approved fallback policy before dispatching live workflows.

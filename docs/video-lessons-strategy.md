...
- Shared viewer channels use explicit support-variant priority from `config/youtube-publish-schedule-policy.json`. [2026-07-05 Decision]: To minimize video rendering load, TTS voice costs, and YouTube API quota limits, we will ONLY publish a single primary variant of paired dialects on shared channels: US English (EN) on the `en` channel, Latin American Spanish (ES-419) on the `es` channel, and Brazilian Portuguese (PT-BR) on the `pt` channel. The secondary variants (British English `EN-GB`, Spain Spanish `ES`, and European Portuguese `PT`) are completely excluded from future video generation and publishing schedules.

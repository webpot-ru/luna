...
- `.github/workflows/youtube-thumbnail-batch-set.yml` - thumbnail-only repair/apply for already uploaded ordinary videos and prepared Polyglot thumbnail reports; for Polyglot reports it must call only `thumbnails.set` and update only Polyglot thumbnail state.
...
2026-07-04 thumbnail-only Polyglot apply rule: use `.github/workflows/youtube-thumbnail-batch-set.yml` only for already uploaded Polyglot videos with prepared JPG covers and a dry-run report that validates as ready. Pass the Polyglot report path through `manifest_path`; the workflow auto-detects it as a Polyglot report instead of an ordinary manifest. The first prepared batch is `outputs/review/youtube-thumbnail-batch-plan-upload-eligible-polyglot-24-20260704.json` plus JPGs under `outputs/design-prototypes/youtube-thumbnail-home_kitchen_cookware_pilot_01-upload-eligible-covers-20260704/polyglot/`; local validation is `24 ready / 0 blocked`, full-scope only, and excludes `KA`/`SR` and `short_unverified`. This workflow must not render video, upload video, update metadata, update playlists, change privacy/schedule, or call image-generation providers.

2026-07-04 live Polyglot thumbnail apply result: because route/project 1 quota was held, `youtube-api-branding` rows (`EN`, `ES`, `JA`, `PT`, `RU`, `TR`) were not run. The remaining applies succeeded for `TH` (`28703808108`, videos `5LSw2gkUw-g`, `h0WZwIlstZI`) and `NE/SW` (`28703851435`, videos `egEGUAHkeMo`, `kdiwJhOtAGw`, `38IW6aIiL4o`, `oOy7NcwDQEY`) and persisted custom thumbnail state. `MY` run `28703827216` failed before `thumbnails.set` because YouTube video readback returned no items for `TUkYm2swvMU` under the `youtube-api-youtube-3`/`MY` OAuth route; do not retry blindly until video ownership/OAuth visibility is checked.

...
If a channel does not yet have YouTube advanced features/custom thumbnail permission, do **not** fail the upload and do **not** reupload the video. Use `thumbnailUploadMode=first_frame_auto`: skip `thumbnails.set`, keep the first intro slide as the opening frame, let YouTube assign an automatic thumbnail from the video, and record `thumbnailSet=false`, `thumbnailSource=youtube-auto-first-frame`, `thumbnailFallbackReason` and, when applicable, `needsThumbnailPermission=true` in `config/youtube-published-videos.json`. The YouTube Data API does not provide a separate way to choose an arbitrary frame as the thumbnail; the controllable API path is either `thumbnails.set` or no custom thumbnail upload.

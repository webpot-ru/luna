...
- `.github/workflows/youtube-thumbnail-set.yml` is the thumbnail-only repair/test workflow for already uploaded videos. It does not render or upload video and does not create playlists. It requires an existing successful Actions `source_run_id` containing matching `youtube_metadata.json` plus `youtube_thumbnail.jpg`, then calls only `youtube.thumbnails.set` through `npm run set:youtube-thumbnail` after OAuth channel/video ownership readback. If the source artifact has only automatic-thumbnail metadata and no `youtube_thumbnail.jpg`, pass `generate_if_missing=true` plus `confirm_thumbnail_spend=GENERATE_THUMBNAILS`; the workflow generates the missing image on GitHub through VectorEngine before the YouTube write. Live use requires `confirm_youtube_write=SET_YOUTUBE_THUMBNAIL` and still spends YouTube Data API quota.
...
- `npm run check:youtube-metadata-language -- <metadata-file-or-dir> --output=<report.json>` is the support-language gate. It blocks obvious English-template metadata on non-English support channels: title/description phrases such as `A1 Vocabulary`, `Words with Pronunciation`, `Learn 50 essential...`, and English SEO tag clusters. This gate is intentionally separate from the SEO gate because a structurally good SEO description can still be in the wrong viewer language.
...
- non-English support metadata should have title, description, tags, hashtags, playlist title and playlist description in the support language. Brand text (`FlashcardsLuna`) is allowed; hidden tag lists are secondary for SEO and must not override visible native-language title/description quality;
- 2026-06-24 playlist localization incident rule: playlist copy must come from fresh `youtube_metadata.json` fields `playlistTitle` / `playlistDescription` when creating or updating playlists. Do not fall back to the generic English playlist template for non-English support channels. The language gate now blocks missing or English-template playlist fields in generated metadata files; existing live playlists with English text require a separate route-scoped `playlists.update(part=snippet)` repair and must not be fixed by reuploading videos;
...
Until that HY create-speech path is green, YouTube publishing must exclude Armenian both as support and target language. The publish workflow inputs `exclude_supports` and `exclude_langs` both default to `HY`, and `scripts/plan-youtube-generation-targets.mjs` supports `--exclude-supports`, `--exclude-targets` / `--exclude-langs`; the preflight report must show excluded HY rows as `skippedExcludedSupportCount` or `skippedExcludedTargetCount`, not render/upload attempts. Do not override with `exclude_supports=NONE` or `exclude_langs=NONE` for bulk runs until HY TTS write/readback is fixed.
...
Metadata включает `title`, `description`, `playlistTitle`, `playlistDescription`, `tags`, `hashtags`, `categoryId=27`, `privacyStatus`, `courseUrl`, `supportLang`, `targetLang`, `setId` и provenance (`source`, `model`, `generatedAt`). С 2026-06-21 production default is `privacyStatus=public`; `private`/`unlisted` are explicit test/pre-publication states only.
...
- existing live playlists that already have English fallback title/description must be repaired through `npm run repair:youtube-playlist-metadata` or `.github/workflows/youtube-playlist-metadata-repair.yml`, not by reuploading videos. The repair is route-scoped, excludes `HY` by default until Armenian TTS is unblocked, uses AI only with `GENERATE_PLAYLIST_METADATA`, and uses YouTube `playlists.update` only with `APPLY_YOUTUBE_PLAYLIST_UPDATE` after token/channel/readback checks. After an `apply` workflow, the separate `persist-playlist-metadata-state` job downloads that run's non-secret artifact and merges only confirmed rows with `playlistMetadataRepair.repairedAt` back into `config/youtube-playlists.json`; if that persist job fails, download the artifact and run `npm run merge:youtube-playlist-metadata-state -- --artifact-dir=.state-artifact` before launching dependent playlist repair batches;
- API-shape/readback guard: `playlists.list` readback can request `fields=items(...)`, but `playlists.update` returns a single playlist resource and must request fields like `id,snippet(...),status(...)` without `items(...)`. YouTube playlist updates can also show stale title/description on immediate readback, so the repair script retries playlist readback after update before marking a row failed. Run `node --check scripts/youtube-repair-playlist-metadata.mjs`, a read-only `npm run repair:youtube-playlist-metadata -- --route=<route>`, and `npm run check:youtube-playlist-naming` before retrying an apply batch after script changes;

...
GitHub metadata workflow выбирает VectorEngine, если задан ключ `VECTORENGINE_API_KEY` / `VECTOR_ENGINE_API_KEY`; иначе используется direct Google key, если он есть; иначе workflow остается успешным на template fallback. Recoverable AI-polish failures now also fall back to template metadata after one JSON-only retry, while missing keys or auth errors remain hard failures when an AI backend is explicitly selected. `VECTORENGINE_GEMINI_MODEL` нужен только для явного override модели. VectorEngine здесь используется только как Gemini text proxy для `youtube_metadata.json`; он не меняет видео-рендеринг, TTS, QR или slide templates.

...
2026-06-21 branding decision: public/video-facing brand is **`FlashcardsLuna`**, matching the real domain/search phrase `flashcardsluna.com`. New generated video intros/outros, video thumbnails, YouTube metadata, hashtags, playlist descriptions and desired channel descriptions must use `FlashcardsLuna`. Channel identity remains **`LunaCards`** for now: do not rename channel titles/handles and do not redraw the accepted channel banners just for this copy change unless the user explicitly opens a separate channel-identity redesign task.

Правило баннера: не перечислять на главном channel art только 2-3 изучаемых языка, потому что это искусственно сужает канал при каталоге 50+ языков. Принятые баннеры остаются в текущем LunaCards channel-identity style: broad flashcard-learning promise, место для будущего расширения за пределы языков и сайт `flashcardsluna.com`. Конкретные target languages живут в плейлистах, названиях видео, metadata и ссылках `?langs=<target>`. Если канал намеренно остается только языковым, допустим narrower promise вроде `Learn 50+ Languages`; для зонтичного LunaCards channel identity предпочтительнее `Learn with Flashcards` / `Languages and more`, чтобы не закрывать будущие неязыковые колоды.
...
5. upload video as `private` or `unlisted` first;
...
9. write `videoId`, `playlistId`, `playlistItemId`, status, privacy, and readback timestamp to a machine-readable ledger;
10. make public only after spot-checking the video, thumbnail, description URL, playlist membership and channel placement.

...
- 2026-07-09 metadata backend correction: bulk ordinary dispatch now passes direct Gemini metadata settings explicitly (`metadata_gemini_backend=api`, `metadata_concurrency=1`, `metadata_batch_size=4`, `metadata_rate_limit_ms=15000`). VectorEngine metadata is no longer an automatic second backend when both direct Gemini and VectorEngine secrets exist; any VectorEngine text metadata run requires `confirm_vectorengine_metadata=USE_VECTORENGINE_METADATA`. In apply mode metadata is generated before video render/TTS for each support, so direct Gemini quota failure stops before spending video-generation resources.
...
GitHub metadata workflow использует direct Google Gemini API by default: `metadata_gemini_backend=api`. Direct keys пробуются в порядке `GEMINI_API_KEY`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY`; если все прямые ключи дают ошибку в `mode=apply`, workflow должен падать до video render/TTS/upload, а не уходить молча в другой paid backend. VectorEngine text metadata is reserve-only: его можно включить только явно через `metadata_gemini_backend=vectorengine` или `api,vectorengine` плюс `confirm_vectorengine_metadata=USE_VECTORENGINE_METADATA`. Если прямых ключей нет, GitHub workflow не подставляет VectorEngine автоматически. Если AI backend не настроен или отключен, `mode=plan` может оставаться успешным на template fallback, но `mode=apply` остается fail-closed через `--require-ai-metadata`. Recoverable AI-polish failures may fall back to template metadata for plan diagnostics; in apply the workflow exports `YOUTUBE_METADATA_AI_STRICT=1`, so direct Gemini quota/timeout/JSON failures stop the run before render/TTS spend.

2026-07-09 direct Gemini batching policy: чтобы не создавать много маленьких Gemini API requests, ordinary metadata generation supports direct-API batches through `scripts/generate-youtube-metadata.mjs --gemini-batch-size <n> --gemini-rate-limit-ms <ms>`. Bulk dispatch defaults are `metadata_gemini_backend=api`, `metadata_concurrency=1`, `metadata_batch_size=4`, `metadata_rate_limit_ms=15000`. The batch prompt still passes per-video facts (`supportLang`, `targetLang`, localized target name, deck title, word count, course URL and sample words) and requires one JSON item per target, so quality constraints remain the same while request burst size drops. Batch mode is direct-API only; `cli` and `vectorengine` stay per-target unless separately designed and tested. Direct Google 3.5 calls use the current Interactions API (`POST /v1beta/interactions`) with `response_format` JSON schema, `thinking_level=low`, and bounded `GEMINI_API_TIMEOUT_MS` (default 30000ms). Single-item ordinary direct metadata uses `max_output_tokens=3200`; batch metadata scales output budget by item count up to a bounded cap. `VECTORENGINE_GEMINI_MODEL` нужен только для явного VectorEngine override модели. VectorEngine stays on the generateContent-compatible proxy path and remains only a reserve Gemini text proxy for `youtube_metadata.json`; it does not change video rendering, TTS, QR or slide templates. Because the VectorEngine compatibility layer can ignore or weaken JSON-mode behavior on longer prompts, `scripts/lib/youtube-metadata.mjs` uses a compact VectorEngine-specific metadata prompt, avoids character-count instructions that triggered prose/counting output, and gives that backend a larger `maxOutputTokens=3200` budget so Russian descriptions plus tags can finish with a closing JSON object.

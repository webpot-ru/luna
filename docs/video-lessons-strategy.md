...
GitHub metadata workflow сначала использует direct Google Gemini API, если задан хотя бы один из `GEMINI_API_KEY`, `GEMINI_API_KEY_2` или `GOOGLE_API_KEY`. Direct keys пробуются в этом порядке; если все прямые ключи дают ошибку, workflow переходит к VectorEngine при наличии `VECTORENGINE_API_KEY` / `VECTOR_ENGINE_API_KEY`. Если прямых ключей нет, но VectorEngine есть, используется VectorEngine. Если AI backend не настроен, `mode=plan` остается успешным на template fallback. Recoverable AI-polish failures still fall back to template metadata after one JSON-only retry for plan diagnostics, while missing keys or auth errors remain hard failures when an AI backend is explicitly selected and no later backend succeeds. `mode=apply` is stricter: SEO and publish-plan gates pass `--require-ai-metadata`, and the direct uploader also blocks `template` / `template-ai-fallback` before OAuth or YouTube writes. `VECTORENGINE_GEMINI_MODEL` нужен только для явного override модели. Direct Google 3.5 calls use the current Interactions API (`POST /v1beta/interactions`) with `response_format` JSON schema, `thinking_level=low`, and bounded `GEMINI_API_TIMEOUT_MS` (default 30000ms) so a stuck direct key can fall through to the next key/backend. Ordinary direct metadata uses `max_output_tokens=3200` because Gemini 3.5 can spend substantial output budget on internal thoughts before returning text. VectorEngine stays on the generateContent-compatible proxy path. VectorEngine здесь используется только как reserve Gemini text proxy для `youtube_metadata.json`; он не меняет видео-рендеринг, TTS, QR или slide templates. Because the VectorEngine compatibility layer can ignore or weaken JSON-mode behavior on longer prompts, `scripts/lib/youtube-metadata.mjs` uses a compact VectorEngine-specific metadata prompt, avoids character-count instructions that triggered prose/counting output, and gives that backend a larger `maxOutputTokens=3200` budget so Russian descriptions plus tags can finish with a closing JSON object. Polyglot live metadata follows the same policy: direct Google Gemini first, VectorEngine only after direct-key failure, and no publish-ready template fallback in `--require-ai` apply mode.

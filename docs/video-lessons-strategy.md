...
- `.github/workflows/youtube-thumbnail-set.yml` is the thumbnail-only repair/test workflow for already uploaded videos. It does not render or upload video and does not create playlists. It requires an existing successful Actions `source_run_id` containing matching `youtube_metadata.json` plus `youtube_thumbnail.jpg`, then calls only `youtube.thumbnails.set` through `npm run set:youtube-thumbnail` after OAuth channel/video ownership readback. Live use requires `confirm_youtube_write=SET_YOUTUBE_THUMBNAIL` and still spends YouTube Data API quota.

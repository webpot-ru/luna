...
Current apply invariants are enforced in code: one workflow owns one physical support channel across branches, different channels may run in parallel, `worker_count=1`, `allow_republish=false`, scheduled publication is the default, direct `publish_at` is disabled, and the uploader requires a healthy strict live-control report no older than 3 hours (180 minutes). Reusable apply workers additionally require the internal `campaign_owned_apply=true` flag from the claimed parent campaign; manual dispatch cannot provide it. Both the early gate and the immediate pre-upload refresh remain strict for campaign-owned workers. Worker evidence is scoped to the candidate video type (`ordinary` or `polyglot`) so an unfinished other-type lane from the same one-finalizer campaign is not misclassified as registry drift; ordinary evidence is bound to the exact worker targets and Polyglot evidence to the exact bundle candidate. Shared calendar collisions and fresh unclassified uploads are still global blockers, and the uploader rejects a scoped report for the wrong video type. `.github/workflows/youtube-publication-control.yml` is the canonical all-route read-only answer to “what is public, scheduled, private-unscheduled, missing or duplicated”; the same run also enumerates all owned playlists and playlist items for stable playlist-identity discovery.
Current apply invariants are enforced in code: one workflow owns one physical support channel across branches, different channels may run in parallel, `worker_count=1`, `allow_republish=false`, scheduled publication is the default, direct `publish_at` is disabled, and the uploader requires a healthy strict live-control report no older than 3 hours (180 minutes). Reusable apply workers additionally require the internal `campaign_owned_apply=true` flag from the claimed parent campaign; manual dispatch cannot provide it. Both the early gate and the immediate pre-upload refresh remain strict for campaign-owned workers. Worker evidence is scoped to the candidate video type (`ordinary` or `polyglot`) so an unfinished other-type lane from the same one-finalizer campaign is not misclassified as registry drift; ordinary evidence is bound to the exact worker targets and Polyglot evidence to the exact bundle candidate. Shared calendar collisions and fresh unclassified uploads are still global blockers, and the uploader rejects a scoped report for the wrong video type. `.github/workflows/youtube-publication-control.yml` is the canonical all-route read-only answer to “what is public, scheduled, private-unscheduled, missing or duplicated”; the same run also enumerates all owned playlists and playlist items for stable playlist-identity discovery. A zero-upload exact recovery may opt into the child worker's `target_playlist_only` preflight only with a durable existing playlist ID and `ALLOW_TARGET_PLAYLIST_ONLY_PREFLIGHT`; this narrows only the playlist identity read and never authorizes playlist creation or replaces a normal all-route control.

For a mixed request such as “next 5 ordinary plus 1 full Polyglot per channel”, [YouTube Publication Campaigns](youtube-publication-campaigns.md) is the primary execution contract. Do not launch 102 independent selectors. Dispatch one no-watch publication-control run with `campaign_ordinary_per_channel=5`, `campaign_polyglot_per_channel=1` and `persist_snapshot=false`; after all four active routes are selected it performs four route readbacks and emits one immutable 51-channel no-spend manifest from the same evidence. For the approved all-tail mode, use high per-channel maxima together with `campaign_allow_partial_ordinary_tail=true`, `campaign_allow_partial_polyglot_tail=true`, `campaign_allow_partial_route_quota_tail=true` and the real per-route cap (currently `1100`): the manifest preserves every available tail except an explicitly recorded unavoidable next-quota-day remainder. A route with `publicationReady=false` stops planning/preflight before provider work and cannot be substituted with another route. Only after review/approval, claim every assignment and calendar slot in one fail-closed operation before apply, batch metadata by the activated route environments, call campaign-owned child workflows, then run one finalizer. Child persistence is disabled automatically when `campaign_id` is present. A normal 306-video campaign uses 63 route-batched metadata requests instead of 102 worker requests. Claims remain active on partial failure; recovery requires a new read-only audit and separate approval.

For a proven zero-upload campaign failure, do not keep repeating live audits when a complete pre-apply report and one complete post-failure report already have identical exact video-ID sets. Generate the replacement with `--replacement-campaign-id=<old-id>`, then use `npm run rearm:youtube-publication-campaign` in dry-run mode. Apply requires the exact zero-upload confirmation and may only supersede old campaign/calendar claims and write the new immutable manifest/claims; it must preserve all assignment keys, reallocate unsafe near-term slots and perform zero provider/YouTube calls. Commit/push the exact durable state before any separately approved publication dispatch. An explicitly authorized all-tail expansion additionally requires `--allow-expanded-zero-upload-recovery` and `EXPAND_ZERO_UPLOAD_YOUTUBE_PUBLICATION_CAMPAIGN`: every old key must remain in the new manifest, at least one new free tail must be added, and the operation must retain the same live-ID, claim, slot and zero-receipt proofs. It records `superseded_zero_upload_expansion`; it is not a retry and does not publish by itself.

For a `claimed` campaign that was never dispatched but whose slots became unsafe, use `npm run rollover:youtube-claimed-campaign` instead of pretending it failed or editing dates in place. It requires one fresh complete exact-scope control report, zero selected live matches/blockers, zero dispatch/finalizer/artifact/receipt evidence and exact old manifest/calendar integrity. Dry-run is the default; apply uses `ROLLOVER_UNLAUNCHED_CLAIMED_YOUTUBE_CAMPAIGN`, preserves the exact assignment set, supersedes only the old claims and writes a new immutable future-slot manifest. It performs zero provider/YouTube calls and does not authorize publication dispatch.

Campaign metadata keeps up to 5 assignments in one request and, after explicit `USE_OPENAI_METADATA`, uses only OpenAI Responses with `gpt-5.6-terra` and `gpt-5.6-luna`; direct Gemini and VectorEngine are disabled for this campaign contour. OpenAI uses Structured Outputs, `store=false`, `OPENAI_SERVICE_TIER=auto` by default and records the successful response model, actual tier and token usage; `flex` is opt-in, while OpenAI Batch remains outside this synchronous workflow because completion may take up to 24 hours. Batches below the large-campaign threshold start with Terra and may make one Luna alternate attempt. Campaigns with at least 100 assignments start with Luna and may make one Terra alternate attempt. A foreign, missing or duplicate request ID, incomplete/refused response, invalid JSON, timeout, network failure, HTTP 429 or HTTP 5xx is never accepted and never retried on the same model; after one failed alternate the route stops fail-closed. Batch size `10` remains forbidden after run `29323860280`; production maximum is `5`. The eight route metadata matrix jobs use bounded `max-parallel: 4`: up to four projects may call Terra/Luna concurrently, while the remaining routes stay queued. The single 2,000,000-token campaign budget is still distributed proportionally across route jobs and must not be multiplied by eight. Metadata failure blocks every render/TTS/upload child. Each completed route batch is checkpointed and the route artifact is uploaded under `if: always()`. Reuse is opt-in only through exact `metadata_resume_run_id` on a separately approved recovery dispatch; ownership/checksum/batch mismatch fails before provider calls, and no recovery dispatch is automatic.

After metadata, the parent starts all eight combined project routes together. Each route owns its ordinary queue and then starts its own Polyglot queue immediately after that route's ordinary jobs finish; it does not wait for ordinary jobs in other projects. Both phases keep the five-support-per-project cap, ordinary keeps three local video builds per support, and the shared per-channel apply lock prevents ordinary and Polyglot from writing to the same physical channel at once. This worker-level increase must not change voice mapping, TTS rate/pitch, render arguments or QA gates. A partial ordinary failure does not suppress independent Polyglot rows in that route; the single finalizer reconciles exact receipts from both phases.

Publication workers may cache only reproducible runner dependencies that do not define content identity: npm downloads, pip downloads and the Playwright Chromium browser payload. They must still run the same dependency installation commands, voice/TTS contract, render commands and output validation. A hosted runner may reuse its existing FFmpeg executable after printing its version; if it is absent, the workflow installs FFmpeg through the original apt path. Never treat dependency caches as reusable audio, MP4, metadata, receipt or publication evidence.

Parallel Edge TTS generation must use unique temporary MP3 paths and atomically publish only signature-valid files into the deterministic shared cache; a retry must never delete the canonical path another worker may already be reading. The Lao production voice remains `lo-LA-KeomanyNeural`, with `lo-LA-ChanthavongNeural` allowed only as its deterministic fallback after bounded primary retries. A resumable `videos.insert` init may be retried only before media upload and before any YouTube ID exists, for bounded network/`408`/`409`/`425`/`429`/`5xx` failures. Never extend this retry to the media PUT or to a response that may already represent an accepted upload.

Use `.github/workflows/youtube-openai-metadata-canary.yml` for one explicitly approved provider check before a publication campaign. It performs exactly one OpenAI Responses request for two synthetic YouTube metadata tasks, validates the exact request IDs and structured fields, and uploads one evidence report containing the actual model, service tier and token usage. It has no render, TTS, calendar, campaign claim, OAuth or YouTube code path. Do not substitute a publication `apply` workflow for this canary.
...
The control workflow always uploads `youtube-publication-snapshot.json`, `youtube-playlist-discovery-snapshot.json` and a compact Markdown map. Its optional `persist_snapshot=true` job commits only `config/youtube-publication-snapshot.json`, `config/youtube-playlist-discovery-snapshot.json` and `docs/youtube-publication-map.md` to the selected branch after the eight-route audit; it performs no YouTube, metadata, render, TTS or image write. Keep the default `false` for a strictly read-only audit. Persisted snapshots are historical inventory, never apply tokens: live upload still requires a new per-support strict report with complete video and playlist pagination, no duplicate/registry/calendar blockers and age below 3 hours (180 minutes).

When complete all-route reports prove the live state but the durable ordinary/Polyglot ledgers are stale, use `npm run reconcile:youtube-publication-registry-control -- --report=<relevant-all-routes.json>`. Pass one or more complete reports, but never mix in a stale deck report merely to satisfy an argument count. It is dry-run by default; `--apply` changes only `config/youtube-published-videos.json`, `config/youtube-polyglot-published-videos.json` and `config/youtube-polyglot-progress.json`. The command refuses incomplete pagination/status evidence and live duplicates, restores observed live IDs as active durable rows, marks conflicting registry-only assignments inactive, migrates historical Polyglot rows out of the ordinary registry, and propagates exact confirmed deletion tombstones into matching Polyglot progress items so deleted slots can be planned again. Rebuild all route reports and the combined snapshot from the same audit artifacts after apply. This recovery never authorizes a GitHub dispatch, YouTube upload, deletion or schedule change.
...
Use `npm run reconcile:youtube-calendar-snapshot` to compare a complete persisted snapshot with `config/youtube-publish-calendar.json`. It is dry-run by default. A row with `cancelledAt`, `deletedAt` or `supersededAt` is inactive even when historical `status` still says `reserved`. `--apply` changes only the local calendar file and must skip every live duplicate group, ambiguous assignment, different-video assignment and occupied physical-channel slot. After apply, rebuild the route reports from the same audit artifacts and require zero assignment duplicates and slot collisions. This command never changes a YouTube schedule; any remote reschedule remains a separately approved YouTube write.
...
Exact transfer of an already-uploaded video into a canonical playlist is governed by [YouTube Publication Campaigns](youtube-publication-campaigns.md#перенос-уже-загруженного-видео-в-канонический-плейлист). It is a separate `playlistItems.insert` workflow with fresh same-channel readback and verified receipts; it never reuploads the video or deletes source playlists. For the 2026-09-26 exact recovery, three previously accepted memberships must be verified as already present, at most 17 new inserts are allowed, and post-insert visibility is awaited by bounded GET-only checks without repeating POST.

The exact 61-playlist visibility cleanup is documented in [YouTube Publication Campaigns](youtube-publication-campaigns.md#видимость-дополнительных-плейлистов-после-переноса). It may change only reviewed extra playlists from public to unlisted after fresh four-route membership and ownership verification; canonical playlists and videos remain untouched. Verified per-playlist receipts, not the dispatch badge, are durable state; no blanket retry or deletion is allowed.
The first apply `36225672576` made zero visibility changes because YouTube rejected `part=status` plus a `snippet` body with HTTP 400 `unexpectedPart`. The corrected request declares `part=snippet,status`, resends existing mutable snippet/status fields unchanged except `privacyStatus=unlisted`, blocks legacy tags, and verifies metadata after readback. Only a separately authorized single replacement dispatch may use that fix.

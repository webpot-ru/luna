...
Campaign metadata keeps up to 10 assignments in one Gemini request. For `gemini-3.5-flash`, set `maxOutputTokens=60000` (below the model's `65536` output limit) and keep generated descriptions concise so the cap is reserve capacity rather than a target. A non-`STOP` finish reason, invalid/truncated JSON or incomplete exact request-ID set is a recoverable response-integrity failure: rotate once to the second direct Gemini key, then use the explicitly confirmed VectorEngine backend; never retry the same key blindly. Metadata failure still blocks every render/TTS/upload child.

...
After metadata, the parent starts all eight combined project routes together. Each route owns its ordinary queue and then starts its own Polyglot queue immediately after that route's ordinary jobs finish; it does not wait for ordinary jobs in other projects. Both phases keep the five-support-per-project cap, ordinary keeps two local video builds per support, and the shared per-channel apply lock prevents ordinary and Polyglot from writing to the same physical channel at once. A partial ordinary failure does not suppress independent Polyglot rows in that route; the single finalizer reconciles exact receipts from both phases.

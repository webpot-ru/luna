...
For a mixed request such as “next 5 ordinary plus 1 full Polyglot per channel”, [YouTube Publication Campaigns](youtube-publication-campaigns.md) is the primary execution contract. Do not launch 102 independent selectors. Dispatch one no-watch publication-control run with `campaign_ordinary_per_channel=5`, `campaign_polyglot_per_channel=1` and `persist_snapshot=false`; after all eight routes are activated it performs eight route readbacks and emits one immutable 51-channel no-spend manifest from the same evidence. For the approved all-tail mode, use high per-channel maxima together with `campaign_allow_partial_ordinary_tail=true`, `campaign_allow_partial_polyglot_tail=true`, `campaign_allow_partial_route_quota_tail=true` and the real per-route cap (normally `100`): the manifest preserves every available tail except an explicitly recorded unavoidable next-quota-day remainder. A route with `publicationReady=false` stops planning/preflight before provider work and cannot be substituted with another route. Only after review/approval, claim every assignment and calendar slot in one fail-closed operation before apply, batch metadata by the activated route environments, call campaign-owned child workflows, then run one finalizer. Child persistence is disabled automatically when `campaign_id` is present. A normal 306-video campaign uses 67 route-batched metadata requests instead of 102 worker requests. Claims remain active on partial failure; recovery requires a new read-only audit and separate approval.

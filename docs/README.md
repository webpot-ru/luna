...
- активные/generated post-reset ordinary Deck Master Plan колоды в Postgres: Sort 1-49. Local readback on 2026-07-07 shows all 49 plan rows as `quality_status=generated_checked` and `selection_status=approved`; this excludes out-of-plan `kids_numbers_counting_a1` and candidate-pool-only rows;
- newly synced tail: Sort 39 `Basic Ingredients & Spices` / `food_basic_ingredients_spices_a1_a2`, Sort 40 `Grocery Shopping Words` / `food_grocery_shopping_words_a2`, Sort 41 `Market Shopping Words` / `shopping_market_shopping_words_a2`, Sort 42 `Restaurant Words` / `food_restaurant_words_a2`, Sort 43 `Advanced Foods & Seafood` / `food_advanced_foods_seafood_b1_b2`, Sort 44 `Street & City Places` / `city_street_city_places_a1_a2`, Sort 45 `City Transport Basics` / `city_transport_basics_a1_a2`, Sort 46 `Metro & Public Transport Words` / `city_metro_public_transport_words_a2`, Sort 47 `Direction Words` / `city_direction_words_a1_a2`, Sort 48 `Taxi & Ride-share Words` / `city_taxi_ride_share_words_a2`, Sort 49 `Car & Driving Basics` / `city_car_driving_basics_a2`;
- next deck state: Sort 50 `Travel Basics` / `travel_basics_a1` exists locally as `generated` / `candidate_pool`, but is not approved/generated_checked and still needs spec/candidate-pool evidence review and explicit approval before it can count as ready;
- отсутствующие в Postgres и не считающиеся current coverage: no Sort 1-49 ordinary deck gaps after the 2026-07-07 local readback;

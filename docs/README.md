...
| [YouTube API Project Routing](youtube-api-project-routing.md) | Source of truth, active/standby routing contract | Распределение 51 публичного YouTube support-channel / 54 support-language variants по восьми маршрутам; `youtube-1`–`youtube-4` готовы, для `youtube-5`–`youtube-8` GitHub OAuth secrets созданы, но действует fail-closed до полного read-only token/channel readback; резервные пары `1/5`, `2/6`, `3/7`, `4/8`, единый предел 400 upload calls/day и запрет автоматического quota fallback. |

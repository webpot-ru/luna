...
Current post-reset ordinary DB status on 2026-07-07: local Postgres confirms Deck Master Plan Sort 1-49 as current ordinary generated coverage. All 49 plan rows have `quality_status=generated_checked`, `selection_status=approved` and 54-language DB coverage; Sort 39-49 are each 32 cards x 54 languages = 1,728 language rows. Pre-reset Google Sheets remain historical unless a row explicitly says it was rebuilt post-reset. Exact Google Sheet upload/readback/freshness evidence is documented in this file through the earlier delivered rows and must be backfilled from original runner/Drive artifacts for Sort 39-49 before making Sheet-ID or delivery-freshness claims for those rows. Sort 50 `Travel Basics` exists locally as `generated` / `candidate_pool`, but is not approved/generated_checked and does not count as ready ordinary coverage.

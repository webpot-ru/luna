...
- If a GitHub Actions bulk dispatcher hits `HTTP 403: API rate limit exceeded` for installation tokens, launch child workflows directly from local CLI via `gh workflow run ...` under the user's OAuth account to use the higher user API quota while keeping compute on cloud runners.
